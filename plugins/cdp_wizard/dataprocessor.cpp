...
int DataProcessor::firstMonthOfHighestQ (float *theClimateArray, int theArrayLength)
{
  ...
  catch (...)
  {
    ...
    myHighestBlockInt = -9999;
  }


  //Return highest value over the quarter (add 1 to get it into base 1 instead of base0)
  return myHighestBlockInt+1;
}
/** This method will return the mean over three months in
  theClimateArray, starting at theCurrentBlock. */
float DataProcessor::meanOverQuarter (float *theClimateArray, int theArrayLength, int theCurrentBlock)
{

  if (debugModeFlag) cout << "DataProcessor::meanOverQuarter (float *theClimateArray, int theArrayLength, int theCurrentBlock) called" << endl;
  if (debugModeFlag) cout << "==========================================================" << endl;
  ...
  try
  {
    ...
    if (theCurrentBlock + QUARTER < theArrayLength)
    {
      //No wrapping necessary
      for (myInt = theCurrentBlock-1; myInt < (theCurrentBlock-1 + (QUARTER)); myInt++)
      {
    ...
    else
    {
      ...
      for (mySegmentInt=theCurrentBlock-1; mySegmentInt < theArrayLength; mySegmentInt++)
      {
      ...
      //Read remainding months from beginning of array
      for (mySegmentInt = 0; mySegmentInt < (QUARTER - (theArrayLength - theCurrentBlock) - 1); mySegmentInt++)
      {
...
float DataProcessor::meanOverLowestQ(std::vector <float> theClimateVector)
{
  ...
  float *myClimateArray=new float[myArrayLengthInt];
...
/**Overloaded version that takes a vector */
float DataProcessor::meanOverQuarter (std::vector <float> theClimateVector, int theCurrentBlock)
{
...
float DataProcessor::greatestTotalRange (std::vector <float> theClimateVector, std::vector <float> theClimateVector2)
{
  ...
  float *myClimateArray=new float[myArrayLengthInt];
  float *myClimateArray2=new float[myArrayLengthInt2];
  //iterate through the climate vector, adding each element to the array
  for (int i=0;i < myArrayLengthInt; i++)
  {
    myClimateArray[i] = theClimateVector[i];
