...
static const char * const name_ = "MapCanvasDemo";
static const char * const description_ = "This is a simple plugin to demonstrate how the mapcanvas can be manipulated through the plugin interface.";
...
void Plugin::initGui()
{
  ...
  pluginMenu->insertItem(QIconSet(icon),"&Mapcanvas Test", this, SLOT(run()));

  menuBarPointer = ((QMainWindow *) qgisMainWindowPointer)->menuBar();

  menuIdInt = qGisInterface->addMenu("&Demo", pluginMenu);
...
void Plugin::run()
{
  PluginGui *myPluginGui=new PluginGui(qgisMainWindowPointer,"Mapcanvas Test",true,0);
