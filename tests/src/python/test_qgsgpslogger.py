...
class TestQgsGpsLogger(QgisTestCase):
    ...
    def test_setters(self):
        ...
        logger.setDestinationField(
            Qgis.GpsInformationComponent.Timestamp, "point_time_field"
        )
        self.assertEqual(
            logger.destinationField(Qgis.GpsInformationComponent.Timestamp),
            "point_time_field",
        )

        logger.setDestinationField(
            Qgis.GpsInformationComponent.TrackDistanceSinceLastPoint,
            "point_distance_from_previous_field",
        )
        self.assertEqual(
            logger.destinationField(
                Qgis.GpsInformationComponent.TrackDistanceSinceLastPoint
            ),
            "point_distance_from_previous_field",
        )
        ...
        logger.setDestinationField(
            ...
            "point_delta_from_previous_field",
        )
        self.assertEqual(
            logger.destinationField(
                Qgis.GpsInformationComponent.TrackTimeSinceLastPoint
        ...
        logger.setDestinationField(
            Qgis.GpsInformationComponent.TrackStartTime, "track_start_time"
        )
        self.assertEqual(
            logger.destinationField(Qgis.GpsInformationComponent.TrackStartTime),
            "track_start_time",
        )

        logger.setDestinationField(
            Qgis.GpsInformationComponent.TrackStartTime, "track_end_time"
        )
        self.assertEqual(
            logger.destinationField(Qgis.GpsInformationComponent.TrackStartTime),
            "track_end_time",
        ...
        logger.setDestinationField(
            Qgis.GpsInformationComponent.TotalTrackLength, "track_length"
        )
        self.assertEqual(
            logger.destinationField(Qgis.GpsInformationComponent.TotalTrackLength),
            "track_length",
        )
    ...
    def test_current_geometry(self):
        ...
        gps_connection.send_message(
            "$GPRMC,084111.185,A,6938.6531,N,01856.8527,E,0.16,2.00,220120,,,A*6E"
        )
        ...
        gps_connection.send_message(
            "$GPGGA,084112.185,6939.6532,N,01856.8526,E,1,04,1.4,35.0,M,29.4,M,,0000*63"
        )
        res, err = logger.currentGeometry(QgsWkbTypes.Type.PointZ)
        self.assertFalse(err)
        ...
        self.assertFalse(err)
        self.assertEqual(
            res.asWkt(4), "LineString Z (18.9475 69.6442 0, 18.9475 69.6609 35)"
        )
        res, err = logger.currentGeometry(QgsWkbTypes.Type.MultiLineStringZ)
        self.assertFalse(err)
        self.assertEqual(
            res.asWkt(4), "MultiLineString Z ((18.9475 69.6442 0, 18.9475 69.6609 35))"
        )
        ...
        gps_connection.send_message(
            "$GPGGA,084112.185,6939.6532,N,01866.8526,E,1,04,1.4,35.0,M,29.4,M,,0000*63"
        )
        ...
        self.assertEqual(
            res.asWkt(4),
            "LineString (18.9475 69.6442, 18.9475 69.6609, 19.1142 69.6609)",
        )
        res, err = logger.currentGeometry(QgsWkbTypes.Type.LineStringZ)
        self.assertFalse(err)
        self.assertEqual(
            res.asWkt(4),
            "LineString Z (18.9475 69.6442 0, 18.9475 69.6609 35, 19.1142 69.6609 35)",
        )
        res, err = logger.currentGeometry(QgsWkbTypes.Type.MultiLineStringZ)
        self.assertFalse(err)
        self.assertEqual(
            res.asWkt(4),
            "MultiLineString Z ((18.9475 69.6442 0, 18.9475 69.6609 35, 19.1142 69.6609 35))",
        )
        ...
        self.assertEqual(
            res.asWkt(4),
            "Polygon ((18.9475 69.6442, 18.9475 69.6609, 19.1142 69.6609, 18.9475 69.6442))",
        )
        res, err = logger.currentGeometry(QgsWkbTypes.Type.PolygonZ)
        self.assertFalse(err)
        self.assertEqual(
            res.asWkt(4),
            "Polygon Z ((18.9475 69.6442 0, 18.9475 69.6609 35, 19.1142 69.6609 35, 18.9475 69.6442 0))",
        )
        res, err = logger.currentGeometry(QgsWkbTypes.Type.MultiPolygonZ)
        self.assertFalse(err)
        self.assertEqual(
            res.asWkt(4),
            "MultiPolygon Z (((18.9475 69.6442 0, 18.9475 69.6609 35, 19.1142 69.6609 35, 18.9475 69.6442 0)))",
        )

    def test_point_recording(self):
        points_layer = QgsVectorLayer(
            "PointZ?crs=EPSG:28355&field=timestamp:datetime&field=distance:double&field=seconds:double",
            "points",
            "memory",
        )
        self.assertTrue(points_layer.isValid())
        ...
        logger.setDestinationField(
            Qgis.GpsInformationComponent.TrackDistanceSinceLastPoint, "distance"
        )
        logger.setDestinationField(
            Qgis.GpsInformationComponent.TrackTimeSinceLastPoint, "seconds"
        )
        ...
        logger.setDestinationField(
            Qgis.GpsInformationComponent.TrackDistanceSinceLastPoint, None
        )
        ...
        logger.setDestinationField(
            Qgis.GpsInformationComponent.TrackTimeSinceLastPoint, None
        )
    ...
    def test_point_recording_no_z(self):
        points_layer = QgsVectorLayer(
            "Point?crs=EPSG:28355&field=timestamp:datetime&field=distance:double&field=seconds:double",
            "points",
            "memory",
        )
        self.assertTrue(points_layer.isValid())
    ...
    def test_point_recording_m_value_epoch(self):
        points_layer = QgsVectorLayer(
            "PointM?crs=EPSG:28355&field=timestamp:datetime&field=distance:double&field=seconds:double",
            "points",
            "memory",
        )
        self.assertTrue(points_layer.isValid())
        ...
        exp.setOffsetFromUtc(3000)
        self.assertEqual(
            f.geometry().asWkt(-3), "Point M (-1297000 21436000 1579682471000)"
        )
        ...
        self.assertEqual(len(spy), 2)
        self.assertEqual(logger.lastMValue(), 1579682473185.0)
        ...
        f = next(features)
        self.assertEqual(
            f.geometry().asWkt(-3), "Point M (-1297000 21435000 1579682473000)"
        )
    ...
    def test_point_recording_m_value_altitude(self):
        ...
        gps_connection.send_message(
            "$GPGGA,084112.185,6938.9152,N,01856.8526,E,1,04,1.4,3335.0,M,29.4,M,,0000*63"
        )
        self.assertEqual(len(spy), 2)
    ...
    def test_track_recording(self):
        line_layer = QgsVectorLayer(
            "LineStringZ?crs=EPSG:28355&field=start:datetime&field=end:string&field=length:double",
            "lines",
            "memory",
        )
        ...
        logger.setDestinationField(
            Qgis.GpsInformationComponent.TotalTrackLength, "length"
        )
        ...
        exp.setOffsetFromUtc(3000)
        self.assertEqual(
            f.attributes(), [exp, "2020-01-22T09:32:06+00:50", 0.021035000317942486]
        )
        self.assertEqual(
            f.geometry().asWkt(-2),
    ...
    def test_track_recording_no_z(self):
        line_layer = QgsVectorLayer(
            "LineString?crs=EPSG:28355&field=start:datetime&field=end:string&field=length:double",
            "lines",
            "memory",
        )
        self.assertTrue(line_layer.isValid())
    ...
    def test_track_recording_z_m_timestamp(self):
        line_layer = QgsVectorLayer(
            "LineStringZM?crs=EPSG:28355&field=start:datetime&field=end:string&field=length:double",
            "lines",
            "memory",
        )
        self.assertTrue(line_layer.isValid())
