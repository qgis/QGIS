...
class TestQgsField(unittest.TestCase):
    ...
    def test_metadata(self):
        ...
        self.assertEqual(field.metadata(Qgis.FieldMetadataProperty.CustomProperty + 2),
                         'test')

        # set standard metadata
        field.setMetadata(Qgis.FieldMetadataProperty.GeometryWkbType, Qgis.WkbType.LineStringZ)
        self.assertEqual(field.metadata(Qgis.FieldMetadataProperty.GeometryWkbType),
        ...
        field.setMetadata({
            Qgis.FieldMetadataProperty.GeometryCrs: QgsCoordinateReferenceSystem("EPSG:3111"),
