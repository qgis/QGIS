...
class TestQgsVtpk(QgisTestCase):
            ...
            tiles.styleDefinition(),
            {
                ...
                "layers": [
                    ...
                    },
                    {
                        ...
                        "paint": {"line-color": "#4C993A", "line-width": 1.33333},
                        "source": "esri",
                        "source-layer": "lines",
                        "type": "line",
                    },
    ...
    def testLayerMetadata(self):
        ...
        self.assertEqual(
            layer_metadata.extent().spatialExtents()[0].bounds.xMinimum(),
            -179.999988600592,
        )
        self.assertEqual(
            layer_metadata.extent().spatialExtents()[0].bounds.xMaximum(),
            179.999988540844,
        )
        self.assertEqual(
            layer_metadata.extent().spatialExtents()[0].bounds.yMinimum(),
            -85.0511277912625,
        )
        self.assertEqual(
            layer_metadata.extent().spatialExtents()[0].bounds.yMaximum(),
            85.0511277912625,
        )
        self.assertEqual(
            layer_metadata.extent().spatialExtents()[0].extentCrs.authid(), "EPSG:4326"
        )
    ...
    def testVectorTileLayerTileMap(self):
        ...
        layer = QgsVectorTileLayer(
            f"type=vtpk&url={unitTestDataPath() + '/vector_tile/vtpk_indexed.vtpk'}",
            "tiles",
        )
        ...
        layer2 = QgsVectorTileLayer(
            f"type=vtpk&url={unitTestDataPath() + '/vector_tile/vtpk_indexed.vtpk'}",
            "tiles",
        )
        tiles = layer2.tileMatrixSet().tilesInRange(QgsTileRange(0, 1023, 0, 1023), 10)
        self.assertCountEqual(tiles, expected)
