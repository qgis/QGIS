...
class TestQgsFileUtils(unittest.TestCase):
    ...
    def testExtensionsFromFilter(self):
        ...
        self.assertEqual(QgsFileUtils.extensionsFromFilter("(*.)"), [])
        self.assertEqual(
            QgsFileUtils.extensionsFromFilter("PNG Files (*.png)"), ["png"]
        )
        self.assertEqual(
            QgsFileUtils.extensionsFromFilter("PNG Files (*.PNG)"), ["PNG"]
        )
        self.assertEqual(
            QgsFileUtils.extensionsFromFilter("Geotiff Files (*.tiff *.tif)"),
            ["tiff", "tif"],
        )
        self.assertEqual(
            QgsFileUtils.extensionsFromFilter("TAR.GZ Files (*.tar.gz *.tgz)"),
            ["tar.gz", "tgz"],
        )
    ...
    def testWildcardsFromFilter(self):
        ...
        self.assertEqual(
            QgsFileUtils.wildcardsFromFilter("Geotiff Files (*.tiff *.tif *.TIFF)"),
            "*.tiff *.tif *.TIFF",
        )
        self.assertEqual(
    ...
    def testFileMatchesFilter(self):
        ...
        self.assertFalse(
            QgsFileUtils.fileMatchesFilter("/home/me/test.tif", "PNG Files (*.png)")
        )
        self.assertTrue(
            QgsFileUtils.fileMatchesFilter("/home/me/test.tif", "Tif  Files (*.tif)")
        )
        self.assertFalse(
            QgsFileUtils.fileMatchesFilter("/home/me/test.tif", "PNG Files (*.PNG)")
        )
        ...
        self.assertTrue(
            QgsFileUtils.fileMatchesFilter(
                "/home/me/test.tif", "Geotiff Files (*.tiff *.tif)"
            )
        )
        self.assertTrue(
            QgsFileUtils.fileMatchesFilter(
                "/home/me/test.tiff", "Geotiff Files (*.tiff *.tif)"
            )
        )
        self.assertTrue(
            QgsFileUtils.fileMatchesFilter(
        ...
        self.assertFalse(
            QgsFileUtils.fileMatchesFilter("/home/me/test.png", "EPT files (ept.json)")
        )
        self.assertTrue(
            QgsFileUtils.fileMatchesFilter("/home/me/ept.json", "EPT files (ept.json)")
        )
        self.assertTrue(
            QgsFileUtils.fileMatchesFilter(
                "/home/me/ept.json", "EPT files (ept.json EPT.JSON)"
            )
        )
        self.assertTrue(
            QgsFileUtils.fileMatchesFilter(
        ...
        self.assertFalse(
            QgsFileUtils.fileMatchesFilter(
                "/home/me/ep.json", "EPT files (ept.json);;Entwine files (entwine.json)"
            )
        )
    ...
    def testEnsureFileNameHasExtension(self):
        ...
        self.assertEqual(
            QgsFileUtils.ensureFileNameHasExtension("test", [".tif"]), "test.tif"
        )
        self.assertEqual(
            QgsFileUtils.ensureFileNameHasExtension("test", ["tif"]), "test.tif"
        )
        ...
        self.assertEqual(
            QgsFileUtils.ensureFileNameHasExtension("test.tif", ["bmp"]), "test.tif.bmp"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            QgsFileUtils.ensureFileNameHasExtension("test.tif", ["tiff", "tif"]),
            "test.tif",
        )
        self.assertEqual(
            QgsFileUtils.ensureFileNameHasExtension("test.tif", ["TIFF", "TIF"]),
    ...
    def testAddExtensionFromFilter(self):
        ...
        self.assertEqual(
            QgsFileUtils.addExtensionFromFilter("test.tif", ""), "test.tif"
        )
        self.assertEqual(
            QgsFileUtils.addExtensionFromFilter("test.tif", "BMP Files (*.bmp)"),
            "test.tif.bmp",
        )
        self.assertEqual(
            QgsFileUtils.addExtensionFromFilter("test.tif", "TIFF Files (*.tiff)"),
            "test.tif.tiff",
        )
        self.assertEqual(
            QgsFileUtils.addExtensionFromFilter(
                "test.tif", "TIFF Files (*.tif *.tiff)"
            ),
            "test.tif",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            QgsFileUtils.addExtensionFromFilter("test", "All Files (*.*)"), "test"
        )
    ...
    def testFindClosestExistingPath(self):
        ...
        self.assertEqual(
            QgsFileUtils.findClosestExistingPath(
                "/usr/youve_been_hacked/by_the_l77t_krew"
            ),
            "/usr",
        )
        ...
        self.assertEqual(
            QgsFileUtils.findClosestExistingPath(file), base_path
        )  # real file!
        self.assertEqual(
            QgsFileUtils.findClosestExistingPath(
                os.path.join(base_path, "non/existent/subfolder")
            ),
            base_path,
        )
        ...
        self.assertEqual(
            QgsFileUtils.findClosestExistingPath(bad_sub_folder), sub_folder2
        )
        self.assertEqual(QgsFileUtils.findClosestExistingPath(sub_folder2), sub_folder2)
        self.assertEqual(
            QgsFileUtils.findClosestExistingPath(sub_folder2 + "/."), sub_folder2
        )
        self.assertEqual(
            QgsFileUtils.findClosestExistingPath(sub_folder2 + "/.."), sub_folder1
        )
        self.assertEqual(
        ...
        self.assertEqual(
            QgsFileUtils.findClosestExistingPath(sub_folder2 + "/../subfolder2"),
            sub_folder2,
        )
        self.assertEqual(
            QgsFileUtils.findClosestExistingPath(
                sub_folder2 + "/../subfolder2/zxcv/asfdasd"
            ),
            sub_folder2,
        )
    ...
    def testAutoFinder(self):
        ...
        with open(os.path.join(side_nest, filename), "w+"):
            ...
            self.assertEqual(
                files[0], os.path.join(side_nest, filename).replace(os.sep, "/")
            )
        # side + side nest  =  2
        ...
        with open(os.path.join(temp_folder, filename), "w+"):
            files = QgsFileUtils.findFile(filename, base_folder, 3, 4)
            self.assertEqual(
                files[0], os.path.join(temp_folder, filename).replace(os.sep, "/")
            )
        # nest
        with open(os.path.join(nest, filename), "w+"):
            files = QgsFileUtils.findFile(os.path.join(base_folder, filename))
            self.assertEqual(
                files[0], os.path.join(nest, filename).replace(os.sep, "/")
            )
        # base level
        with open(os.path.join(base_folder, filename), "w+"):
            files = QgsFileUtils.findFile(filename, base_folder, 2, 4)
            self.assertEqual(
                files[0], os.path.join(base_folder, filename).replace(os.sep, "/")
            )
    ...
    def test_sidecar_files_for_path(self):
        ...
        self.assertFalse(
            QgsFileUtils.sidecarFilesForPath(f"{unitTestDataPath()}/kbs.qgs")
        )

        # shapefile
        self.assertEqual(
            QgsFileUtils.sidecarFilesForPath(f"{unitTestDataPath()}/lines.shp"),
            {
                f"{unitTestDataPath()}/lines.shx",
                f"{unitTestDataPath()}/lines.dbf",
                f"{unitTestDataPath()}/lines.prj",
            },
        )
        # gpkg
        self.assertFalse(
            QgsFileUtils.sidecarFilesForPath(f"{unitTestDataPath()}/points_gpkg.gpkg")
        )

        # MapInfo TAB file
        self.assertEqual(
            QgsFileUtils.sidecarFilesForPath(f"{unitTestDataPath()}/ogr_types.tab"),
            {
                f"{unitTestDataPath()}/ogr_types.dat",
                f"{unitTestDataPath()}/ogr_types.id",
        ...
        self.assertEqual(
            QgsFileUtils.sidecarFilesForPath(
                f"{unitTestDataPath()}/invalidgeometries.gml"
            ),
    ...
    def test_rename_dataset(self):
        ...
        # skip qml file
        res, error = QgsFileUtils.renameDataset(
            f"{base_path}/other_lines.shp",
            f"{base_path}/other_lines2.shp",
        ...
        with open(f"{base_path}/yy.shp", "w") as f:
            f.write("")
        res, error = QgsFileUtils.renameDataset(
            f"{base_path}/ll.shp", f"{base_path}/yy.shp", Qgis.FileOperationFlags()
        )
        ...
        with open(f"{base_path}/yyy.shx", "w") as f:
            f.write("")
        res, error = QgsFileUtils.renameDataset(
            f"{base_path}/ll.shp", f"{base_path}/yyy.shp"
        )
        self.assertFalse(res)
        self.assertTrue(error)
        ...
        res, error = QgsFileUtils.renameDataset(
            "/not a file.txt", f"{base_path}/not a file.txt", Qgis.FileOperationFlags()
        )
        self.assertFalse(res)
        self.assertTrue(error)
    ...
    def testSplitPathToComponents(self):
        ...
        self.assertEqual(
            QgsFileUtils.splitPathToComponents("/home/user"), ["/", "home", "user"]
        )
        self.assertEqual(QgsFileUtils.splitPathToComponents("/home"), ["/", "home"])
        self.assertEqual(QgsFileUtils.splitPathToComponents("/"), ["/"])
        self.assertEqual(QgsFileUtils.splitPathToComponents(""), [])
        self.assertEqual(
            QgsFileUtils.splitPathToComponents("c:/home/user"), ["c:", "home", "user"]
        )
    ...
    def testUniquePath(self):
        ...
        self.assertEqual(
            QgsFileUtils.uniquePath(os.path.join(temp_path, "test.txt")),
            os.path.join(temp_path, "test_2.txt"),
        )
        ...
        self.assertEqual(
            QgsFileUtils.uniquePath(os.path.join(temp_path, "test_2.txt")),
            os.path.join(temp_path, "test_2_2.txt"),
        )
        self.assertEqual(
            QgsFileUtils.uniquePath(os.path.join(temp_path, "test.txt")),
            os.path.join(temp_path, "test_3.txt"),
        )
        self.assertEqual(
            QgsFileUtils.uniquePath(os.path.join(temp_path, "test_1.txt")),
            os.path.join(temp_path, "test_1.txt"),
        )
