...
class TestPyQgsProviderConnectionOracle(
    ...
    def test_tables_with_options(self):
        ...
        def get_tables(
            schema,
            configuration,
            flags=QgsAbstractDatabaseProviderConnection.TableFlags(),
        ):
        ...
        self.assertEqual(
            get_tables(
                "QGIS", {}, QgsAbstractDatabaseProviderConnection.TableFlag.Aspatial
            ),
        ...
        self.assertEqual(
            get_tables(
                "QGIS", {}, QgsAbstractDatabaseProviderConnection.TableFlag.Vector
            ),
            [
                "LINE_DATA",
                "POINT_DATA",
        ...
        self.execSQLCommand(
            'CREATE TABLE OTHER_USER.OTHER_TABLE ( "pk" INTEGER PRIMARY KEY, GEOM SDO_GEOMETRY)'
        )
        ...
        self.assertEqual(
            get_tables("OTHER_USER", {"userTablesOnly": False}), ["OTHER_TABLE"]
        )
    ...
    def test_configuration(self):
        ...
        uri = (
            "authcfg='test_cfg' dbname='qgis_test' username='QGIS' password='qgis' dbworkspace='workspace' "
    ...
    def test_pkcols(self):
        ...
        self.execSQLCommand(
            """CREATE OR REPLACE VIEW "QGIS"."SOME_DATA_VIEW" AS SELECT * FROM "QGIS"."SOME_DATA" """
        )
        ...
        self.assertEqual(
            sorted(tables_dict["SOME_DATA_VIEW"]),
            ["GEOM", "cnt", "date", "dt", "name", "name2", "num_char", "pk", "time"],
        )
