...
class TestQgsServerAccessControlWMS(TestQgsServerAccessControl):
    ...
    def test_wms_getprojectsettings(self):
        ...
        self.assertTrue(
            str(response).find(
                "<LayerDrawingOrder>Country_Diagrams,Country_Labels,Country,dem,Hello_Filter_SubsetString,Hello_Project_SubsetString,Hello_SubsetString,Hello_Filter,Hello_OnOff,Hello,db_point</LayerDrawingOrder>"
            )
            != -1,
        ...
        self.assertTrue(
            str(response).find(
                "<LayerDrawingOrder>Country_Diagrams,Country_Labels,dem,Hello_Filter_SubsetString,Hello_Project_SubsetString,Hello_SubsetString,Hello_Filter,Hello,db_point</LayerDrawingOrder>"
            )
            != -1,
    ...
    def test_wms_getcontext(self):
        ...
        self.assertTrue(
            str(response).find('name="Country"') < str(response).find('name="Hello"'),
    ...
    def test_wms_describelayer_country(self):
        ...
        response, headers = self._get_fullaccess(query_string)
        self.assertTrue(
            str(response).find("<se:FeatureTypeName>Country</se:FeatureTypeName>")
            != -1,
    ...
    def test_wms_getmap_subsetstring_with_filter(self):
        ...
        self._img_diff_error(
            response, headers, "Restricted_WMS_GetMap_projectsubstring"
        )

        filter = (
            "<Filter><Or><PropertyIsEqualTo><PropertyName>pkuid</PropertyName><Literal>7</Literal>"
        ...
        response, headers = self._get_restricted(query_string)
        self._img_diff_error(
            response, headers, "Restricted_WMS_GetMap_projectsubstring_OGC"
        )
    ...
    def test_security_issue_gh32475(self):
        ...
        class Filter(QgsAccessControlFilter):
            ...
            def layerFilterSubsetString(self, layer):
                ...
                if handler.parameter("LAYER_PERM") == "yes":
                    if (
                        layer.name() == "as_symbols"
                        or layer.shortName() == "as_symbols"
                    ):
                        return '"gid" != 1'
                return None
        ...
        def _gfi(restrict, layers):
            qs = (
                ...
                + "&INFO_FORMAT=application/json&I=508&J=560&FEATURE_COUNT=10"
            ) % layers
            if restrict:
