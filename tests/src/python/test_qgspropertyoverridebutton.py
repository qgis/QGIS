...
class TestQgsPropertyOverrideButton(QgisTestCase):
    ...
    def testProjectColor(self):
        ...
        self.assertEqual(
            [a.text() for a in color_action.menu().actions()][0], "No colors set"
        )

        # add some project colors
        scheme.setColors(
            [[QColor(255, 0, 0), "color 1"], [QColor(255, 255, 0), "burnt marigold"]]
        )
        ...
        color_action = [a for a in button.menu().actions() if a.text() == "Color"][0]
        self.assertEqual(
            [a.text() for a in color_action.menu().actions()],
        ...
        self.assertEqual(
            button.toProperty().asExpression(), "project_color_object('burnt marigold')"
        )

        button.menuActionTriggered(color_action.menu().actions()[0])
        self.assertTrue(button.toProperty().isActive())
        self.assertEqual(
            button.toProperty().asExpression(), "project_color_object('color 1')"
        )

        button.setToProperty(
            QgsProperty.fromExpression("project_color_object('burnt marigold')")
        )
        button.aboutToShowMenu()
        color_action = [a for a in button.menu().actions() if a.text() == "Color"][0]
        self.assertTrue(color_action.isChecked())
        self.assertEqual(
            [a.isChecked() for a in color_action.menu().actions()], [False, True]
        )
        ...
        self.assertTrue(color_action.isChecked())
        self.assertEqual(
            [a.isChecked() for a in color_action.menu().actions()], [True, False]
        )

        # should also see color menu for ColorNoAlpha properties
        definition = QgsPropertyDefinition(
            "test", "test", QgsPropertyDefinition.StandardPropertyTemplate.ColorNoAlpha
        )
        button = QgsPropertyOverrideButton()
        button.init(0, QgsProperty(), definition)
        ...
        definition = QgsPropertyDefinition(
            "test", "test", QgsPropertyDefinition.StandardPropertyTemplate.Double
        )
        button = QgsPropertyOverrideButton()
        button.init(0, QgsProperty(), definition)
    ...
    def testLinkedColorButton(self):
        ...
        self.assertFalse(cb.linkedProjectColorName())
        button.setToProperty(
            QgsProperty.fromExpression("project_color('Cthulhu's delight')")
        )
        self.assertTrue(cb.isEnabled())
        self.assertFalse(cb.linkedProjectColorName())
