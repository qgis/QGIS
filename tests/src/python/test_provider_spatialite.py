...
class TestQgsSpatialiteProvider(QgisTestCase, ProviderTestCase):
    ...
    def testStyleStorage(self):
        ...
        res, err = QgsProviderRegistry.instance().styleExists(
            "spatialite", vl.source(), "name_test2"
        )
        self.assertTrue(res)
        self.assertFalse(err)
        ...
        self.assertCountEqual(
            desclist,
            ["description_bis", "description2", "description3", "description_test2"],
        )
    ...
    def _aliased_sql_helper(self, dbname):
        ...
        for sql in queries:
            vl = QgsVectorLayer(
                f"dbname='{dbname}' table=\"{sql}\" (geom) sql=",
                "test",
                "spatialite",
            )
            self.assertTrue(vl.isValid(), f"dbname: {dbname} - sql: {sql}")
    ...
    def testPkLessQuery(self):
        ...
        def _make_table(table_name):
            ...
            sql = (
                f"SELECT AddGeometryColumn('{table_name}', 'geom', 4326, 'POINT', 'XY')"
            )
            cur.execute(sql)
        ...
        vl_pk = QgsVectorLayer(
            f'dbname=\'{dbname}\' table="(select * from \\"test pk\\")" (geometry) sql=',
            "pk",
            "spatialite",
        )
        ...
        vl_no_pk = QgsVectorLayer(
            f"dbname='{dbname}' table=\"(select * from somedata)\" (geom) sql=",
            "pk",
            "spatialite",
        )
        ...
        vl_no_pk = QgsVectorLayer(
            f'dbname=\'{dbname}\' table="(select * from \\"some data\\")" (geom) sql=',
            "pk",
            "spatialite",
        )
        self.assertTrue(vl_no_pk.isValid())
    ...
    def testGeometryTypes(self):
        ...
        tests = [
            ("Point", "Point (0 0)", QgsWkbTypes.Type.Point),
            ...
            (
                "MultiLineStringZ",
                "MultiLineStringZ ((0 0 10, 0 1 10))",
            ...
            (
                "PolygonZ",
                "PolygonZ ((0 0 10, 0 1 10, 1 1 10, 1 0 10, 0 0 10))",
            ...
            (
                "MultiPolygonZ",
                "MultiPolygonZ (((0 0 10, 0 1 10, 1 1 10, 1 0 10, 0 0 10)))",
        ...
        for typeStr, wkt, qgisType in tests:
            ml = QgsVectorLayer(
                (typeStr + "?crs=epsg:4326&field=id:int"), typeStr, "memory"
            )
    ...
    def testBigint(self):
        ...
        l = QgsVectorLayer(
            f"dbname={self.dbname} table='test_bigint' (position) key='id'",
            "test_bigint",
            "spatialite",
        )
        self.assertTrue(l.isValid())
    ...
    def testSpatialiteDefaultValues(self):
        ...
        vl = QgsVectorLayer(
            f"dbname='{dbname}' table='test_table_default_values'",
    ...
    def testSpatialiteAspatialMultipleAdd(self):
        ...
        dbname = os.path.join(
            tempfile.gettempdir(), "test_aspatial_multiple_edits.sqlite"
        )
        if os.path.exists(dbname):
            os.remove(dbname)
        con = spatialite_connect(dbname, isolation_level=None)
        ...
        vl = QgsVectorLayer(
            f"dbname='{dbname}' table='test_aspatial_multiple_edits'",
            "test_aspatial_multiple_edits",
            "spatialite",
        )
        self.assertTrue(vl.isValid())
    ...
    def testAddFeatureNoFields(self):
        ...
        vl = QgsVectorLayer(
            "dbname=%s table='test_nofields' (geometry)" % self.dbname,
            "test_nofields",
            "spatialite",
        )
        self.assertTrue(vl.isValid())
    ...
    def testBLOBType(self):
        """Test binary field"""
        vl = QgsVectorLayer(
            f'dbname={self.dbname} table="blob_table" sql=',
            "testBLOBType",
            "spatialite",
        )
        self.assertTrue(vl.isValid())
        ...
        values = {feat["id"]: feat["fld1"] for feat in vl.getFeatures()}
        expected = {1: QByteArray(b"bbbvx"), 2: QByteArray(), 3: QByteArray(b"SQLite")}
        self.assertEqual(values, expected)
    ...
    def testTransactions(self):
        ...
        vl = QgsVectorLayer(
            "dbname=%s table=test_transactions1 ()" % self.dbname,
            "test_transactions1",
            "spatialite",
        )
        self.assertTrue(vl.isValid())
        vl2 = QgsVectorLayer(
            "dbname=%s table=test_transactions2 ()" % self.dbname,
            "test_transactions2",
            "spatialite",
        )
        self.assertTrue(vl.isValid())
    ...
    def testGetQueryGeometryDetails(self):
        ...
        query = (
            "SELECT id, srid, x, y, MakePoint(x,y,srid) as geom FROM test_querygeometry"
        )
        vl = QgsVectorLayer(
            f"dbname={self.dbname} table='({query})' (geom) key='id'",
            "QueryGeometryXY",
            "spatialite",
        )
        self.assertTrue(vl.isValid())
    ...
    def testViewsExtentFilter(self):
        ...
        vl = QgsVectorLayer(
            "dbname='%s' table=\"vs_controle_ok_nok\" (geom)"
            % os.path.join(TEST_DATA_DIR, "views_test.sqlite"),
            "vs_controle_ok_nok",
            "spatialite",
        )
        self.assertTrue(vl.isValid())
    ...
    def testRegression50523(self):
        ...
        layer = QgsVectorLayer(
            f"dbname='{self.dbname}' table=\"table50523\" (position) sql=",
            "test",
            "spatialite",
        )
    ...
    def test_absolute_relative_uri(self):
        ...
        absolute_uri = "dbname='{}' table=\"somedata\" (geom)".format(
    ...
    def testRegression54622Multisurface(self):
        ...
        def _check_feature():
            layer = QgsVectorLayer(
                f"dbname='{self.dbname}' table=\"table54622\" (geometry) sql=",
                "test",
                "spatialite",
            )
            ...
            self.assertTrue(
                compareWkt(
                    ...
                    "MultiPolygon (((-0.886 0.135, -0.886 -0.038, -0.448 -0.070, -0.426 0.143, -0.886 0.135)))",
                    0.01,
                )
            )
        ...
        geom = QgsGeometry.fromWkt(
            "MULTISURFACE(CURVEPOLYGON(COMPOUNDCURVE((-0.886 0.135,-0.886 -0.038,-0.448 -0.070,-0.427 0.144,-0.886 0.135))))"
        )
    ...
    def test_extent(self):
        ...
        self.assertEqual(
            layer2D.extent3D(), QgsBox3D(0, 0, float("nan"), 1, 1, float("nan"))
        )
