...
class TestQgsExpressionBuilderWidget(QgisTestCase):
    ...
    def testStoredExpressions(self):
        ...
        w.expressionTree().saveToUserExpressions(
            "Stored Expression Number One", '"field_one" = 123', "An humble expression"
        )
    ...
    def testValuesList(self):
        ...
        f1 = QgsFeature(1)
        f1.setAttributes(
            [["one 'item'", "B"], "another 'item'", 0, [1, 2], [1.1, 2.1], True]
        )
        f2 = QgsFeature(2)
        ...
        datas = sorted(
            [
                (
                    valuesModel.data(
                        valuesModel.index(i, 0), Qt.ItemDataRole.DisplayRole
                    ),
                    valuesModel.data(
                        valuesModel.index(i, 0), Qt.ItemDataRole.UserRole + 1
                    ),
                )
                for i in range(4)
            ]
        ...
        self.assertEqual(
            datas, [("0", "0"), ("1", "1"), ("2", "2"), ("NULL [NULL]", "NULL")]
        )
        ...
        datas = [
            (
                valuesModel.data(valuesModel.index(i, 0), Qt.ItemDataRole.DisplayRole),
                valuesModel.data(valuesModel.index(i, 0), Qt.ItemDataRole.UserRole + 1),
            )
            for i in range(4)
        ]
