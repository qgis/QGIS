...
class TestPyQgsMssqlProvider(QgisTestCase, ProviderTestCase):
    ...
    def testStyle(self):
        ...
        self.assertFalse(err)
        res, err = QgsProviderRegistry.instance().styleExists(
            "mssql", vl.source(), "default style"
        )
        self.assertTrue(res)
        self.assertFalse(err)
        res, err = QgsProviderRegistry.instance().styleExists(
            "mssql", vl.source(), "related style"
        )
        self.assertTrue(res)
        self.assertFalse(err)
        res, err = QgsProviderRegistry.instance().styleExists(
    ...
    def testOverwriteExisting(self):
        ...
        uri = f'{self.dbconn} table="qgis_test"."sacrificialLamb" sql='
        new_layer = QgsVectorLayer(uri, "new", "mssql")
        self.assertTrue(new_layer.isValid())
        ...
        # try to overwrite
        error, message = QgsVectorLayerExporter.exportLayer(
            layer, uri, "mssql", QgsCoordinateReferenceSystem()
        )
    ...
    def testMultiGeomColumns(self):
        uri = f'{self.dbconn} table="qgis_test"."multiGeomColumns" (geom1) sql='
        new_layer = QgsVectorLayer(uri, "new", "mssql")
        self.assertTrue(new_layer.isValid())
        ...
        uri = f'{self.dbconn} table="qgis_test"."multiGeomColumns" (geom2) sql='
        ...
        self.assertEqual(
            geom,
            {
                1: "LineString (2 3, 4 5)",
                2: "LineString (3 4, 5 6)",
                3: "LineString (1 2, 3 4)",
                4: "LineString (5 6, 7 8)",
                5: "",
            },
        )
    ...
    def testPktComposite(self):
        ...
        """
        vl = QgsVectorLayer(
            f'{self.dbconn} type=POINT estimatedmetadata=true key=\'"pk1","pk2"\' table="qgis_test"."tb_test_compound_pk" (geom)',
            "test_compound",
            "mssql",
        )
        self.assertTrue(vl.isValid())
        ...
        vl2 = QgsVectorLayer(
            f'{self.dbconn} type=POINT estimatedmetadata=true table="qgis_test"."tb_test_compound_pk" (geom) key=\'"pk1","pk2"\' ',
            "test_compound2",
            "mssql",
        )
    ...
    def testPktCompositeFloat(self):
        ...
        """
        vl = QgsVectorLayer(
            f'{self.dbconn} type=POINT key=\'"pk1","pk2","pk3"\' table="qgis_test"."tb_test_composite_float_pk" (geom)',
    ...
    def testIdentityFieldHandling(self):
        ...
        conn.execSql(
            """CREATE TABLE [qgis_test].[test_identity](
...
(
        ...
        uri = f'{self.dbconn} table="qgis_test"."test_identity" sql='
        ...
        self.assertEqual(
            identity_field.constraints().constraints(),
            QgsFieldConstraints.Constraint.ConstraintNotNull,
        )
        self.assertEqual(
            identity_field.constraints().constraintOrigin(
                QgsFieldConstraints.Constraint.ConstraintNotNull
            ),
            QgsFieldConstraints.ConstraintOrigin.ConstraintOriginProvider,
        )
        self.assertEqual(
            identity_field.constraints().constraintStrength(
                QgsFieldConstraints.Constraint.ConstraintNotNull
            ),
