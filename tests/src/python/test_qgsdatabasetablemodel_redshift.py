# -*- coding: utf-8 -*-
"""QGIS Unit tests for Redshift QgsDatabaseTableModel
...
class TestPyQgsDatabaseTableModel(unittest.TestCase):
    ...
    def testModel(self):
        conn = QgsProviderRegistry.instance().providerMetadata(self.providerKey).createConnection(self.uri, {})
        ...
        self.assertIn('qgis_test.some_data', tables)
        self.assertIn('qgis_test.some_poly_data', tables)
        self.assertIn('information_schema.column_privileges', tables)
        self.assertEqual(model.data(model.index(tables.index('qgis_test.some_data'), 0, QModelIndex()),
                                    QgsDatabaseTableModel.RoleTableName), 'some_data')
        self.assertEqual(model.data(model.index(tables.index('qgis_test.some_data'), 0, QModelIndex()),
        ...
        conn.createVectorTable('qgis_test', 'my_new_table', fields, QgsWkbTypes.Point, QgsCoordinateReferenceSystem('EPSG:3857'), True, {})
        self.assertEqual(model.rowCount(), old_count)
        model.refresh()
        ...
        self.assertIn('qgis_test.some_poly_data', tables)
        self.assertIn('information_schema.column_privileges', tables)
        self.assertIn('qgis_test.my_new_table', tables)

        conn.createVectorTable('qgis_test', 'my_new_table2', fields, QgsWkbTypes.Point, QgsCoordinateReferenceSystem('EPSG:3857'), False, {})
        conn.createVectorTable('qgis_test', 'my_new_table3', fields, QgsWkbTypes.Point, QgsCoordinateReferenceSystem('EPSG:3857'), False, {})
        model.refresh()
        ...
        self.assertIn('information_schema.column_privileges', tables)
        self.assertIn('qgis_test.my_new_table', tables)
        self.assertIn('qgis_test.my_new_table2', tables)
        self.assertIn('qgis_test.my_new_table3', tables)

        conn.createVectorTable('qgis_test', 'my_new_table4', fields, QgsWkbTypes.Point, QgsCoordinateReferenceSystem('EPSG:3857'), False, {})
        conn.dropVectorTable('qgis_test', 'my_new_table2')
        conn.dropVectorTable('qgis_test', 'my_new_table')
        model.refresh()
        ...
        self.assertIn('qgis_test.some_poly_data', tables)
        self.assertIn('information_schema.column_privileges', tables)
        self.assertNotIn('qgis_test.my_new_table', tables)
        self.assertNotIn('qgis_test.my_new_table2', tables)
        self.assertIn('qgis_test.my_new_table3', tables)
        self.assertIn('qgis_test.my_new_table4', tables)

        conn.dropVectorTable('qgis_test', 'my_new_table3')
        conn.dropVectorTable('qgis_test', 'my_new_table4')
        model.refresh()
        self.assertEqual(model.rowCount(), old_count)
        tables = [model.data(model.index(r, 0, QModelIndex()), Qt.DisplayRole) for r in range(model.rowCount())]
        self.assertIn('qgis_test.some_data', tables)
        ...
        self.assertNotIn('qgis_test.my_new_table', tables)
        self.assertNotIn('qgis_test.my_new_table2', tables)
        self.assertNotIn('qgis_test.my_new_table3', tables)
        self.assertNotIn('qgis_test.my_new_table4', tables)
    ...
    def testModelSpecificSchema(self):
        ...
        self.assertEqual(model.columnCount(), 1)
        tables = [model.data(model.index(r, 0, QModelIndex()), Qt.DisplayRole) for r in range(model.rowCount())]
        self.assertIn('some_data', tables)
        self.assertIn('some_poly_data', tables)
        self.assertNotIn('column_privileges', tables)
        self.assertEqual(model.data(model.index(tables.index('some_data'), 0, QModelIndex()),
                                    QgsDatabaseTableModel.RoleTableName), 'some_data')
        self.assertEqual(model.data(model.index(tables.index('some_data'), 0, QModelIndex()),
                                    QgsDatabaseTableModel.RoleSchema), 'qgis_test')
        self.assertEqual(model.data(model.index(tables.index('some_data'), 0, QModelIndex()),
    ...
    def test_model_allow_empty(self):
        ...
        self.assertFalse(model.data(model.index(tables.index('qgis_test.some_data'), 0, QModelIndex()), QgsDatabaseTableModel.RoleEmpty))
