...
class TestQgsValueMapFieldFormatter(QgisTestCase):
    ...
    def test_representValue(self):
        ...
        self.assertEqual(
            fieldFormatter.representValue(layer, 1, config, None, 2.5), "twoandhalf"
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 2, config, None, "NULL"), "NULL text"
        )
        # Tests with null values of different types, if value map contains null
        self.assertEqual(
            fieldFormatter.representValue(layer, 3, config, None, None), "nothing"
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 4, config, None, None), "nothing"
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 5, config, None, None), "nothing"
        )
        ...
        self.assertEqual(
            fieldFormatter.representValue(layer, 1, config, None, 2.5), "(2.50000)"
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 2, config, None, "NULL"), "(NULL)"
        )
        # Tests with fallback display for null in different types of fields
        self.assertEqual(
            fieldFormatter.representValue(layer, 3, config, None, None), "(NULL)"
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 4, config, None, None), "(NULL)"
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 5, config, None, None), "(NULL)"
        )
...
class TestQgsValueRelationFieldFormatter(QgisTestCase):
    ...
    def test_representValue(self):
        ...
        second_layer = QgsVectorLayer(
            "none?field=pkid:integer&field=decoded:string", "second_layer", "memory"
        )
        ...
        config = {"Layer": second_layer.id(), "Key": "pkid", "Value": "decoded"}
        self.assertEqual(
            fieldFormatter.representValue(first_layer, 0, config, None, "456"), "(456)"
        )
    ...
    def test_valueToStringList(self):
        ...
        _test('["1","2","3"]', ["1", "2", "3"])
        _test(
            r'["a string,comma","a string\"quote", "another string[]"]',
    ...
    def test_expressionRequiresFormScope(self):
        res = list(
            QgsValueRelationFieldFormatter.expressionFormAttributes(
                "current_value('ONE') AND current_value('TWO')"
            )
        )
        res = sorted(res)
        ...
        res = list(
            QgsValueRelationFieldFormatter.expressionFormVariables("@current_geometry")
        )
        ...
        self.assertTrue(
            QgsValueRelationFieldFormatter.expressionIsUsable("", QgsFeature())
        )
        self.assertFalse(
            QgsValueRelationFieldFormatter.expressionIsUsable(
                "@current_geometry", QgsFeature()
            )
        )
        self.assertFalse(
            QgsValueRelationFieldFormatter.expressionIsUsable(
                "current_value ( 'TWO' )", QgsFeature()
            )
        )

        layer = QgsVectorLayer(
            "none?field=pkid:integer&field=decoded:string", "layer", "memory"
        )
        self.assertTrue(layer.isValid())
        QgsProject.instance().addMapLayer(layer)
        ...
        f.setGeometry(point)
        self.assertTrue(
            QgsValueRelationFieldFormatter.expressionIsUsable("current_geometry", f)
        )
        self.assertFalse(
            QgsValueRelationFieldFormatter.expressionIsUsable(
                "current_value ( 'TWO' )", f
        ...
        self.assertTrue(
            QgsValueRelationFieldFormatter.expressionIsUsable(
                "current_value ( 'pkid' )", f
            )
        )
        self.assertTrue(
            QgsValueRelationFieldFormatter.expressionIsUsable(
                "@current_geometry current_value ( 'pkid' )", f
            )
        )
...
class TestQgsRelationReferenceFieldFormatter(QgisTestCase):
    ...
    def test_representValue(self):
        ...
        second_layer.setDisplayExpression("decoded")
        self.assertEqual(
            fieldFormatter.representValue(first_layer, 0, config, None, "123"), "123"
        )
        ...
        second_layer.setDisplayExpression("decoded")
        self.assertEqual(
            fieldFormatter.representValue(second_layer, 0, config, None, "123"), "123"
        )
        ...
        second_layer.setDisplayExpression("decoded")
        self.assertEqual(
            fieldFormatter.representValue(first_layer, 1, config, None, "123"), "123"
        )
...
class TestQgsRangeFieldFormatter(QgisTestCase):
    ...
    def test_representValue(self):
        ...
        self.assertEqual(
            fieldFormatter.representValue(layer, 0, {"Precision": 1}, None, "123"),
            "123",
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 0, {"Precision": 1}, None, "123000"),
            "123,000",
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 0, {"Precision": 1}, None, "9999999"),
            "9,999,999",
        )  # no scientific notation for integers!
        self.assertEqual(
            fieldFormatter.representValue(layer, 0, {"Precision": 1}, None, None),
            "NULL",
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 2, {"Precision": 1}, None, "123"),
            "123",
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 2, {"Precision": 1}, None, "123000"),
            "123,000",
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 2, {"Precision": 1}, None, "9999999"),
            "9,999,999",
        )  # no scientific notation for long longs!
        self.assertEqual(
            fieldFormatter.representValue(layer, 2, {"Precision": 1}, None, None),
            "NULL",
        )

        self.assertEqual(
            fieldFormatter.representValue(layer, 1, {"Precision": 1}, None, None),
            "NULL",
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 1, {"Precision": 1}, None, "123"),
            "123.0",
        )

        self.assertEqual(
            fieldFormatter.representValue(layer, 1, {"Precision": 2}, None, None),
            "NULL",
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 1, {"Precision": 2}, None, "123000"),
            "123,000.00",
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 1, {"Precision": 2}, None, "0"), "0.00"
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 1, {"Precision": 2}, None, "123"),
            "123.00",
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 1, {"Precision": 2}, None, "0.123"),
            "0.12",
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 1, {"Precision": 2}, None, "0.127"),
            "0.13",
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 1, {"Precision": 3}, None, "0"),
            "0.000",
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 1, {"Precision": 3}, None, "0.127"),
            "0.127",
        )
        self.assertEqual(
            fieldFormatter.representValue(layer, 1, {"Precision": 3}, None, "1.27e-1"),
            "0.127",
        )
        ...
        self.assertEqual(
            fieldFormatter.representValue(layer, 2, {"Precision": 1}, None, "9999999"),
            "9.999.999",
        )  # scientific notation for long longs!
        self.assertEqual(
            fieldFormatter.representValue(layer, 2, {"Precision": 1}, None, None),
            "NULL",
        )
...
class TestQgsCheckBoxFieldFormatter(QgisTestCase):
    ...
    def test_representValue(self):
        ...
        layer = QgsVectorLayer(
            "point?field=int:integer&field=str:string&field=bool:bool",
            "layer",
            "memory",
        )
        self.assertTrue(layer.isValid())
        ...
        self.assertEqual(
            field_formatter.representValue(layer, 0, config, None, 1), "true"
        )
        self.assertEqual(
            field_formatter.representValue(layer, 0, config, None, 0), "false"
        )
        self.assertEqual(
            field_formatter.representValue(layer, 0, config, None, 10), "(10)"
        )

        # displaying stored values
        config["TextDisplayMethod"] = (
            QgsCheckBoxFieldFormatter.TextDisplayMethod.ShowStoredValues
        )
        ...
        self.assertEqual(
            field_formatter.representValue(layer, 0, config, None, 0), "true"
        )
        self.assertEqual(
            field_formatter.representValue(layer, 0, config, None, 1), "false"
        )
        ...
        self.assertEqual(
            field_formatter.representValue(layer, 1, config, None, "yeah"), "true"
        )
        self.assertEqual(
            field_formatter.representValue(layer, 1, config, None, "nooh"), "false"
        )
        self.assertEqual(
            field_formatter.representValue(layer, 1, config, None, "oops"), "(oops)"
        )
        ...
        self.assertEqual(
            field_formatter.representValue(layer, 0, config, None, "yeah"), "yeah"
        )
        self.assertEqual(
            field_formatter.representValue(layer, 0, config, None, "nooh"), "nooh"
        )
        self.assertEqual(
            field_formatter.representValue(layer, 0, config, None, "oops"), "(oops)"
        )

        # bool
        config["TextDisplayMethod"] = (
            QgsCheckBoxFieldFormatter.TextDisplayMethod.ShowTrueFalse
        )
        self.assertEqual(
            field_formatter.representValue(layer, 2, config, None, True), "true"
        )
        self.assertEqual(
            field_formatter.representValue(layer, 2, config, None, False), "false"
        )
        self.assertEqual(
            field_formatter.representValue(layer, 2, config, None, NULL), "NULL"
        )
...
class TestQgsFallbackFieldFormatter(QgisTestCase):
    ...
    def test_representValue(self):
        ...
        def _test(layer, is_gpkg=False):
            ...
            # Precision is ignored for integers and longlongs
            self.assertEqual(
                fieldFormatter.representValue(layer, 0 + offset, {}, None, "123"), "123"
            )
            self.assertEqual(
                fieldFormatter.representValue(layer, 0 + offset, {}, None, "123000"),
                "123,000",
            )
            self.assertEqual(
                fieldFormatter.representValue(layer, 0 + offset, {}, None, "9999999"),
                "9,999,999",
            )
            self.assertEqual(
                fieldFormatter.representValue(layer, 0 + offset, {}, None, None), "NULL"
            )
            self.assertEqual(
                fieldFormatter.representValue(layer, 2 + offset, {}, None, "123"), "123"
            )
            self.assertEqual(
                fieldFormatter.representValue(layer, 2 + offset, {}, None, "123000"),
                "123,000",
            )
            self.assertEqual(
                fieldFormatter.representValue(layer, 2 + offset, {}, None, "9999999"),
                "9,999,999",
            )
            self.assertEqual(
                fieldFormatter.representValue(layer, 2 + offset, {}, None, None), "NULL"
            )

            self.assertEqual(
                fieldFormatter.representValue(layer, 1 + offset, {}, None, None), "NULL"
            )

            if not is_gpkg:
                self.assertEqual(
                    fieldFormatter.representValue(layer, 1 + offset, {}, None, "123"),
                    "123.00000",
                )
            else:
                self.assertEqual(
            ...
            self.assertEqual(
                fieldFormatter.representValue(layer, 1 + offset, {}, None, "0"), "0"
            )
            self.assertEqual(
                fieldFormatter.representValue(layer, 1 + offset, {}, None, "0.127"),
                "0.127",
            )
            self.assertEqual(
                fieldFormatter.representValue(layer, 1 + offset, {}, None, "1.27e-1"),
                "0.127",
            )
            ...
            # Check string
            self.assertEqual(
                fieldFormatter.representValue(layer, 3 + offset, {}, None, "123"), "123"
            )
            self.assertEqual(
                fieldFormatter.representValue(layer, 3 + offset, {}, None, "a string"),
                "a string",
            )
            self.assertEqual(
            ...
            self.assertEqual(
                fieldFormatter.representValue(layer, 3 + offset, {}, None, None), "NULL"
            )

            # Check NULLs (this is what happens in real life inside QGIS)
            self.assertEqual(
                fieldFormatter.representValue(layer, 0 + offset, {}, None, NULL), "NULL"
            )
            self.assertEqual(
                fieldFormatter.representValue(layer, 1 + offset, {}, None, NULL), "NULL"
            )
            self.assertEqual(
                fieldFormatter.representValue(layer, 2 + offset, {}, None, NULL), "NULL"
            )
            self.assertEqual(
                fieldFormatter.representValue(layer, 3 + offset, {}, None, NULL), "NULL"
            )
    ...
    def test_representValueWithDefault(self):
        ...
        self.assertEqual(fieldFormatter.representValue(vl, 1, {}, None, NULL), "NULL")
        self.assertEqual(fieldFormatter.representValue(vl, 1, {}, None, 4), "4")
        self.assertEqual(fieldFormatter.representValue(vl, 1, {}, None, "123"), "123")
        # bad field index
...
class TestQgsDateTimeFieldFormatter(QgisTestCase):
    ...
    def test_representValue(self):
        ...
        for locale, assertions in locale_assertions.items():
            ...
            self.assertEqual(
                field_formatter.defaultDisplayFormat(QVariant.Date),
                assertions["date_format"],
                locale.name(),
            )
            self.assertEqual(
                field_formatter.defaultDisplayFormat(QVariant.Time),
                assertions["time_format"],
                locale.name(),
            )
            self.assertEqual(
                field_formatter.defaultDisplayFormat(QVariant.DateTime),
                assertions["datetime_format"],
                locale.name(),
            )
            ...
            self.assertEqual(
                ...
                assertions["datetime_utc+1"],
                locale.name(),
            )
            self.assertEqual(
                field_formatter.representValue(
                    layer, 1, config, None, QDate(2020, 3, 4)
                ),
                assertions["datetime_utc"].split(" ")[0],
                locale.name(),
            )
            config = {"display_format": "HH:mm:s"}
            self.assertEqual(
                field_formatter.representValue(
                    layer, 2, config, None, QTime(12, 13, 14)
                ),
                assertions["datetime_utc"].split(" ")[1],
                locale.name(),
            )
