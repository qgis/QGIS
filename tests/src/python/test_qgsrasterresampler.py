...
class TestQgsRasterResampler(QgisTestCase):
    ...
    def checkRawBlockContents(self, block, expected):
        ...
        for r in range(block.height()):
            res.append(
                [
                    (0 if block.isNoData(r, c) else block.value(r, c))
    ...
    def testGDALResampling_downsampling_bilinear_beyond_max_oversampling_factor_containing_raster_extent(
        self,
    ):
    ...
    def testGDALResampling_nominal_resolution_slightly_overlapping_left_edge(self):
        ...
        extent = QgsRectangle(
            xmin - 0.2 * xres, ymax - 4.25 * yres, xmin + 1.8 * xres, ymax - 2.25 * yres
        )
    ...
    def testGDALResampling_nominal_resolution_slightly_overlapping_top_edge(self):
        ...
        extent = QgsRectangle(
            xmin + 2.25 * xres, ymax - 1.8 * yres, xmin + 4.25 * xres, ymax + 0.2 * yres
        )
    ...
    def testGDALResampling_less_than_one_pixel(self):
        ...
        extent = QgsRectangle(
            xmin + 2.25 * xres, ymax - 4.25 * yres, xmin + 2.5 * xres, ymax - 4.5 * yres
        )
