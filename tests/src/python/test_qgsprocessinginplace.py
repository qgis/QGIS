...
class TestQgsProcessingInPlace(QgisTestCase):
    ...
    def test_support_in_place_edit(self):
        ...
        POLYGON_ONLY_NOT_M_NOT_Z = {
            t: t in ("Polygon", "MultiPolygon") for t in _all_true().keys()
        }
        MULTI_ONLY = {t: t.find("Multi") == 0 for t in _all_true().keys()}
        SINGLE_ONLY = {t: t.find("Multi") == -1 for t in _all_true().keys()}
        LINESTRING_AND_POLYGON_ONLY = {
            t: (t.find("LineString") >= 0 or t.find("Polygon") >= 0)
            for t in _all_true().keys()
        }
        LINESTRING_AND_POLYGON_ONLY_NOT_M = {
            t: (
                t.rfind("M") < 1
                and (t.find("LineString") >= 0 or t.find("Polygon") >= 0)
            )
            for t in _all_true().keys()
        }
        ...
        self._support_inplace_edit_tester(
            "native:arraytranslatedfeatures", GEOMETRY_ONLY
        )
        self._support_inplace_edit_tester("native:reprojectlayer", GEOMETRY_ONLY)
        self._support_inplace_edit_tester(
            "qgis:densifygeometries", LINESTRING_AND_POLYGON_ONLY
        )
        self._support_inplace_edit_tester(
            "qgis:densifygeometriesgivenaninterval", LINESTRING_AND_POLYGON_ONLY
        )
        ...
        self._support_inplace_edit_tester(
            "native:minimumenclosingcircle", POLYGON_ONLY_NOT_M_NOT_Z
        )
        self._support_inplace_edit_tester(
            "native:multiringconstantbuffer", POLYGON_ONLY_NOT_M_NOT_Z
        )
        self._support_inplace_edit_tester(
            "native:orientedminimumboundingbox", POLYGON_ONLY_NOT_M_NOT_Z
        )
        self._support_inplace_edit_tester(
            "qgis:orthogonalize", LINESTRING_AND_POLYGON_ONLY
        )
        self._support_inplace_edit_tester(
            "native:removeduplicatevertices", GEOMETRY_ONLY
        )
        ...
        self._support_inplace_edit_tester(
            "native:simplifygeometries", LINESTRING_AND_POLYGON_ONLY
        )
        self._support_inplace_edit_tester("native:snappointstogrid", GEOMETRY_ONLY)
        self._support_inplace_edit_tester(
            "native:multiparttosingleparts", GEOMETRY_ONLY
        )
        ...
        self._support_inplace_edit_tester(
            "native:splitwithlines", LINESTRING_AND_POLYGON_ONLY
        )
    ...
    def _make_compatible_tester(self, feature_wkt, layer_wkb_name, attrs=[1]):
        ...
        self.assertTrue(
            ...
            f"Fail: {feature_wkt} - {attrs} - {layer_wkb_name}",
        )
    ...
    def test_QgsVectorLayerUtilsmakeFeaturesCompatible(self):
        ...
        self._make_compatible_tester(
            "Polygon((1 1, 2 2, 3 3, 1 1))", "Polygon", [1, "nope"]
        )
        self._make_compatible_tester(
            "Polygon z ((1 1 1, 2 2 2, 3 3 3, 1 1 1))", "Polygon"
        )
        self._make_compatible_tester(
            "Polygon z ((1 1 1, 2 2 2, 3 3 3, 1 1 1))", "PolygonZ"
        )

        # Adding Z back
        l, f = self._make_compatible_tester(
            "Polygon ((1 1, 2 2, 3 3, 1 1))", "PolygonZ"
        )
        g = f[0].geometry()
        g2 = g.constGet()
        ...
        # Adding M back
        l, f = self._make_compatible_tester(
            "Polygon ((1 1, 2 2, 3 3, 1 1))", "PolygonM"
        )
        g = f[0].geometry()
        g2 = g.constGet()
        for v in g2.vertices():
        ...
        self._make_compatible_tester(
            "Polygon m ((1 1 1, 2 2 2, 3 3 3, 1 1 1))", "Polygon"
        )
        self._make_compatible_tester(
            "Polygon m ((1 1 1, 2 2 2, 3 3 3, 1 1 1))", "PolygonM"
        )
        ...
        self._make_compatible_tester(
            "LineString(1 1, 2 2, 3 3, 1 1)", "LineString", [1, "nope"]
        )
        self._make_compatible_tester(
            "LineString z (1 1 1, 2 2 2, 3 3 3, 1 1 1)", "LineString"
        )
        self._make_compatible_tester(
            "LineString z (1 1 1, 2 2 2, 3 3 3, 1 1 1)", "LineStringZ"
        )
        self._make_compatible_tester(
            "LineString m (1 1 1, 2 2 2, 3 3 3, 1 1 1)", "LineString"
        )
        self._make_compatible_tester(
            "LineString m (1 1 1, 2 2 2, 3 3 3, 1 1 1)", "LineStringM"
        )

        # Adding Z back
        l, f = self._make_compatible_tester(
            "LineString (1 1, 2 2, 3 3, 1 1)", "LineStringZ"
        )
        g = f[0].geometry()
        g2 = g.constGet()
        for v in g2.vertices():
        ...
        # Adding M back
        l, f = self._make_compatible_tester(
            "LineString (1 1, 2 2, 3 3, 1 1)", "LineStringM"
        )
        g = f[0].geometry()
        g2 = g.constGet()
        for v in g2.vertices():
        ...
        self._make_compatible_tester(
            "LineString(1 1, 2 2, 3 3, 1 1)", "MultiLineString"
        )
        self._make_compatible_tester(
        ...
        l, f = self._make_compatible_tester(
            "MultiLineString ((1 1, 2 2),(4 4, 3 3))", "Point"
        )
        ...
        l, f = self._make_compatible_tester(
            "MultiLineString ((1 1, 2 2),(4 4, 3 3))", "MultiPoint"
        )
        self.assertEqual(len(f), 1)
        self.assertEqual(
            f[0].geometry().asWkt(), "MultiPoint ((1 1),(2 2),(4 4),(3 3))"
        )
        ...
        l, f = self._make_compatible_tester(
            "LineString (1 1, 1 2, 2 2)", "MultiPolygon"
        )
        self.assertEqual(len(f), 1)
        self.assertEqual(
            f[0].geometry().asWkt(), "MultiPolygon (((1 1, 1 2, 2 2, 1 1)))"
        )

        l, f = self._make_compatible_tester(
            "MultiLineString ((1 1, 1 2, 2 2, 1 1),(3 3, 4 3, 4 4))", "Polygon"
        )
        self.assertEqual(len(f), 2)
        self.assertEqual(f[0].geometry().asWkt(), "Polygon ((1 1, 1 2, 2 2, 1 1))")
        ...
        l, f = self._make_compatible_tester(
            "MultiLineString ((1 1, 1 2, 2 2, 1 1),(3 3, 4 3, 4 4))", "MultiPolygon"
        )
        self.assertEqual(len(f), 1)
        self.assertEqual(
            f[0].geometry().asWkt(),
        ...
        l, f = self._make_compatible_tester(
            "CircularString (1 1, 1 2, 2 2, 2 0, 1 1)", "CurvePolygon"
        )
        self.assertEqual(len(f), 1)
        self.assertEqual(
            f[0].geometry().asWkt(),
        ...
        l, f = self._make_compatible_tester(
            "CircularString (1 1, 1 2, 2 2, 2 0, 1 1)", "Polygon"
        )
        self.assertEqual(len(f), 1)
        self.assertTrue(
            f[0].geometry().asWkt(2).startswith("Polygon ((1 1, 0.99 1.01, 0.98 1.02")
        )
        ...
        l, f = self._make_compatible_tester(
            "Polygon ((1 1, 1 2, 2 2, 1 1))", "MultiPoint"
        )
        self.assertEqual(len(f), 1)
        self.assertEqual(f[0].geometry().asWkt(), "MultiPoint ((1 1),(1 2),(2 2))")

        l, f = self._make_compatible_tester(
            "MultiPolygon (((1 1, 1 2, 2 2, 1 1)),((3 3, 4 3, 4 4, 3 3)))", "Point"
        )
        ...
        l, f = self._make_compatible_tester(
            "MultiPolygon (((1 1, 1 2, 2 2, 1 1)),((3 3, 4 3, 4 4, 3 3)))", "MultiPoint"
        )
        self.assertEqual(len(f), 1)
        self.assertEqual(
            f[0].geometry().asWkt(), "MultiPoint ((1 1),(1 2),(2 2),(3 3),(4 3),(4 4))"
        )

        # polygon -> lines
        l, f = self._make_compatible_tester(
            "Polygon ((1 1, 1 2, 2 2, 1 1))", "LineString"
        )
        self.assertEqual(len(f), 1)
        self.assertEqual(f[0].geometry().asWkt(), "LineString (1 1, 1 2, 2 2, 1 1)")

        l, f = self._make_compatible_tester(
            "Polygon ((1 1, 1 2, 2 2, 1 1))", "MultiLineString"
        )
        self.assertEqual(len(f), 1)
        self.assertEqual(
            f[0].geometry().asWkt(), "MultiLineString ((1 1, 1 2, 2 2, 1 1))"
        )

        l, f = self._make_compatible_tester(
            "MultiPolygon (((1 1, 1 2, 2 2, 1 1)),((3 3, 4 3, 4 4, 3 3)))", "LineString"
        )
        self.assertEqual(len(f), 2)
        self.assertEqual(f[0].geometry().asWkt(), "LineString (1 1, 1 2, 2 2, 1 1)")
    ...
    def _alg_tester(
        ...
        input_layer,
        parameters,
        invalid_geometry_policy=QgsFeatureRequest.InvalidGeometryCheck.GeometryNoCheck,
        ...
        if not retain_selection:
            ...
            # Check selected
            self.assertEqual(
                input_layer.selectedFeatureIds(), [old_features[0].id()], alg_name
            )
        ...
        self.assertEqual(
            new_features[0].geometry().wkbType(), old_features[0].geometry().wkbType()
        )
    ...
    def test_execute_in_place_run(self):
        ...
        self.assertAlmostEqual(
            new_features[0].geometry().asPoint().x(),
            old_features[0].geometry().asPoint().x() + 1.1,
            delta=0.01,
        )
        self.assertAlmostEqual(
            new_features[0].geometry().asPoint().y(),
            old_features[0].geometry().asPoint().y() + 1.1,
            delta=0.01,
        )
        ...
        self.assertEqual(
            new_features[1].geometry().asPoint().x(),
            old_features[1].geometry().asPoint().x(),
        )
        self.assertEqual(
            new_features[1].geometry().asPoint().y(),
            old_features[1].geometry().asPoint().y(),
        )
    ...
    def test_multi_to_single(self):
        ...
        old_features, new_features = self._alg_tester(
            "native:multiparttosingleparts", self.multipoly_vl, {}
        )
    ...
    def test_snappointstogrid(self):
        ...
        f1.setGeometry(
            QgsGeometry.fromWkt(
                "POLYGON((1.2 1.2, 1.2 2.2, 2.2 2.2, 2.2 1.2, 1.2 1.2))"
            )
        )
    ...
    def test_fix_geometries(self):
        ...
        f3.setAttributes([1])
        f3.setGeometry(
            QgsGeometry.fromWkt("POLYGON Z((0 0 1, 2 2 1, 0 2 3, 2 0 4, 0 0 1))")
        )
        ...
        old_features, new_features = self._alg_tester(
            "native:fixgeometries", polygonz_layer, {}
        )
    ...
    def test_unique_constraints(self):
        ...
        gpkg_layer = QgsVectorLayer(
            gpkg_path + "|layername=Multi_to_Singleparts_FID_bug", "lyr", "ogr"
        )
    ...
    def test_regenerate_fid(self):
        ...
        res = QgsVectorLayerUtils.makeFeatureCompatible(
            f, gpkg_layer, QgsFeatureSink.SinkFlag.RegeneratePrimaryKey
        )
    ...
    def test_datadefinedvalue(self):
        ...
        self.assertEqual(
            geometries[0].asWkt(2),
            "Polygon ((1.2 1.2, 1.2 1.7, 1.2 2.2, 1.7 2.2, 2.2 2.2, 2.2 1.7, 2.2 1.2, 1.7 1.2, 1.2 1.2))",
        )
        self.assertEqual(
            geometries[1].asWkt(2),
            "Polygon ((1.1 1.1, 1.1 1.43, 1.1 1.77, 1.1 2.1, 1.43 2.1, 1.77 2.1, 2.1 2.1, 2.1 1.77, 2.1 1.43, 2.1 1.1, 1.77 1.1, 1.43 1.1, 1.1 1.1))",
        )
        # Check selected
