...
class TestQgsAuthManager(QgisTestCase):
    ...
    def checkCA(self):
        ...
        cert_ids = {
            QgsAuthCertUtils.shaHexForCert(c)
            for c in QgsAuthCertUtils.certsFromFile(
                PKIDATA + "/chain_subissuer-issuer-root.pem"
            )
        }
        cfg = QSslConfiguration.defaultConfiguration()
        # Get list of CA certificates
        ids = {QgsAuthCertUtils.shaHexForCert(c) for c in cfg.caCertificates()}
        for c in cert_ids:
    ...
    def test_040_authorities(self):
        ...
        self.assertTrue(self.authm.storeAuthSetting("cafile", full_chain_path), msg)

        msg = "Authority file 'allow invalids' setting could not be stored"
        self.assertTrue(self.authm.storeAuthSetting("cafileallowinvalid", False), msg)
        ...
        self.assertTrue(self.authm.removeAuthSetting("cafileallowinvalid"), msg)
    ...
    def test_060_identities(self):
        ...
        # noinspection PyTypeChecker
        bundle1 = QgsPkiBundle.fromPemPaths(
            client_cert_path, client_key_path, client_key_pass
        )
        ...
        self.assertTrue(self.authm.storeCertIdentity(bundle_cert, bundle_key), msg)
        ...
        self.assertTrue(self.authm.storeAuthenticationConfig(bundle_config), msg)
        ...
        self.assertTrue(
            self.authm.loadAuthenticationConfig(bundle_configid, bundle_config2, True),
            msg,
        )
        ...
        self.assertTrue(self.authm.removeAuthenticationConfig(bundle_configid), msg)
    ...
    def test_070_servers(self):
        ...
        self.assertTrue(self.authm.existsSslCertCustomConfig(cert_sha, hostport), msg)
        ...
        self.assertEqual(config.sslPeerVerifyMode(), config2.sslPeerVerifyMode(), msg)

        msg = "PeerVerifyDepth of retrieved SSL config does not match"
        self.assertEqual(config.sslPeerVerifyDepth(), config2.sslPeerVerifyDepth(), msg)
        ...
        self.assertTrue(self.authm.removeSslCertCustomConfig(cert_sha, hostport), msg)

        msg = "Could not verify removal of SSL config"
        self.assertFalse(self.authm.existsSslCertCustomConfig(cert_sha, hostport), msg)
    ...
    def config_obj(self, kind, base=True):
        ...
        if kind == "Basic":
            ...
            config.setConfig(
                "keypath", os.path.join(PKIDATA, "gerardus_key_w-pass.pem")
            )
    ...
    def config_values_valid(self, kind, config):
        ...
        if (
            config.name() != kind
            or config.method() != kind
            or config.uri() != "http://example.com"
        ):
            return False
        if kind == "Basic":
            return (
                config.config("username") == "username"
                and config.config("password") == "password"
                and config.config("realm") == "Realm"
            )
        elif kind == "PKI-Paths":
            return (
                config.config("certpath") == os.path.join(PKIDATA, "gerardus_cert.pem")
                and config.config("keypath")
                == os.path.join(PKIDATA, "gerardus_key_w-pass.pem")
                and config.config("keypass") == "password"
            )
        elif kind == "PKI-PKCS#12":
            return (
                config.config("bundlepath") == os.path.join(PKIDATA, "gerardus.p12")
                and config.config("bundlepass") == "password"
            )
    ...
    def test_090_auth_configs(self):
        ...
        for kind in self.config_list():
            ...
            self.assertTrue(self.authm.configAuthMethodKey(configid) == kind, msg)
            ...
            self.assertTrue(
                self.authm.loadAuthenticationConfig(configid, config2, True), msg
            )
            ...
            self.assertTrue(
                self.authm.loadAuthenticationConfig(configid, config3, True), msg
            )
            ...
            self.assertTrue(self.authm.removeAuthenticationConfig(configid), msg)
    ...
    def test_100_auth_db(self):
        ...
        self.assertTrue((len(self.authm.configIds()) == len(self.config_list())), msg)
        ...
        backup = None
        reserase, backup = self.authm.eraseAuthenticationDatabase(True, backup)
        ...
        self.assertTrue(
            not self.authm.masterPasswordIsSet()
            and not self.authm.masterPasswordHashInDatabase(),
            msg,
        )
    ...
    def test_150_verify_keychain(self):
        ...
        def testChain(path):
            ...
            self.assertGreater(
                len(
                    QgsAuthCertUtils.validateCertChain(
                        QgsAuthCertUtils.certsFromFile(path)
                    )
                ),
                0,
            )
            ...
            self.assertEqual(
                len(
                    QgsAuthCertUtils.validateCertChain(
                        QgsAuthCertUtils.certsFromFile(path), None, True
                    )
                ),
                0,
            )
            ...
            self.assertGreater(
                len(
                    QgsAuthCertUtils.validateCertChain(
                        QgsAuthCertUtils.certsFromFile(path), "my.wrong.domain", True
                    )
                ),
                0,
            )
        ...
        self.assertEqual(
            len(
                QgsAuthCertUtils.validateCertChain(
                    QgsAuthCertUtils.certsFromFile(path), "localhost", True
                )
            ),
            0,
        )
        ...
        self.assertGreater(
            len(
                QgsAuthCertUtils.validateCertChain(
                    QgsAuthCertUtils.certsFromFile(path), "localhost", False
                )
            ),
            0,
        )
    ...
    def test_validate_pki_bundle(self):
        ...
        self.assertEqual(
            QgsAuthCertUtils.validatePKIBundle(bundle),
            [
        ...
        self.assertEqual(
            QgsAuthCertUtils.validatePKIBundle(bundle, False),
            [
        ...
        # Wrong chain
        bundle = self.mkPEMBundle(
            "fra_cert.pem", "fra_key.pem", "password", "chain_issuer2-root2.pem"
        )
        ...
        self.assertTrue(
            QgsApplication.authManager().storeCertTrustPolicy(
                root2, QgsAuthCertUtils.CertTrustPolicy.Untrusted
            )
        )
    ...
    def test_170_pki_key_encoding(self):
        ...
        self.assertFalse(
            QgsAuthCertUtils.keyFromFile(PKIDATA + "/" + "ptolemy_key.der").isNull()
        )
        self.assertFalse(
            QgsAuthCertUtils.keyFromFile(PKIDATA + "/" + "ptolemy_key_pem.key").isNull()
        )
        self.assertFalse(
            QgsAuthCertUtils.keyFromFile(PKIDATA + "/" + "ptolemy_key_der.key").isNull()
        )
        self.assertFalse(
            QgsAuthCertUtils.keyFromFile(PKIDATA + "/" + "donald_key_EC.pem").isNull()
        )
        self.assertFalse(
            QgsAuthCertUtils.keyFromFile(PKIDATA + "/" + "donald_key_EC.der").isNull()
        )
        self.assertFalse(
            QgsAuthCertUtils.keyFromFile(PKIDATA + "/" + "donald_key_DSA.pem").isNull()
        )
        self.assertFalse(
            QgsAuthCertUtils.keyFromFile(PKIDATA + "/" + "donald_key_DSA.der").isNull()
        )
        self.assertFalse(
            QgsAuthCertUtils.keyFromFile(
                PKIDATA + "/" + "donald_key_DSA_crlf.pem"
            ).isNull()
        )
        self.assertFalse(
            QgsAuthCertUtils.keyFromFile(
                PKIDATA + "/" + "donald_key_DSA_nonl.pem"
            ).isNull()
        )
        donald_dsa = QgsAuthCertUtils.keyFromFile(
            PKIDATA + "/" + "donald_key_DSA.pem"
        ).toPem()
        self.assertEqual(
            donald_dsa,
            QgsAuthCertUtils.keyFromFile(PKIDATA + "/" + "donald_key_DSA.der").toPem(),
        )
        self.assertEqual(
            donald_dsa,
            QgsAuthCertUtils.keyFromFile(
