...
class TestPyQgsProviderConnectionBase():
    ...
    def _test_operations(self, md, conn):
        ...
        # Remove connection
        spy_about_to_be_deleted = QSignalSpy(md.connectionAboutToBeDeleted)
        spy_deleted = QSignalSpy(md.connectionDeleted)

        def before_delete(name):
            self.assertEqual(name, 'qgis_test1')
            self.assertEqual(list(md.connections().keys()), ['qgis_test1'])
            before_delete.called = True
            self.assertEqual(len(spy_deleted), 0)
        md.connectionAboutToBeDeleted.connect(before_delete)

        md.deleteConnection('qgis_test1')
        self.assertTrue(before_delete.called)
        self.assertEqual(list(md.connections().values()), [])
        self.assertEqual(len(spy_about_to_be_deleted), 1)
