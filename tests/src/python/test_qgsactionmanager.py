...
class TestQgsActionManager(QgisTestCase):
    ...
    def testAddAction(self):
        ...
        self.assertEqual(
            self.manager.actions()[0].type(), QgsAction.ActionType.GenericPython
        )
        ...
        self.assertEqual(
            self.manager.action(action2.id()).type(), QgsAction.ActionType.Windows
        )
        ...
        id3 = self.manager.addAction(
            QgsAction.ActionType.Generic, "Test Action3", "i=3"
        )
    ...
    def testRemoveActions(self):
        ...
        # add some actions
        id1 = self.manager.addAction(
            QgsAction.ActionType.GenericPython, "test_action", "i=1"
        )
        id2 = self.manager.addAction(
            QgsAction.ActionType.GenericPython, "test_action2", "i=2"
        )
        id3 = self.manager.addAction(
            QgsAction.ActionType.GenericPython, "test_action3", "i=3"
        )
    ...
    @unittest.skipIf(
        ...
        "Test is flaky on Travis environment",
    )
    def testDoAction(self):
        ...
        id1 = self.manager.addAction(
            QgsAction.ActionType.Unix,
            "test_action",
            self.create_action(temp_file, "test output"),
        )
        ...
        self.assertEqual(
            self.check_action_result(temp_file), "test 1 output test_layer"
        )
        ...
        id3 = self.manager.addAction(
            ...
            self.create_action(
                temp_file,
                "test : [% @field_index %] : [% @field_name %] : [% @field_value%]",
            ),
        )
        ...
        time.sleep(0.5)
        self.assertEqual(
            self.check_action_result(temp_file), "test : 1 : my_other_field : val"
        )
