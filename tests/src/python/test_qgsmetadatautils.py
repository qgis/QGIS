...
class TestPyQgsMetadataUtils(QgisTestCase):
    ...
    def testConvertEsri(self):
        ...
        self.assertEqual(
            metadata.keywords(),
            {"Search keys": ["road"], "gmd:topicCategory": ["TRANSPORTATION Land"]},
        )
        self.assertEqual(metadata.categories(), ["TRANSPORTATION Land"])
        self.assertEqual(
            metadata.extent().temporalExtents()[0].begin(), QDateTime(2016, 6, 28, 0, 0)
        )
        self.assertEqual(metadata.crs().authid(), "EPSG:4283")
        self.assertEqual(
            metadata.extent().spatialExtents()[0].bounds.xMinimum(), 137.921721
        )
        self.assertEqual(
            metadata.extent().spatialExtents()[0].bounds.xMaximum(), 153.551682
        )
        self.assertEqual(
            metadata.extent().spatialExtents()[0].bounds.yMinimum(), -29.177948
        )
        self.assertEqual(
            metadata.extent().spatialExtents()[0].bounds.yMaximum(), -9.373145
        )
        self.assertEqual(
            metadata.extent().spatialExtents()[0].extentCrs.authid(), "EPSG:4283"
        )

        self.assertEqual(
            metadata.dateTime(Qgis.MetadataDateType.Created),
            QDateTime(2022, 11, 1, 0, 0),
        )
        self.assertEqual(
            metadata.dateTime(Qgis.MetadataDateType.Published),
            QDateTime(2016, 6, 28, 0, 0),
        )
        self.assertEqual(
            metadata.dateTime(Qgis.MetadataDateType.Revised),
            QDateTime(2022, 11, 5, 0, 0),
        )
        self.assertEqual(
            metadata.dateTime(Qgis.MetadataDateType.Superseded),
            QDateTime(2022, 11, 12, 0, 0),
        )
        ...
        self.assertIn(
            "Dataset is wholly created and owned by Natural Resources and Mines for the State of Queensland",
        ...
        self.assertEqual(
            ...
            "http://qldspatial.information.qld.gov.au/catalog/custom/",
        )
        ...
        self.assertEqual(
            ...
            [
                ...
                "Data source: Local Government data",
    ...
    def testConvertEsriOld(self):
        ...
        self.assertEqual(
            metadata.keywords(), {"gmd:topicCategory": ["GEOSCIENCES Geology"]}
        )
        self.assertEqual(metadata.categories(), ["GEOSCIENCES Geology"])
        self.assertEqual(
            metadata.extent().temporalExtents()[0].begin(), QDateTime(2012, 7, 1, 0, 0)
        )
        self.assertEqual(
            metadata.extent().spatialExtents()[0].bounds.xMinimum(), 137.9947
        )
        self.assertEqual(
            metadata.extent().spatialExtents()[0].bounds.xMaximum(), 153.55183
        )
        self.assertEqual(
            metadata.extent().spatialExtents()[0].bounds.yMinimum(), -29.17849
        )
        self.assertEqual(
            metadata.extent().spatialExtents()[0].bounds.yMaximum(), -9.2296
        )

        self.assertEqual(
            metadata.rights(), ["Creative Commons Attribution 3.0 Australia (CC BY)"]
        )
        self.assertIn(
            "Unrestricted to all levels of government and community.",
            metadata.constraints()[0].constraint,
