...
class TestQgsAnnotationLayer(QgisTestCase):
    ...
    def testReplaceItem(self):
        ...
        layer.replaceItem(
            linestring_item_id,
            QgsAnnotationLineItem(
                QgsLineString([QgsPoint(21, 13), QgsPoint(22, 13), QgsPoint(22, 15)])
            ),
        )
        self.assertEqual(
            layer.item(polygon_item_id).geometry().asWkt(),
    ...
    def testItemsInBounds(self):
        ...
        item2uuid = layer.addItem(
            QgsAnnotationLineItem(
                QgsLineString([QgsPoint(11, 13), QgsPoint(12, 13), QgsPoint(12, 150)])
            )
        )
        ...
        self.assertCountEqual(
            layer.itemsInBounds(QgsRectangle(12, 130, 14, 150), rc), [item2uuid]
        )
        self.assertCountEqual(
            layer.itemsInBounds(QgsRectangle(110, 0, 120, 20), rc), [item3uuid]
        )
    ...
    def testClone(self):
        ...
        self.assertNotEqual(
            layer.items()[polygon_item_id], layer2.items()[polygon_item_id]
        )
        self.assertIsInstance(layer2.items()[linestring_item_id], QgsAnnotationLineItem)
        self.assertIsInstance(layer2.items()[marker_item_id], QgsAnnotationMarkerItem)
        self.assertEqual(layer2.linkedVisibilityLayer(), other_layer)
    ...
    def testProjectMainAnnotationLayer(self):
        ...
        self.assertIsInstance(
            p2.mainAnnotationLayer().items()[polygon_item_id], QgsAnnotationPolygonItem
        )
        self.assertIsInstance(
            p2.mainAnnotationLayer().items()[linestring_item_id], QgsAnnotationLineItem
        )
        self.assertIsInstance(
            p2.mainAnnotationLayer().items()[marker_item_id], QgsAnnotationMarkerItem
        )
    ...
    def test_apply_edit(self):
        ...
        self.assertEqual(
            layer.applyEdit(
                QgsAnnotationItemEditOperationMoveNode(
                    "xxx", QgsVertexId(0, 0, 2), QgsPoint(14, 15), QgsPoint(19, 15)
                )
            ),
    ...
    def testRenderLayer(self):
        ...
        item.setSymbol(
            QgsMarkerSymbol.createSimple(
                {"color": "100,200,200", "size": "6", "outline_color": "black"}
            )
        )
        item.setZIndex(1)
        ...
        self.assertEqual(
            [i.boundingBox().toString(1) for i in item_details if i.itemId() == i3_id][
                0
            ],
            "11.7,12.7 : 12.3,13.3",
        )
    ...
    def testRenderWithTransform(self):
        ...
        rc.setCoordinateTransform(
            QgsCoordinateTransform(
                layer.crs(), settings.destinationCrs(), QgsProject.instance()
            )
        )
        rc.setExtent(
            rc.coordinateTransform().transformBoundingBox(
                settings.extent(),
        ...
        self.assertTrue(
            self.image_check("layer_render_transform", "layer_render_transform", image)
        )
        ...
        self.assertEqual(
            [i.boundingBox().toString(2) for i in item_details if i.itemId() == i3_id][
                0
            ],
    ...
    def testRenderLayerWithReferenceScale(self):
        ...
        self.assertTrue(
            self.image_check(
                "layer_render_reference_scale", "layer_render_reference_scale", image
            )
        )
    ...
    def testRenderLayerWithLinkedVisibility(self):
        ...
        self.assertTrue(
            self.image_check(
                "layer_render_not_visible", "layer_render_not_visible", image
            )
        )
    ...
    def test_render_via_job(self):
        ...
        self.assertCountEqual(
            [
                i.itemId()
                for i in item_results.renderedAnnotationItemsInBounds(
    ...
    def testRenderWithDisabledItems(self):
        ...
        self.assertTrue(
            self.image_check("layer_render_disabled", "layer_render_disabled", image)
        )
        # also check details of rendered items
        item_details = renderer.takeRenderedItemDetails()
