...
class TestQgsLayoutItemElevationProfile(QgisTestCase, LayoutItemTestCase):
    ...
    def test_opacity(self):
        ...
        self.assertFalse(profile_item.requiresRasterization())
        self.assertTrue(profile_item.containsAdvancedEffects())

        self.assertTrue(self.render_layout_check("opacity", layout))
    ...
    def test_opacity_rendering_designer_preview(self):
        ...
        self.assertTrue(self.image_check("opacity", "opacity", im, allowed_mismatch=0))
    ...
    def test_blend_mode_designer_preview(self):
        ...
        self.assertTrue(
            self.image_check("blendmode", "blendmode", im, allowed_mismatch=0)
        )
    ...
    def test_settings(self):
        ...
        profile.setDistanceScale(551)
        profile.setElevationScale(552)
        profile.setRangeMethod(Qgis.ElevationProfileRangeMethod.FixedScale)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                profile2.rangeMethod(), Qgis.ElevationProfileRangeMethod.FixedScale
            )
            self.assertEqual(profile2.distanceScale(), 551)
            self.assertEqual(profile2.elevationScale(), 552)
    ...
    def test_subsections(self):
        """
        Test subsections setter/getter
        """
        project = QgsProject()
        layout = QgsPrintLayout(project)
        profile = QgsLayoutItemElevationProfile(layout)
        ...
        self.assertFalse(profile.subsectionsSymbol())

        line_symbol = QgsLineSymbol.createSimple({"color": "#0000ff", "width": 1.5})
        profile.setSubsectionsSymbol(line_symbol)
        subsections_symbol = profile.subsectionsSymbol()
        self.assertTrue(subsections_symbol)
        self.assertEqual(subsections_symbol, line_symbol)

        # test that subsections settings are written/restored
        with tempfile.TemporaryDirectory() as temp_dir:
            self.assertTrue(
                project.write(os.path.join(temp_dir, "project_subsections.qgs"))
            )

            project2 = QgsProject()
            self.assertTrue(
                project2.read(os.path.join(temp_dir, "project_subsections.qgs"))
            )

            layout2 = project2.layoutManager().printLayouts()[0]
            profile2 = [
                item
                for item in layout2.items()
                if isinstance(item, QgsLayoutItemElevationProfile)
            ][0]

            first_symbol = profile.subsectionsSymbol()
            second_symbol = profile2.subsectionsSymbol()
            self.assertEqual(second_symbol.color(), first_symbol.color())
            self.assertEqual(second_symbol.width(), first_symbol.width())
            self.assertEqual(second_symbol.opacity(), first_symbol.opacity())
    ...
    def test_draw_distance_units(self):
        ...
        profile_item.plot().xAxis().setLabelSuffixPlacement(
    ...
    def test_draw_fixed_scale(self):
        """
        Test rendering the layout profile item in fixed scale mode
        """
        vl = QgsVectorLayer("PolygonZ?crs=EPSG:27700", "lines", "memory")
        vl.setCrs(QgsCoordinateReferenceSystem())
        self.assertTrue(vl.isValid())
        ...
        profile_item.setRangeMethod(Qgis.ElevationProfileRangeMethod.FixedScale)
        profile_item.setDistanceScale(300)
        profile_item.setElevationScale(200)
        profile_item.plot().setYMinimum(-10)
        ...
        profile_item.setSources([vl])
        self.assertTrue(self.render_layout_check("fixed_scale", layout))
    ...
    def test_draw_subsections(self):
        """
        Test rendering the layout profile item with subsections
        """
        vl = QgsVectorLayer("PolygonZ?crs=EPSG:27700", "lines", "memory")
        vl.setCrs(QgsCoordinateReferenceSystem())
        self.assertTrue(vl.isValid())
        ...
        curve = QgsLineString()
        curve.fromWkt(
            "LineString (321897.2149876729818061 129916.80851080644060858, 321905.02818883035797626 129922.76423395283927675, 321911.9273928712354973 129924.85758218745468184, 321921.27375048212707043 129916.66109191668510903, 321927.22947362851118669 129917.1328323639172595, 321926.90515207103453577 129926.12538463941018563, 321934.89525589614640921 129927.95337887246569153)"
        )
