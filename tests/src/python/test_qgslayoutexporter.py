...
class TestQgsLayoutExporter(QgisTestCase):
    ...
    def testRenderPageToImage(self):
        ...
        rendered_file_path = os.path.join(
            self.basetestpath, "test_rendertoimagepage.png"
        )
        image.save(rendered_file_path, "PNG")
        self.assertTrue(
            self.image_check(
                "layoutexporter_rendertoimagepage",
                "layoutexporter_rendertoimagepage",
                image,
                color_tolerance=2,
                allowed_mismatch=20,
    ...
    def testRenderRegionToImage(self):
        ...
        self.assertTrue(
            self.image_check(
                "layoutexporter_rendertoimageregionsize",
                "layoutexporter_rendertoimageregionsize",
                image,
                color_tolerance=2,
                allowed_mismatch=20,
        ...
        self.assertTrue(
            self.image_check(
                "layoutexporter_rendertoimageregiondpi",
                "layoutexporter_rendertoimageregiondpi",
                image,
                color_tolerance=2,
                allowed_mismatch=20,
        ...
        self.assertTrue(
            self.image_check(
                "layoutexporter_rendertoimageregionoverridedpi",
                "layoutexporter_rendertoimageregionoverridedpi",
                image,
                color_tolerance=2,
                allowed_mismatch=20,
    ...
    def testExportToImage(self):
        ...
        rendered_file_path = os.path.join(
            self.basetestpath, "test_exporttoimagedpi.png"
        )
        self.assertEqual(
        ...
        self.assertTrue(
            self.image_check(
                "layoutexporter_exporttoimagedpi_page1",
                "layoutexporter_exporttoimagedpi_page1",
                image,
                color_tolerance=2,
                allowed_mismatch=20,
        ...
        self.assertTrue(
            self.image_check(
                "layoutexporter_exporttoimagedpi_page2",
                "layoutexporter_exporttoimagedpi_page2",
                image,
                color_tolerance=2,
                allowed_mismatch=20,
        ...
        rendered_file_path = os.path.join(
            self.basetestpath, "test_exporttoimagecropped.png"
        )
        self.assertEqual(
            exporter.exportToImage(rendered_file_path, settings),
            QgsLayoutExporter.ExportResult.Success,
        ...
        self.assertTrue(
            self.image_check(
                "layoutexporter_exporttoimagecropped_page1",
                "layoutexporter_exporttoimagecropped_page1",
                image,
                color_tolerance=2,
                allowed_mismatch=20,
        ...
        self.assertTrue(
            self.image_check(
                "layoutexporter_exporttoimagecropped_page2",
                "layoutexporter_exporttoimagecropped_page2",
                image,
                color_tolerance=2,
                allowed_mismatch=20,
        ...
        rendered_file_path = os.path.join(
            self.basetestpath, "test_exporttoimagepages.png"
        )
        self.assertEqual(
            exporter.exportToImage(rendered_file_path, settings),
            QgsLayoutExporter.ExportResult.Success,
        ...
        rendered_file_path = os.path.join(
            self.basetestpath, "test_exporttoimagesize.png"
        )
        self.assertEqual(
            exporter.exportToImage(rendered_file_path, settings),
            QgsLayoutExporter.ExportResult.Success,
        ...
        self.assertTrue(
            self.image_check(
                "layoutexporter_exporttoimagesize_page2",
                "layoutexporter_exporttoimagesize_page2",
                image,
                color_tolerance=2,
                allowed_mismatch=20,
        ...
        page2_path = os.path.join(
            self.basetestpath, "test_exporttoimagesizebadaspect_2.png"
        )
        image = QImage(page2_path)
        self.assertTrue(
            self.image_check(
                "layoutexporter_exporttoimagesize_badaspect",
                "layoutexporter_exporttoimagedpi_page2",
                image,
                color_tolerance=2,
                allowed_mismatch=20,
    ...
    def testExportToPdf(self):
        ...
        self.assertTrue(
            self.image_check(
                "layoutexporter_exporttopdfdpi_page2",
                "layoutexporter_exporttopdfdpi_page2",
                image,
                color_tolerance=2,
                allowed_mismatch=20,
        ...
        namespaces = dict(
            [node for _, node in etree.iterparse(StringIO(xmp), events=["start-ns"])]
        )

        title = xmpDoc.findall(
            "rdf:RDF/rdf:Description/dc:title/rdf:Alt/rdf:li", namespaces
        )
        self.assertEqual(len(title), 1)
        self.assertEqual(title[0].text, projTitle)

        creator = xmpDoc.findall(
            "rdf:RDF/rdf:Description/dc:creator/rdf:Seq/rdf:li", namespaces
        )
        ...
        self.assertEqual(
            producer[0].attrib["{" + namespaces["pdf"] + "}" + "Producer"], qgisId
        )

        producer2 = xmpDoc.findall(
            "rdf:RDF/rdf:Description[@xmp:CreatorTool]", namespaces
        )
        self.assertEqual(len(producer2), 1)
        self.assertEqual(
            producer2[0].attrib["{" + namespaces["xmp"] + "}" + "CreatorTool"], qgisId
        )

        creationDateTags = xmpDoc.findall(
            "rdf:RDF/rdf:Description[@xmp:CreateDate]", namespaces
        )
        self.assertEqual(len(creationDateTags), 1)
        creationDate = creationDateTags[0].attrib[
            "{" + namespaces["xmp"] + "}" + "CreateDate"
        ]
        self.assertEqual(creationDate, "2011-05-03T09:04:05+10:00")

        metadataDateTags = xmpDoc.findall(
            "rdf:RDF/rdf:Description[@xmp:MetadataDate]", namespaces
        )
        self.assertEqual(len(metadataDateTags), 1)
        metadataDate = metadataDateTags[0].attrib[
            "{" + namespaces["xmp"] + "}" + "MetadataDate"
        ]
        self.assertEqual(metadataDate, "2011-05-03T09:04:05+10:00")

        modifyDateTags = xmpDoc.findall(
            "rdf:RDF/rdf:Description[@xmp:ModifyDate]", namespaces
        )
        self.assertEqual(len(modifyDateTags), 1)
        modifyDate = modifyDateTags[0].attrib[
            "{" + namespaces["xmp"] + "}" + "ModifyDate"
        ]
        self.assertEqual(modifyDate, "2011-05-03T09:04:05+10:00")

        docIdTags = xmpDoc.findall(
            "rdf:RDF/rdf:Description[@xmpMM:DocumentID]", namespaces
        )
    ...
    def testExportToPdfGeoreference(self):
        ...
        pdf_file_path = os.path.join(
            self.basetestpath, "test_exporttopdf_georeference.pdf"
        )
        self.assertEqual(
            exporter.exportToPdf(pdf_file_path, settings),
            QgsLayoutExporter.ExportResult.Success,
        ...
        pdf_file_path = os.path.join(
            self.basetestpath, "test_exporttopdf_nogeoreference.pdf"
        )
        self.assertEqual(
            exporter.exportToPdf(pdf_file_path, settings),
            QgsLayoutExporter.ExportResult.Success,
    ...
    def testExportToPdfSkipFirstPage(self):
        ...
        pdf_file_path = os.path.join(
            self.basetestpath, "test_exporttopdfdpi_skip_first.pdf"
        )
        self.assertEqual(
            exporter.exportToPdf(pdf_file_path, settings),
            QgsLayoutExporter.ExportResult.Success,
        ...
        rendered_page_1 = os.path.join(
            self.basetestpath, "test_exporttopdfdpi_skip_first.png"
        )
        dpi = 80
        pdfToPng(pdf_file_path, rendered_page_1, dpi=dpi, page=1)
        ...
        self.assertTrue(
            self.image_check(
                "test_exporttopdfdpi_skip_first",
                "layoutexporter_exporttopdfdpi_page2",
                image,
                color_tolerance=2,
                allowed_mismatch=20,
    ...
    def testExportToSvg(self):
        ...
        self.assertEqual(
            exporter.exportToSvg(svg_file_path, settings),
            QgsLayoutExporter.ExportResult.Success,
        )
        ...
        def checkMetadata(f, expected):
            ...
            self.assertEqual(
                'xmlns:cc="http://creativecommons.org/ns#"' in open(f).read(), expected
            )
        ...
        svg_file_path_2 = os.path.join(
            self.basetestpath, "test_exporttosvglayered_2.svg"
        )
        self.assertEqual(
            exporter.exportToSvg(svg_file_path, settings),
            QgsLayoutExporter.ExportResult.Success,
        ...
        svgToPng(svg_file_path, rendered_page_1, width=936)
        rendered_page_2 = os.path.join(
            self.basetestpath, "test_exporttosvglayered2.png"
        )
        svgToPng(svg_file_path_2, rendered_page_2, width=467)
    ...
    def testExportToSvgTextRenderFormat(self):
        ...
        svg_file_path = os.path.join(
            self.basetestpath, "test_exporttosvgtextformattext.svg"
        )
        self.assertEqual(
            exporter.exportToSvg(svg_file_path, settings),
            QgsLayoutExporter.ExportResult.Success,
    ...
    def testPrint(self):
        ...
        self.assertEqual(
            exporter.print(printer, settings), QgsLayoutExporter.ExportResult.Success
        )
        self.assertTrue(os.path.exists(pdf_file_path))
    ...
    def testExportWorldFile(self):
        ...
        rendered_file_path = os.path.join(
            self.basetestpath, "test_exportwithworldfile.png"
        )
        world_file_path = os.path.join(
            self.basetestpath, "test_exportwithworldfile.pgw"
        )
        self.assertEqual(
            exporter.exportToImage(rendered_file_path, settings),
            QgsLayoutExporter.ExportResult.Success,
    ...
    def testExcludePagesImage(self):
        ...
        rendered_file_path = os.path.join(
            self.basetestpath, "test_exclude_export_excluded.png"
        )
    ...
    def testPageFileName(self):
        ...
        details.page = 1
        self.assertEqual(
            exporter.generateFileName(details), "/tmp/output/my_maps_2.png"
        )
        details.page = 2
        self.assertEqual(
    ...
    def testIteratorToImages(self):
        ...
        page1_path = os.path.join(
            self.basetestpath, "test_exportiteratortoimage_Basse-Normandie.png"
        )
        image = QImage(page1_path)
        self.assertTrue(
            self.image_check(
                "iteratortoimage1",
                "layoutexporter_iteratortoimage1",
                image,
                color_tolerance=2,
                allowed_mismatch=20,
            )
        ...
        page2_path = os.path.join(
            self.basetestpath, "test_exportiteratortoimage_Bretagne.png"
        )
        image = QImage(page2_path)
        self.assertTrue(
            self.image_check(
                "iteratortoimage2",
                "layoutexporter_iteratortoimage2",
                image,
                color_tolerance=2,
                allowed_mismatch=20,
            )
        )
        page3_path = os.path.join(
            self.basetestpath, "test_exportiteratortoimage_Centre.png"
        )
        self.assertTrue(os.path.exists(page3_path))
        page4_path = os.path.join(
            self.basetestpath, "test_exportiteratortoimage_Pays de la Loire.png"
        )
    ...
    def testIteratorToSvgs(self):
        ...
        result, error = QgsLayoutExporter.exportToSvg(
            atlas, self.basetestpath + "/", settings
        )
        self.assertEqual(result, QgsLayoutExporter.ExportResult.Success, error)

        page1_path = os.path.join(
            self.basetestpath, "test_exportiteratortosvg_Basse-Normandie.svg"
        )
        rendered_page_1 = os.path.join(
            self.basetestpath, "test_exportiteratortosvg_Basse-Normandie.png"
        )
        ...
        page2_path = os.path.join(
            self.basetestpath, "test_exportiteratortosvg_Bretagne.svg"
        )
        rendered_page_2 = os.path.join(
            self.basetestpath, "test_exportiteratortosvg_Bretagne.png"
        )
        ...
        page3_path = os.path.join(
            self.basetestpath, "test_exportiteratortosvg_Centre.svg"
        )
        self.assertTrue(os.path.exists(page3_path))
        page4_path = os.path.join(
            self.basetestpath, "test_exportiteratortosvg_Pays de la Loire.svg"
        )
        self.assertTrue(os.path.exists(page4_path))
    ...
    def testIteratorToPdfs(self):
        ...
        page1_path = os.path.join(
            self.basetestpath, "test_exportiteratortopdf_Basse-Normandie.pdf"
        )
        rendered_page_1 = os.path.join(
            self.basetestpath, "test_exportiteratortopdf_Basse-Normandie.png"
        )
        ...
        page2_path = os.path.join(
            self.basetestpath, "test_exportiteratortopdf_Bretagne.pdf"
        )
        rendered_page_2 = os.path.join(
            self.basetestpath, "test_exportiteratortopdf_Bretagne.png"
        )
        ...
        page3_path = os.path.join(
            self.basetestpath, "test_exportiteratortopdf_Centre.pdf"
        )
        self.assertTrue(os.path.exists(page3_path))
        page4_path = os.path.join(
            self.basetestpath, "test_exportiteratortopdf_Pays de la Loire.pdf"
        )
        self.assertTrue(os.path.exists(page4_path))
    ...
    def testIteratorToPdf(self):
        ...
        pdf_path = os.path.join(
            self.basetestpath, "test_exportiteratortopdf_single.pdf"
        )
        result, error = QgsLayoutExporter.exportToPdf(atlas, pdf_path, settings)
        self.assertEqual(result, QgsLayoutExporter.ExportResult.Success, error)

        rendered_page_1 = os.path.join(
            self.basetestpath, "test_exportiteratortopdf_single1.png"
        )
        pdfToPng(pdf_path, rendered_page_1, dpi=80, page=1)
        image = QImage(rendered_page_1)
        self.assertTrue(
            self.image_check(
                "iteratortopdfsingle1",
                "layoutexporter_iteratortoimage1",
                image,
                color_tolerance=2,
                allowed_mismatch=20,
        ...
        rendered_page_2 = os.path.join(
            self.basetestpath, "test_exportiteratortopdf_single2.png"
        )
        pdfToPng(pdf_path, rendered_page_2, dpi=80, page=2)
        image = QImage(rendered_page_2)
        self.assertTrue(
            self.image_check(
                "iteratortopdfsingle2",
                "layoutexporter_iteratortoimage2",
                image,
                color_tolerance=2,
                allowed_mismatch=20,
        ...
        rendered_page_3 = os.path.join(
            self.basetestpath, "test_exportiteratortopdf_single3.png"
        )
        pdfToPng(pdf_path, rendered_page_3, dpi=80, page=3)
        self.assertTrue(os.path.exists(rendered_page_3))
        rendered_page_4 = os.path.join(
            self.basetestpath, "test_exportiteratortopdf_single4.png"
        )
    ...
    def testLabelingResults(self):
        ...
        f.setAttributes([1])
        f.setGeometry(
            QgsGeometry.fromPointXY(QgsPointXY(-6.250851540391068, 53.335006994584944))
        )
        self.assertTrue(vl.dataProvider().addFeature(f))
        f.setAttributes([8888])
        f.setGeometry(
            QgsGeometry.fromPointXY(QgsPointXY(-21.950014487179544, 64.150023619739216))
        )
        self.assertTrue(vl.dataProvider().addFeature(f))
        f.setAttributes([33333])
        f.setGeometry(
            QgsGeometry.fromPointXY(QgsPointXY(-0.118667702475932, 51.5019405883275))
        )
        self.assertTrue(vl.dataProvider().addFeature(f))
        vl.updateExtents()
        ...
        rendered_file_path = os.path.join(
            self.basetestpath, "test_exportlabelresults.png"
        )
        self.assertEqual(
            exporter.exportToImage(rendered_file_path, settings),
            QgsLayoutExporter.ExportResult.Success,
