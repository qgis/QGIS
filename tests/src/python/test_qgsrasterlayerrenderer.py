...
class TestQgsRasterLayerRenderer(QgisTestCase):
    ...
    def test_contour_render_dem_with_z_range_filter(self):
        ...
        renderer.setContourSymbol(QgsLineSymbol.createSimple({"width": 1}))
    ...
    def test_render_fixed_elevation_range_with_z_range_filter(self):
        ...
        raster_layer.elevationProperties().setFixedRange(QgsDoubleRange(33, 38))
    ...
    def test_render_dynamic_range_per_band_with_z_range_filter(self):
        ...
        raster_layer.elevationProperties().dataDefinedProperties().setProperty(
            ...
            QgsProperty.fromExpression(
                "case when @band=3 then 33 when @band=4 then 35 when @band=5 then 40 else null end"
            ),
        )
        raster_layer.elevationProperties().dataDefinedProperties().setProperty(
            QgsRasterLayerElevationProperties.Property.RasterPerBandUpperElevation,
            QgsProperty.fromExpression(
                "case when @band=3 then 38 when @band=4 then 40 when @band=5 then 48 else null end"
            ),
        )
    ...
    def test_render_fixed_range_per_band_with_temporal_range_filter(self):
        ...
        raster_layer.temporalProperties().setFixedRangePerBand(
            {
                ...
                4: QgsDateTimeRange(
                    QDateTime(QDate(2023, 5, 7), QTime(12, 13, 14)),
                    QDateTime(QDate(2023, 5, 9), QTime(12, 13, 14)),
                ),
                5: QgsDateTimeRange(
                    QDateTime(QDate(2023, 5, 9), QTime(12, 13, 14)),
                    QDateTime(QDate(2023, 5, 11), QTime(12, 13, 14)),
                ),
            }
        )
        ...
        map_settings.setTemporalRange(
            QgsDateTimeRange(
                QDateTime(QDate(2023, 5, 3), QTime(12, 13, 14)),
                QDateTime(QDate(2023, 5, 6), QTime(13, 13, 14)),
            )
        )
        self.assertTrue(
            self.render_map_settings_check(
        ...
        map_settings.setTemporalRange(
            QgsDateTimeRange(
                QDateTime(QDate(2023, 5, 5), QTime(12, 13, 14)),
                QDateTime(QDate(2023, 5, 8), QTime(13, 13, 14)),
            )
        )
        self.assertTrue(
            self.render_map_settings_check(
        ...
        map_settings.setTemporalRange(
            QgsDateTimeRange(
                QDateTime(QDate(2023, 5, 10), QTime(12, 13, 14)),
                QDateTime(QDate(2023, 5, 15), QTime(13, 13, 14)),
            )
        )
        self.assertTrue(
            self.render_map_settings_check(
        ...
        map_settings.setTemporalRange(
            QgsDateTimeRange(
                QDateTime(QDate(2024, 5, 10), QTime(12, 13, 14)),
                QDateTime(QDate(2024, 5, 15), QTime(13, 13, 14)),
            )
        )
        self.assertTrue(
            self.render_map_settings_check(
    ...
    def test_render_represents_temporal_values(self):
        ...
        raster_layer.temporalProperties().setTemporalRepresentationOffset(
            QDateTime(QDate(2024, 1, 1), QTime(0, 0, 0))
        )
        raster_layer.temporalProperties().setTemporalRepresentationScale(
            QgsInterval(1, Qgis.TemporalUnit.Days)
        )
