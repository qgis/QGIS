# -*- coding: utf-8 -*-
"""QGIS Unit tests for the Redshift project storage.
...
class TestPyQgsProjectStoragePostgres(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Create test layers
        cls.vl = QgsVectorLayer(cls.dbconn + ' sslmode=disable key=\'pk\' srid=4326 type=POINT table="qgis_test"."some_data" (geom) sql=', 'test', 'redshift')
    ...
    def testSaveLoadProject(self):
        ...
        prj = QgsProject()
        uri = self.vl.source()
        vl1 = QgsVectorLayer(uri, 'test', 'redshift')
        self.assertEqual(vl1.isValid(), True)
        prj.addMapLayer(vl1)

        prj_storage = QgsApplication.projectStorageRegistry().projectStorageFromType("redshift")
        ...
        self.assertTrue(abs(prj2.lastModified().secsTo(QDateTime.currentDateTime())) < 100)

        # check that layer is the same
        vector_layer = prj2.mapLayersByName('test')[0]
        self.assertEqual(vector_layer.source(), uri)
        ...
        self.assertTrue(abs(time_diff) < 30)
