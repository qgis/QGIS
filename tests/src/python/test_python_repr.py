...
class TestPython__repr__(QgisTestCase):
    ...
    def testQgsGeometryRepr(self):
        ...
        self.assertTrue(g.__repr__().endswith("...>"))
    ...
    def testQgsReferencedPointXYRepr(self):
        p = QgsReferencedPointXY(
            QgsPointXY(123.456, 987.654), QgsCoordinateReferenceSystem("EPSG:4326")
        )
    ...
    def testQgsCircularstringRepr(self):
        ...
        self.assertEqual(
            cs.__repr__(), "<QgsCircularString: CircularString (1 2, 2 3, 3 4)>"
        )
    ...
    def testQgsMulticurveRepr(self):
        ...
        self.assertEqual(
            mc.__repr__(),
            "<QgsMultiCurve: MultiCurve (CircularString (1 10, 2 11, 3 12),CircularString (4 20, 5 22, 6 24))>",
        )
    ...
    def testQgsMultilineStringRepr(self):
        ...
        self.assertEqual(
            ml.constGet().__repr__(),
            "<QgsMultiLineString: MultiLineString ((0 0, 1 0, 1 1, 2 1, 2 0),(3 0, 3 1, 5 1, 5 0, 6 0))>",
        )
    ...
    def testQgsMultipolygonRepr(self):
        ...
        self.assertEqual(
            mp.constGet().__repr__(),
            "<QgsMultiPolygon: MultiPolygon (((1 1, 2 2, 1 2, 1 1)),((2 2, 3 3, 3 1, 2 2)))>",
        )
    ...
    def testQgsPolygonRepr(self):
        ...
        self.assertEqual(
            p.constGet().__repr__(), "<QgsPolygon: Polygon ((0 0, 2 0, 2 2, 0 2, 0 0))>"
        )
    ...
    def testQgsReferencedGeometryRepr(self):
        g = QgsReferencedGeometry(
            QgsGeometry.fromPointXY(QgsPointXY(1, 2)),
            QgsCoordinateReferenceSystem("EPSG:4326"),
        )
        self.assertEqual(
            g.__repr__(), "<QgsReferencedGeometry: Point (1 2) (EPSG:4326)>"
        )
    ...
    def testQgsCoordinateReferenceSystem(self):
        ...
        self.assertEqual(
            crs.__repr__(), "<QgsCoordinateReferenceSystem: EPSG:4326 @ 2021.3>"
        )
    ...
    def testQgsCoordinateTransform(self):
        ...
        self.assertEqual(
            xform.__repr__(), "<QgsCoordinateTransform: EPSG:4326 to NULL>"
        )
        xform = QgsCoordinateTransform(
            QgsCoordinateReferenceSystem(),
            QgsCoordinateReferenceSystem("EPSG:4326"),
        ...
        self.assertEqual(
            xform.__repr__(), "<QgsCoordinateTransform: NULL to EPSG:4326>"
        )
        xform = QgsCoordinateTransform(
        ...
        self.assertEqual(
            xform.__repr__(), "<QgsCoordinateTransform: EPSG:3111 to EPSG:4326>"
        )
    ...
    def testQgsProjectRepr(self):
        ...
        self.assertEqual(
            QgsProject.instance().__repr__(), "<QgsProject: '' (singleton instance)>"
        )
        QgsProject.instance().setFileName("/home/test/my_project.qgs")
        self.assertEqual(
            QgsProject.instance().__repr__(),
            "<QgsProject: '/home/test/my_project.qgs' (singleton instance)>",
        )
    ...
    def testQgsBookmark(self):
        ...
        self.assertEqual(
            b.__repr__(), "<QgsBookmark: 'test bookmark' (1 2, 3 4 - EPSG:3111)>"
        )
    ...
    def testQgsConditionalStyle(self):
        ...
        b.setName("test name")
        self.assertEqual(
            b.__repr__(), "<QgsConditionalStyle: 'test name' (@value > 20)>"
        )
    ...
    def testQgsProperty(self):
        ...
        self.assertEqual(
            p.__repr__(), "<QgsProperty: INACTIVE expression (5*5 || 'a')>"
        )
        p = QgsProperty()
    ...
    def testProviderMetadata(self):
        self.assertEqual(
            QgsProviderRegistry.instance().providerMetadata("ogr").__repr__(),
            "<QgsProviderMetadata: ogr>",
        )
    ...
    def testDataSourceUri(self):
        ...
        self.assertEqual(
            ds.__repr__(),
            "<QgsDataSourceUri: dbname='my_db' host=my_host port=2322 user='user' password='pw'>",
        )

    def testDoubleRange(self):
        self.assertEqual(QgsDoubleRange(1, 10).__repr__(), "<QgsDoubleRange: [1, 10]>")
        self.assertEqual(
            QgsDoubleRange(1, 10, False).__repr__(), "<QgsDoubleRange: (1, 10]>"
        )
        self.assertEqual(
            QgsDoubleRange(1, 10, True, False).__repr__(), "<QgsDoubleRange: [1, 10)>"
        )

    def testIntRange(self):
        self.assertEqual(QgsIntRange(1, 10).__repr__(), "<QgsIntRange: [1, 10]>")
        self.assertEqual(QgsIntRange(1, 10, False).__repr__(), "<QgsIntRange: (1, 10]>")
        self.assertEqual(
            QgsIntRange(1, 10, True, False).__repr__(), "<QgsIntRange: [1, 10)>"
        )
    ...
    def testRendererRange(self):
        ...
        self.assertEqual(
            QgsRendererRange(1.0, 2.0, None, None).__repr__(),
            "<QgsRendererRange: 1 - 2>",
        )
        self.assertEqual(
            QgsRendererRange(1.0, 2.0, None, "my class").__repr__(),
            "<QgsRendererRange: 1 - 2 (my class)>",
        )

    def testRendererCategory(self):
        self.assertEqual(QgsRendererCategory().__repr__(), "<QgsRendererCategory>")
        self.assertEqual(
            QgsRendererCategory(5, None, None).__repr__(), "<QgsRendererCategory: 5>"
        )
        self.assertEqual(
            QgsRendererCategory("abc", None, None).__repr__(),
            "<QgsRendererCategory: abc>",
        )
        self.assertEqual(
            QgsRendererCategory("abc", None, "my class").__repr__(),
            "<QgsRendererCategory: abc (my class)>",
        )
