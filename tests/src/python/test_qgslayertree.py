...
class TestQgsLayerTree(QgisTestCase):
    ...
    def testCustomLayerOrder(self):
        ...
        prj.layerTreeRoot().setCustomLayerOrder([layer2, layer])
        self.assertEqual(
            len(layer_order_changed_spy), 1
        )  # no signal, order not changed
    ...
    def test_group_layer_updates_from_node(self):
        ...
        self.assertEqual(
            group_layer.childLayers(), [layer5, layer, layer4, layer3, layer2]
        )
    ...
    def test_layer_order_with_group_layer(self):
        ...
        self.assertEqual(
            p.layerTreeRoot().layerOrder(), [layer, layer2, layer3, layer4]
        )
    ...
    def test_nested_groups(self):
        ...
        self.assertEqual(
            p.layerTreeRoot().checkedLayers(), [layer, layer2, layer3, layer4]
        )
        ...
        self.assertEqual(p.layerTreeRoot().checkedLayers(), [group_layer])
        self.assertEqual(
            group_layer.childLayers(), [grandchild_group_layer, layer3, layer2, layer]
        )
        ...
        self.assertEqual(
            child_group_layer.childLayers(), [grandchild_group_layer, layer3]
        )
        self.assertEqual(grandchild_group_layer.childLayers(), [layer4])
        self.assertGreater(len(spy), spy_count)
        spy_count = len(spy)
    ...
    def test_reorder_group_layers(self):
        ...
        self.assertEqual(
            [l.layer() for l in group.children()], [layer, layer2, layer3, layer4]
        )
        ...
        self.assertEqual(
            [l.layer() for l in group.children()], [layer4, layer2, layer, layer3]
        )

        group.reorderGroupLayers([layer3])
        self.assertEqual(
            [l.layer() for l in group.children()], [layer3, layer4, layer2, layer]
        )
        ...
        self.assertEqual(
            [
                l.layer() if isinstance(l, QgsLayerTreeLayer) else "group"
                for l in group.children()
            ],
            [layer, layer3, layer4, layer2, "group"],
        )
