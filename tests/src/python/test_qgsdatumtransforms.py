...
class TestPyQgsDatumTransform(QgisTestCase):
    ...
    def testOperations(self):
        ...
        ops = QgsDatumTransform.operations(
            QgsCoordinateReferenceSystem("EPSG:3111"), QgsCoordinateReferenceSystem()
        )
        self.assertEqual(ops, [])
        ops = QgsDatumTransform.operations(
            QgsCoordinateReferenceSystem(), QgsCoordinateReferenceSystem("EPSG:3111")
        )
        self.assertEqual(ops, [])
    ...
    @unittest.skipIf(
        QgsProjUtils.projVersionMajor() > 9
        or (
            QgsProjUtils.projVersionMajor() == 9
            and QgsProjUtils.projVersionMinor() >= 2
        ),
        "NADCON5 support added in Proj 9.2",
    )
    def testNoLasLos(self):
        ...
        """
        ops = QgsDatumTransform.operations(
            QgsCoordinateReferenceSystem("EPSG:4138"),
    ...
    def testDatumEnsembles(self):
        ...
        self.assertEqual(
            str(ensemble),
            "<QgsDatumEnsemble: World Geodetic System 1984 ensemble (EPSG:6326)>",
        )
        self.assertEqual(
            ensemble.members()[0].name(), "World Geodetic System 1984 (Transit)"
        )
        self.assertEqual(ensemble.members()[0].authority(), "EPSG")
        self.assertEqual(ensemble.members()[0].code(), "1166")
        self.assertEqual(
            ensemble.members()[0].scope(),
            "Geodesy. Navigation and positioning using GPS satellite system.",
        )
        self.assertEqual(
            str(ensemble.members()[0]),
            "<QgsDatumEnsembleMember: World Geodetic System 1984 (Transit) (EPSG:1166)>",
        )
        self.assertEqual(
            ensemble.members()[1].name(), "World Geodetic System 1984 (G730)"
        )
        self.assertEqual(ensemble.members()[1].authority(), "EPSG")
        self.assertEqual(ensemble.members()[1].code(), "1152")
        self.assertEqual(
            ensemble.members()[1].scope(),
            "Geodesy. Navigation and positioning using GPS satellite system.",
        )
        ...
        self.assertTrue(ensemble.isValid())
        self.assertEqual(
            ensemble.name(), "European Terrestrial Reference System 1989 ensemble"
        )
        self.assertEqual(ensemble.authority(), "EPSG")
