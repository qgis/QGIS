...
class TestQgsNurbsCurve(unittest.TestCase):
    ...
    def testEvaluateExceptions(self):
        """Test that evaluate() raises ValueError for invalid parameter t"""
        control_points = [QgsPoint(0, 0), QgsPoint(10, 10)]
        degree = 1
        knots = [0, 0, 1, 1]
        weights = [1, 1]
        ...
        # Test t < 0 raises ValueError
        with self.assertRaises(ValueError):
            curve.evaluate(-0.1)

        # Test t > 1 raises ValueError
        with self.assertRaises(ValueError):
            curve.evaluate(1.1)
    ...
    def testWeightExceptions(self):
        """Test that weight() raises IndexError for invalid index"""
        control_points = [QgsPoint(0, 0), QgsPoint(5, 5), QgsPoint(10, 0)]
        degree = 2
        knots = [0, 0, 0, 1, 1, 1]
        weights = [1, 2, 1]
        ...
        self.assertEqual(curve.weight(0), 1.0)
        self.assertEqual(curve.weight(1), 2.0)
        self.assertEqual(curve.weight(2), 1.0)

        # Test negative index raises IndexError
        with self.assertRaises(IndexError):
            curve.weight(-1)

        # Test index >= count raises IndexError
        with self.assertRaises(IndexError):
            curve.weight(3)
    ...
    def testSetWeightExceptions(self):
        """Test that setWeight() raises exceptions for invalid parameters"""
        control_points = [QgsPoint(0, 0), QgsPoint(5, 5), QgsPoint(10, 0)]
        degree = 2
        knots = [0, 0, 0, 1, 1, 1]
        weights = [1, 1, 1]
        ...
        # Test valid setWeight works
        curve.setWeight(1, 2.0)
        self.assertEqual(curve.weight(1), 2.0)

        # Test negative index raises IndexError
        with self.assertRaises(IndexError):
            curve.setWeight(-1, 1.0)

        # Test index >= count raises IndexError
        with self.assertRaises(IndexError):
            curve.setWeight(3, 1.0)

        # Test weight <= 0 raises ValueError
        with self.assertRaises(ValueError):
            curve.setWeight(0, 0.0)
