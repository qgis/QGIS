...
class TestQgsRasterRendererCreateSld(QgisTestCase):
    ...
    def testSingleBandPseudoColorRenderer_Interpolated(self):
        ...
        stats = self.raster_layer.dataProvider().bandStatistics(
            bandNo, QgsRasterBandStats.Stats.Min | QgsRasterBandStats.Stats.Max
        )
        ...
        colorRampShaderFcn.setClassificationMode(
    ...
    def testSingleBandPseudoColorRenderer_Exact(self):
        ...
        for index in range(255):
            items.append(
                QgsColorRampShader.ColorRampItem(
                    index, QColor.fromHsv(index, 255, 255, 255), f"{index}"
                )
            )
        colorRampShaderFcn.setColorRampItemList(items)
        shader.setRasterShaderFunction(colorRampShaderFcn)
    ...
    def testSingleBandGrayRenderer(self):
        ...
        self.assertEqual(
            clorMap1.attributes().namedItem("color").nodeValue(), "#000000"
        )
        self.assertEqual(clorMap1.attributes().namedItem("quantity").nodeValue(), "0")

        clorMap2 = colorMapEntries.at(1)
        self.assertEqual(
            clorMap2.attributes().namedItem("color").nodeValue(), "#ffffff"
        )
        ...
        self.raster_layer.setContrastEnhancement(
            algorithm=QgsContrastEnhancement.ContrastEnhancementAlgorithm.StretchAndClipToMinimumMaximum,
            limits=QgsRasterMinMaxOrigin.Limits.MinMax,
        )
        ...
        # then min/max have always to be the real one and not that set in the contrastEnhancement
        self.raster_layer.setContrastEnhancement(
            algorithm=QgsContrastEnhancement.ContrastEnhancementAlgorithm.ClipToMinimumMaximum,
            limits=QgsRasterMinMaxOrigin.Limits.MinMax,
        )
        minmum = self.raster_layer.renderer().contrastEnhancement().setMinimumValue(100)
        maximum = self.raster_layer.renderer().contrastEnhancement().maximumValue()
    ...
    def testStretchingAlgorithm(self):
        ...
        self.assertContrastEnhancement(
            root, "sld:RedChannel", "StretchToMinimumMaximum", "51", "172"
        )
        self.assertContrastEnhancement(
            root, "sld:GreenChannel", "StretchToMinimumMaximum", "122", "130"
        )
        self.assertContrastEnhancement(
            root, "sld:BlueChannel", "StretchToMinimumMaximum", "133", "148"
        )
        ...
        dom, root = self.rendererToSld(self.raster_layer.renderer())
        self.assertContrastEnhancement(
            root, "sld:RedChannel", "ClipToZero", "51", "172"
        )
        self.assertContrastEnhancement(
            root, "sld:GreenChannel", "ClipToZero", "122", "130"
        )
        self.assertContrastEnhancement(
            root, "sld:BlueChannel", "ClipToZero", "133", "148"
        )
        ...
        dom, root = self.rendererToSld(self.raster_layer.renderer())
        self.assertContrastEnhancement(
            root, "sld:RedChannel", "ClipToMinimumMaximum", "51", "172"
        )
        self.assertContrastEnhancement(
            root, "sld:GreenChannel", "ClipToMinimumMaximum", "122", "130"
        )
        self.assertContrastEnhancement(
            root, "sld:BlueChannel", "ClipToMinimumMaximum", "133", "148"
        )
    ...
    def assertContrastEnhancement(
        ...
        contrastEnhancementName = band.firstChildElement("sld:ContrastEnhancement")
        self.assertEqual(
            "sld:Normalize", contrastEnhancementName.firstChild().nodeName()
        )
