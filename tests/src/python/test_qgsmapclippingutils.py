...
class TestQgsMapClippingUtils(QgisTestCase):
    ...
    def testClippingRegionsForLayer(self):
        ...
        self.assertEqual(
            regions[0].geometry().asWkt(1), "Polygon ((0 0, 1 0, 1 1, 0 1, 0 0))"
        )
        self.assertEqual(
            regions[1].geometry().asWkt(1), "Polygon ((0 0, 0.1 0, 0.1 2, 0 2, 0 0))"
        )
    ...
    def testCalculateFeatureRequestGeometry(self):
        ...
        geom, should_clip = QgsMapClippingUtils.calculateFeatureRequestGeometry(
            [region], rc
        )
        geom.normalize()
        ...
        self.assertTrue(should_clip)
        self.assertEqual(
            geom.asWkt(0), "Polygon ((0 0, 0 111325, 11132 111325, 11132 0, 0 0))"
        )
    ...
    def testCalculateFeatureIntersectionGeometry(self):
        ...
        geom, should_clip = QgsMapClippingUtils.calculateFeatureIntersectionGeometry(
            [], rc
        )
        self.assertFalse(should_clip)
        self.assertTrue(geom.isNull())
