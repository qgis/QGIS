...
class TestQgsSQLStatementCustomFunctions(unittest.TestCase):
    ...
    def testNominalSimple(self):
        ...
        self.assertEqual(statement_node.nodeType(), QgsSQLStatement.NodeType.ntSelect)
        tables = statement_node.tables()
        self.assertEqual(len(tables), 1)
        table = tables[0]
        self.assertEqual(table.nodeType(), QgsSQLStatement.NodeType.ntTableDef)
        self.assertEqual(table.name(), "t")
        self.assertEqual(table.alias(), "")
        columns = statement_node.columns()
        self.assertEqual(len(columns), 1)
        column = columns[0]
        self.assertEqual(column.nodeType(), QgsSQLStatement.NodeType.ntSelectedColumn)
        column_ref = column.column()
        self.assertEqual(column.alias(), "")
        self.assertEqual(column_ref.nodeType(), QgsSQLStatement.NodeType.ntColumnRef)
    ...
    def testNominalFrom(self):
        ...
        expected_dump = (
            'SELECT a FROM t1, t2 AS at2, t3 AS at3, "1quoted", t4 AS "2quoted"'
        )
        self.checkNominal(statement, expected_dump)

    def testNominalWhere(self):
        statement = (
            "SELECT a FROM t WHERE 1.5 <= 'a' OR TRUE OR FALSE OR a IS NULL AND b IS NOT NULL "
    ...
    def testJoin(self):
        statement = (
            'SELECT a FROM t JOIN j1 ON TRUE JOIN j2 USING (a) JOIN j3 USING ("1a", b)'
        )
        self.checkNominal(statement)
    ...
    def testBasicValidationCheck(self):
        ...
        exp = QgsSQLStatement("SELECT CAST(1 + foo(t_unknown.a) AS varchar) FROM t")
        (b, errorMsg) = exp.doBasicValidationChecks()
        self.assertFalse(b)
        self.assertEqual(
            errorMsg,
