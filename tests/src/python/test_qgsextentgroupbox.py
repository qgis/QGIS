...
class TestQgsExtentGroupBox(QgisTestCase):
    ...
    def testSnapToGrid(self):
        ...
        # Test default values (snap to grid disabled)
        self.assertFalse(w.snapToGrid())
        self.assertEqual(w.rasterXRes(), 1.0)
        self.assertEqual(w.rasterYRes(), 1.0)
        self.assertEqual(w.rasterMinX(), 0.0)
        self.assertEqual(w.rasterMinY(), 0.0)
        
        # Set original extent and output CRS
        w.setOriginalExtent(QgsRectangle(10, 20, 110, 120), QgsCoordinateReferenceSystem('EPSG:4326'))
        w.setOutputCrs(QgsCoordinateReferenceSystem('EPSG:4326'))
        w.setOutputExtentFromOriginal()
        
        # Verify extent without snapping
        extent_no_snap = w.outputExtent()
        self.assertEqual(extent_no_snap, QgsRectangle(10, 20, 110, 120))
        
        # Enable snap to grid with a 5x10 grid starting at (0,0)
        w.setSnapToGrid(True, 5.0, 10.0, 0.0, 0.0)
        
        # Verify snap to grid is enabled with correct parameters
        self.assertTrue(w.snapToGrid())
        self.assertEqual(w.rasterXRes(), 5.0)
        self.assertEqual(w.rasterYRes(), 10.0)
        self.assertEqual(w.rasterMinX(), 0.0)
        self.assertEqual(w.rasterMinY(), 0.0)
        
        # Verify extent snaps to grid
        extent_snapped = w.outputExtent()
        # Expected results based on the floor/ceil formulas:
        # xmin = 0 + floor((10-0)/5)*5 = 10
        # ymin = 0 + floor((20-0)/10)*10 = 20
        # xmax = 0 + ceil((110-0)/5)*5 = 110
        # ymax = 0 + ceil((120-0)/10)*10 = 120
        self.assertEqual(extent_snapped, QgsRectangle(10, 20, 110, 120))
        
        # Test with a different extent that should actually snap
        w.setOriginalExtent(QgsRectangle(12, 23, 109, 118), QgsCoordinateReferenceSystem('EPSG:4326'))
        w.setOutputExtentFromOriginal()
        
        extent_snapped = w.outputExtent()
        # Expected results based on the floor/ceil formulas:
        # xmin = 0 + floor((12-0)/5)*5 = 10
        # ymin = 0 + floor((23-0)/10)*10 = 20
        # xmax = 0 + ceil((109-0)/5)*5 = 110
        # ymax = 0 + ceil((118-0)/10)*10 = 120
        self.assertEqual(extent_snapped, QgsRectangle(10, 20, 110, 120))
        
        # Test with a different grid origin
        w.setSnapToGrid(True, 5.0, 10.0, 2.0, 3.0)
        extent_snapped = w.outputExtent()
        # Expected results based on the floor/ceil formulas with offset origin:
        # xmin = 2 + floor((12-2)/5)*5 = 12
        # ymin = 3 + floor((23-3)/10)*10 = 23
        # xmax = 2 + ceil((109-2)/5)*5 = 107
        # ymax = 3 + ceil((118-3)/10)*10 = 123
        self.assertEqual(extent_snapped, QgsRectangle(12, 23, 107, 123))
        
        # Disable snap to grid
        w.setSnapToGrid(False, 5.0, 10.0, 0.0, 0.0)
        self.assertFalse(w.snapToGrid())
        
        # Verify extent is not snapped when disabled
        extent_no_snap = w.outputExtent()
        self.assertEqual(extent_no_snap, QgsRectangle(12, 23, 109, 118))
