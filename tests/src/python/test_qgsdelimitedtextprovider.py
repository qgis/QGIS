...
class TestQgsDelimitedTextProviderOther(QgisTestCase):
    ...
    def delimitedTextData(self, testname, filename, requests, verbose, **params):
        ...
        with MessageLogger("DelimitedText") as logger:
            ...
            self.assertTrue(
                compareUrl(
                    ...
                    QgsProviderRegistry.instance().encodeUri(
                        "delimitedtext",
                        QgsProviderRegistry.instance().decodeUri(
                            "delimitedtext", layer.source()
                        ),
                    ),
                )
            )
            ...
            for msg in logger.messages():
                ...
                msg = re.sub(
                    r"file\s+.*" + re.escape(filename), "file " + filelogname, msg
                )
    ...
    def runTest(self, file, requests, **params):
        ...
        log = result["log"]
        failures = []
        if normalize_query_items_order(result["uri"]) != normalize_query_items_order(
            wanted["uri"]
        ):
            msg = "Layer Uri ({}) doesn't match expected ({})".format(
                normalize_query_items_order(result["uri"]),
                normalize_query_items_order(wanted["uri"]),
            )
            print("    " + msg)
            failures.append(msg)
    ...
    def test_026_filter_fid(self):
        ...
        filename = "test.csv"
        params = {"geomType": "none", "type": "csv"}
        requests = [{"fid": 3}, {"fid": 9}, {"fid": 20}, {"fid": 3}]
        self.runTest(filename, requests, **params)
    ...
    def test_045_Z(self):
        ...
        ), "wrong PointZ geometry"
        self.assertEqual(
            vl.extent(),
            QgsRectangle(
                -71.12300000000000466,
                66.32999999999999829,
                -65.31999999999999318,
                78.29999999999999716,
            ),
        )
        self.assertEqual(
            vl.extent3D(),
    ...
    def testCREndOfLineAndWorkingBuffer(self):
        ...
        try:
            basetestfile = os.path.join(
                unitTestDataPath("delimitedtext"), "test_cr_end_of_line.csv"
            )
    ...
    def test_type_detection_csvt(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                [3, 6.789, "text", 2000000000, 4000000000, QDate(2021, 11, 13), False],
            ],
        )
        ...
        vl = self._make_test_file(
            ...
            "Point(x),CoordY,Point(z),Integer64(20),Real(float32),String,Integer(10),Integer64(20),Date,Integer(Boolean)",
            uri_options="xField=X&yField=Y&zField=Z",
        )
        ...
        self.assertGeometriesEqual(
            geometries[-1],
            QgsGeometry.fromWkt(
                "PointZ (-106.10858956482799442 35.40740071231100217 456)"
            ),
        )
    ...
    def test_type_override(self):
        ...
        self.assertEqual(
            attrs,
            [
                [1, False, 9189304972279762602, 1.234, "text"],
        ...
        self.assertEqual(
            attrs,
            [
                [1.0, 0, 9.189304972279763e18, "1.234", "text"],
    ...
    def test_absolute_relative_uri(self):
        ...
        relative_uri = (
            "file:./provider/delimited_xy.csv?crs=epsg:4326&type=csv&xField=X&yField=Y"
        )
