...
class TestQgsLayoutItemLegend(QgisTestCase, LayoutItemTestCase):
    ...
    def test_exclude_from_default_legend_switch_to_manual(self):
        """
        Test layer legend content when switching from auto to manual
        """
        point_path = os.path.join(TEST_DATA_DIR, "points.shp")
        point_layer1 = QgsVectorLayer(point_path, "layer 1", "ogr")
        point_layer2 = QgsVectorLayer(point_path, "layer 2", "ogr")
        ...
        # switch to manual mode -- the original filtered content, excluding
        # layer 2, should be retained
        legend.setAutoUpdateModel(False)
    ...
    def test_sync_mode(self):
        ...
        legend = QgsLayoutItemLegend(layout)
        layout.addLayoutItem(legend)
        self.assertEqual(legend.syncMode(), Qgis.LegendSyncMode.AllProjectLayers)
        ...
        self.assertEqual(
            legend_restore.syncMode(), Qgis.LegendSyncMode.AllProjectLayers
        )
        ...
        self.assertEqual(legend_restore.syncMode(), Qgis.LegendSyncMode.VisibleLayers)
        ...
        self.assertEqual(legend_restore.syncMode(), Qgis.LegendSyncMode.Manual)

    def test_sync_mode_all_project_layers(self):
        """
        Test layer legend content when set to AllProjectLayers sync mode
        """
        point_path = os.path.join(TEST_DATA_DIR, "points.shp")
        point_layer1 = QgsVectorLayer(point_path, "layer 1", "ogr")
        point_layer2 = QgsVectorLayer(point_path, "layer 2", "ogr")
        ...
        # add another layer
        point_layer4 = QgsVectorLayer(point_path, "layer 4", "ogr")
        p.addMapLayers([point_layer4])
        ...
        point_layer4.setRenderer(QgsSingleSymbolRenderer(marker_symbol.clone()))

        self.assertTrue(
            self.render_layout_check("composer_legend_all_project_layers2", layout)
        )

    def test_sync_mode_visible_layers_linked_map(self):
        """
        ...
        # add more layers
        point_layer4 = QgsVectorLayer(point_path, "layer 4", "ogr")
        point_layer5 = QgsVectorLayer(point_path, "layer 5", "ogr")
        p.addMapLayers([point_layer4, point_layer5])
        ...
        point_layer4.setRenderer(QgsSingleSymbolRenderer(marker_symbol.clone()))
        point_layer5.setRenderer(QgsSingleSymbolRenderer(marker_symbol.clone()))
        map.setLayers([point_layer2, point_layer3, point_layer4])

        self.assertTrue(
            self.render_layout_check("composer_legend_visible_layers2", layout)
        )

    def test_sync_mode_visible_layers_no_linked_map(self):
        """
        Test layer legend content when set to VisibleLayers sync mode
        """
        point_path = os.path.join(TEST_DATA_DIR, "points.shp")
        ...
        point_layer4.setRenderer(QgsSingleSymbolRenderer(marker_symbol.clone()))
        point_layer5.setRenderer(QgsSingleSymbolRenderer(marker_symbol.clone()))
        p.layerTreeRoot().findLayer(point_layer5).setItemVisibilityChecked(False)
