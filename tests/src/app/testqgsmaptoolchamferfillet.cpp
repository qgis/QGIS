...
class TestQgsMapToolChamferFillet : public QObject
{
  ...
  private slots:
    ...
    void testChamfer();
    void testFillet();
    void testChamferFilletControlModifier();
...
{
  qDebug() << "TestQgsMapToolChamferFillet::initTestCase()";
...
void TestQgsMapToolChamferFillet::testChamferFilletControlModifier()
{
  TestQgsMapToolUtils utils( mChamferFilletTool );

  QgsSettingsRegistryCore::settingsDigitizingChamferFilletOperation->setValue( "fillet" );

  QCOMPARE( mLayerBase->featureCount(), 2 );
  // coarse fillet
  QgsSettingsRegistryCore::settingsDigitizingChamferFilletSegment->setValue( 3 );
  utils.mouseClick( 1, 1, Qt::LeftButton, Qt::KeyboardModifiers(), true );
  utils.mouseMove( 0.25, 0.5 );
  utils.mouseClick( 0.25, 0.5, Qt::LeftButton, Qt::ControlModifier, true );

  QCOMPARE( mLayerBase->featureCount(), 3 );

  const QString wkt1 = "Polygon ((0 0, 0 1, 0.41 0.93, 0.72 0.72, 0.93 0.41, 1 0, 0 0))";
  QgsFeatureIterator featIte = mLayerBase->getFeatures();
  QgsFeature f1;

  QVERIFY( featIte.nextFeature( f1 ) );
