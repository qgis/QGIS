/***************************************************************************
     testqgssvgcache.cpp
     --------------------
    Date                 : October 2017
    Copyright            : (C) 2017 by Nyall Dawson
    Email                : nyall dot dawson at gmail dot com
 ***************************************************************************
 *                                                                         *
 *   This program is free software; you can redistribute it and/or modify  *
 *   it under the terms of the GNU General Public License as published by  *
 *   the Free Software Foundation; either version 2 of the License, or     *
 *   (at your option) any later version.                                   *
 *                                                                         *
 ***************************************************************************/
#include "qgsapplication.h"
#include "qgssvgcache.h"
#include "qgstest.h"

#include <QApplication>
#include <QDesktopServices>
#include <QDir>
#include <QElapsedTimer>
#include <QFileInfo>
#include <QObject>
#include <QPainter>
#include <QPicture>
#include <QString>
#include <QStringList>
#include <QtConcurrent>

/**
 * \ingroup UnitTests
 * This is a unit test for QgsSvgCache.
 */
class TestQgsSvgCache : public QgsTest
{
    Q_OBJECT

  public:
    TestQgsSvgCache()
      : QgsTest( u"QgsSvgCache Tests"_s ) {}

  private:
    bool imageCheck( const QString &testName, QImage &image, int mismatchCount );

  private slots:
    void initTestCase();    // will be called before the first testfunction is executed.
    void cleanupTestCase(); // will be called after the last testfunction was executed.
    void init() {}          // will be called before each testfunction is executed.
    void cleanup() {}       // will be called after every testfunction.
    void fillCache();
    void broken();
    void threadSafePicture();
    void threadSafeImage();
    void changeImage(); //check that cache is updated if svg source file changes
    void base64();
    void replaceParams();
    void dynamicSvg();
    void aspectRatio();
    void noViewBox();
};


void TestQgsSvgCache::initTestCase()
{
  QgsApplication::init();
  QgsApplication::initQgis();
}

void TestQgsSvgCache::cleanupTestCase()
{
  QgsApplication::exitQgis();
}

void TestQgsSvgCache::fillCache()
{
  QgsSvgCache cache;
  // flood cache to fill it
  const QString svgPath = TEST_DATA_DIR + u"/sample_svg.svg"_s;
  bool fitInCache = false;

  // we loop forever, continually increasing the size of the requested
  // svg render. The continually changing image size should quickly fill
  // the svg cache size, forcing use of non-cached images.
  // We break after hitting a certain threshold of non-cached images,
  // (after testing that the result is non-null, i.e. rendered on demand,
  // not from cache).
  int uncached = 0;
  for ( double size = 1000; uncached < 10; size += 100 )
  {
    const QImage image = cache.svgAsImage( svgPath, size, QColor( 255, 0, 0 ), QColor( 0, 255, 0 ), 1, 1, fitInCache );
    QVERIFY( !image.isNull() );
    if ( !fitInCache )
      uncached++;
  }
}

void TestQgsSvgCache::broken()
{
  QgsSvgCache cache;
  bool missingImage = false;
  QByteArray content = cache.svgContent( u"bbbbbbb"_s, 14, QColor( 0, 0, 0 ), QColor( 255, 255, 255 ), 1, 1, 0, false, {}, &missingImage );
  QVERIFY( missingImage );
  content = cache.svgContent( u"bbbbbbb"_s, 14, QColor( 0, 0, 0 ), QColor( 255, 255, 255 ), 1, 1, 0, false, {}, &missingImage );
  QVERIFY( missingImage );
  content = cache.svgContent( TEST_DATA_DIR + u"/sample_svg.svg"_s, 14, QColor( 0, 0, 0 ), QColor( 255, 255, 255 ), 1, 1, 0, false, {}, &missingImage );
  QVERIFY( !missingImage );
}

struct RenderPictureWrapper
{
    QgsSvgCache &cache;
    QString svgPath;
    double size = 100;
    explicit RenderPictureWrapper( QgsSvgCache &cache, const QString &svgPath )
      : cache( cache )
      , svgPath( svgPath )
    {}
    void operator()( int )
    {
      const QPicture pic = cache.svgAsPicture( svgPath, size, QColor( 255, 0, 0 ), QColor( 0, 255, 0 ), 1, 1, true );
      const QSize imageSize = pic.boundingRect().size();
      QImage image( imageSize, QImage::Format_ARGB32_Premultiplied );
      image.fill( 0 ); // transparent background
      QPainter p( &image );
      p.drawPicture( 0, 0, pic );
    }
};

void TestQgsSvgCache::threadSafePicture()
{
  // QPicture playback is NOT thread safe with implicitly shared copies - this
  // unit test checks that concurrent drawing of svg as QPicture from QgsSvgCache
  // returns a detached copy which is safe to use across threads

  // refs:
  // https://github.com/qgis/QGIS/issues/24976
  // https://github.com/qgis/QGIS/issues/24988

  QgsSvgCache cache;
  const QString svgPath = TEST_DATA_DIR + u"/sample_svg.svg"_s;

  // smash picture rendering over multiple threads
  QVector<int> list;
  list.resize( 100 );
  QtConcurrent::blockingMap( list, RenderPictureWrapper( cache, svgPath ) );
}


struct RenderImageWrapper
{
    QgsSvgCache &cache;
    QString svgPath;
    double size = 100;
    explicit RenderImageWrapper( QgsSvgCache &cache, const QString &svgPath )
      : cache( cache )
      , svgPath( svgPath )
    {}
    void operator()( int )
    {
      bool fitsInCache = false;
      const QImage cachedImage = cache.svgAsImage( svgPath, size, QColor( 255, 0, 0 ), QColor( 0, 255, 0 ), 1, 1, fitsInCache );
      QImage image( cachedImage.size(), QImage::Format_ARGB32_Premultiplied );
      image.fill( 0 ); // transparent background
      QPainter p( &image );
      p.drawImage( 0, 0, cachedImage );
    }
};

void TestQgsSvgCache::threadSafeImage()
{
  // This unit test checks that concurrent rendering of svg as QImage from QgsSvgCache
  // works without issues across threads

  QgsSvgCache cache;
  const QString svgPath = TEST_DATA_DIR + u"/sample_svg.svg"_s;

  // smash image rendering over multiple threads
  QVector<int> list;
  list.resize( 100 );
  QtConcurrent::blockingMap( list, RenderImageWrapper( cache, svgPath ) );
}

void TestQgsSvgCache::changeImage()
{
  bool inCache;
  QgsSvgCache cache;
  // no minimum time between checks
  cache.mFileModifiedCheckTimeout = 0;

  //copy an image to the temp folder
  const QString tempImagePath = QDir::tempPath() + "/svg_cache.svg";

  const QString originalImage = TEST_DATA_DIR + u"/test_symbol_svg.svg"_s;
  if ( QFileInfo::exists( tempImagePath ) )
    QFile::remove( tempImagePath );
  QFile::copy( originalImage, tempImagePath );

  //render it through the cache
  QImage img = cache.svgAsImage( tempImagePath, 200, QColor( 0, 0, 0 ), QColor( 0, 0, 0 ), 1.0, 1.0, inCache );
  QVERIFY( imageCheck( "svgcache_changed_before", img, 30 ) );

  // wait a second so that modified time is different
  QElapsedTimer t;
  t.start();
  while ( !t.hasExpired( 1000 ) )
  {
  }

  //replace the image in the temp folder
  const QString newImage = TEST_DATA_DIR + u"/test_symbol_svg2.svg"_s;
  QFile::remove( tempImagePath );
  QFile::copy( newImage, tempImagePath );

  //re-render it
  img = cache.svgAsImage( tempImagePath, 200, QColor( 0, 0, 0 ), QColor( 0, 0, 0 ), 1.0, 1.0, inCache );
  QVERIFY( imageCheck( "svgcache_changed_after", img, 30 ) );

  // repeat, with minimum time between checks
  QgsSvgCache cache2;
  QFile::remove( tempImagePath );
  QFile::copy( originalImage, tempImagePath );
  img = cache2.svgAsImage( tempImagePath, 200, QColor( 0, 0, 0 ), QColor( 0, 0, 0 ), 1.0, 1.0, inCache );
  QVERIFY( imageCheck( "svgcache_changed_before", img, 30 ) );

  // wait a second so that modified time is different
  t.restart();
  while ( !t.hasExpired( 1000 ) )
  {
  }

  //replace the image in the temp folder
  QFile::remove( tempImagePath );
  QFile::copy( newImage, tempImagePath );

  //re-render it - not enough time has elapsed between checks, so file modification time will NOT be rechecked and
  // existing cached image should be used
  img = cache2.svgAsImage( tempImagePath, 200, QColor( 0, 0, 0 ), QColor( 0, 0, 0 ), 1.0, 1.0, inCache );
  QVERIFY( imageCheck( "svgcache_changed_before", img, 30 ) );
}

void TestQgsSvgCache::base64()
{
  // test rendering svgs encoded in base 64
  QgsSvgCache cache;
  bool inCache = false;

  // invalid base64 strings
  QImage img = cache.svgAsImage( u"base64:"_s, 200, QColor( 0, 0, 0 ), QColor( 0, 0, 0 ), 1.0, 1.0, inCache );
  QVERIFY( imageCheck( u"null_image"_s, img, 0 ) );

  img = cache.svgAsImage( u"base64:zzzzzzzzzzzzzzzzzzzz"_s, 200, QColor( 0, 0, 0 ), QColor( 0, 0, 0 ), 1.0, 1.0, inCache );
  QVERIFY( imageCheck( u"null_image"_s, img, 0 ) );

  //valid base 64
  img = cache.svgAsImage( u"base64:"_s, 200, QColor( 0, 0, 0 ), QColor( 0, 0, 0 ), 1.0, 1.0, inCache );
  QVERIFY( imageCheck( u"svgcache_base64"_s, img, 30 ) );
}

void TestQgsSvgCache::replaceParams()
{
  QDomDocument doc;
  QDomElement elem = doc.createElement( u"svg"_s );
  elem.setAttribute( u"not_style"_s, u"val"_s );
  elem.setAttribute( u"style"_s, u"font-weight:bold; font-size:12px"_s );

  QgsSvgCache cache;
  cache.replaceElemParams( elem, QColor( 255, 0, 0, 150 ), QColor( 0, 255, 0, 100 ), 0.6, {} );

  // params in styles
  QCOMPARE( elem.attribute( u"not_style"_s ), u"val"_s );
  QCOMPARE( elem.attribute( u"style"_s ), u"font-weight:bold; font-size:12px"_s );

  // with fill color
  elem.setAttribute( u"style"_s, u"font-weight:bold; fill: param(Fill); font-size:12px"_s );
  cache.replaceElemParams( elem, QColor( 255, 0, 0, 150 ), QColor( 0, 255, 0, 100 ), 0.6, {} );
  QCOMPARE( elem.attribute( u"style"_s ), u"font-weight:bold; fill:#ff0000; font-size:12px"_s );
  // with fill opacity
  elem.setAttribute( u"style"_s, u"font-weight:bold; fill: param(Fill);fill-opacity:param(fill-opacity);font-size:12px"_s );
  cache.replaceElemParams( elem, QColor( 255, 0, 0, 25 ), QColor( 0, 255, 0, 100 ), 0.6, {} );
  QCOMPARE( elem.attribute( u"style"_s ), u"font-weight:bold; fill:#ff0000;fill-opacity:0.0980392;font-size:12px"_s );
  // with stroke color
  elem.setAttribute( u"style"_s, u"font-weight:bold; outline: param(Outline);font-size:12px"_s );
  cache.replaceElemParams( elem, QColor( 255, 0, 0, 25 ), QColor( 0, 255, 0, 100 ), 0.6, {} );
  QCOMPARE( elem.attribute( u"style"_s ), u"font-weight:bold; outline:#00ff00;font-size:12px"_s );
  // with stroke opacity
  elem.setAttribute( u"style"_s, u"font-weight:bold; outline: param(Outline);outline-opacity:param(outline-opacity);font-size:12px"_s );
  cache.replaceElemParams( elem, QColor( 255, 0, 0, 25 ), QColor( 0, 255, 0, 100 ), 0.6, {} );
  QCOMPARE( elem.attribute( u"style"_s ), u"font-weight:bold; outline:#00ff00;outline-opacity:0.392157;font-size:12px"_s );
  // with stroke size
  elem.setAttribute( u"style"_s, u"font-weight:bold; outline: param(Outline);outline-opacity:param(outline-opacity);stroke-width: param(outline-width) ;font-size:12px"_s );
  cache.replaceElemParams( elem, QColor( 255, 0, 0, 25 ), QColor( 0, 255, 0, 100 ), 0.6, {} );
  QCOMPARE( elem.attribute( u"style"_s ), u"font-weight:bold; outline:#00ff00;outline-opacity:0.392157;stroke-width:0.6;font-size:12px"_s );

  // params in attributes

  // with fill color
  elem.setAttribute( u"fill"_s, u" param(Fill) "_s );
  cache.replaceElemParams( elem, QColor( 255, 0, 0, 150 ), QColor( 0, 255, 0, 100 ), 0.6, {} );
  QCOMPARE( elem.attribute( u"fill"_s ), u"#ff0000"_s );
  // with fill opacity
  elem.setAttribute( u"fill-opacity"_s, u"param(fill-opacity)"_s );
  cache.replaceElemParams( elem, QColor( 255, 0, 0, 25 ), QColor( 0, 255, 0, 100 ), 0.6, {} );
  QCOMPARE( elem.attribute( u"fill-opacity"_s ).left( 6 ), u"0.0980"_s );
  // with stroke color
  elem.setAttribute( u"stroke"_s, u" param(Outline) "_s );
  cache.replaceElemParams( elem, QColor( 255, 0, 0, 25 ), QColor( 0, 255, 0, 100 ), 0.6, {} );
  QCOMPARE( elem.attribute( u"stroke"_s ), u"#00ff00"_s );
  // with stroke opacity
  elem.setAttribute( u"stroke-opacity"_s, u"param(outline-opacity)"_s );
  cache.replaceElemParams( elem, QColor( 255, 0, 0, 25 ), QColor( 0, 255, 0, 100 ), 0.6, {} );
  QCOMPARE( elem.attribute( u"stroke-opacity"_s ).left( 6 ), u"0.3921"_s );
  // with stroke size
  elem.setAttribute( u"stroke-size"_s, u"param(outline-width) "_s );
  cache.replaceElemParams( elem, QColor( 255, 0, 0, 25 ), QColor( 0, 255, 0, 100 ), 0.6, {} );
  QCOMPARE( elem.attribute( u"stroke-size"_s ), u"0.6"_s );
}

void TestQgsSvgCache::dynamicSvg()
{
  // test rendering SVGs with manual aspect ratio
  QgsSvgCache cache;
  const QString dynamicImage = TEST_DATA_DIR + u"/svg/test_dynamic_svg.svg"_s;
  const QString svg = cache.svgContent( dynamicImage, 200, QColor( 0, 0, 0 ), QColor( 0, 0, 0 ), 1.0, 1.0, 0, false, { { "text1", "green?" }, { "text2", "supergreen" }, { "align", "middle" } } );

  QDomDocument doc;
  QVERIFY( doc.setContent( svg ) );

  // because Qt ordering of QDomElement attributes are random, we can't directly compare the XML string
  // and instead have to check manually element by element...
  const QDomElement svgElement = doc.firstChildElement( u"svg"_s );
  QVERIFY( !svgElement.isNull() );
  QCOMPARE( svgElement.attribute( u"width"_s ), u"30mm"_s );
  QCOMPARE( svgElement.attribute( u"height"_s ), u"30mm"_s );
  QCOMPARE( svgElement.attribute( u"version"_s ), u"1.1"_s );
  QCOMPARE( svgElement.attribute( u"viewBox"_s ), u"0 0 32.75 32.75"_s );
  QCOMPARE( svgElement.attribute( u"xmlns"_s ), u"http://www.w3.org/2000/svg"_s );

  const QDomElement gElement = svgElement.firstChildElement( u"g"_s );
  QVERIFY( !gElement.isNull() );
  QCOMPARE( gElement.attribute( u"transform"_s ), u"translate(-81.521 -137.75)"_s );

  const QDomElement g2Element = gElement.firstChildElement( u"g"_s );
  QVERIFY( !g2Element.isNull() );
  QCOMPARE( g2Element.attribute( u"fill"_s ), u"#FFFFFF"_s );
  QCOMPARE( g2Element.attribute( u"fill-opacity"_s ), u"0.7"_s );
  QCOMPARE( g2Element.attribute( u"stroke"_s ), u"#000000"_s );

  const QDomElement circleElement = g2Element.firstChildElement( u"circle"_s );
  QVERIFY( !circleElement.isNull() );
  QCOMPARE( circleElement.attribute( u"cx"_s ), u"97.896"_s );
  QCOMPARE( circleElement.attribute( u"cy"_s ), u"154.12"_s );
  QCOMPARE( circleElement.attribute( u"r"_s ), u"15.875"_s );

  const QDomElement path1Element = g2Element.firstChildElement( u"path"_s );
  QVERIFY( !path1Element.isNull() );
  QCOMPARE( path1Element.attribute( u"d"_s ), u"m82.815 148.83h30.162"_s );
  QCOMPARE( path1Element.attribute( u"stroke-width"_s ), u".26458px"_s );

  const QDomElement path2Element = path1Element.nextSiblingElement( u"path"_s );
  QVERIFY( !path2Element.isNull() );
  QCOMPARE( path2Element.attribute( u"d"_s ), u"m82.815 159.42h30.162"_s );
  QCOMPARE( path2Element.attribute( u"stroke-width"_s ), u".25px"_s );

  const QDomElement g3Element = g2Element.nextSiblingElement( u"g"_s );
  QVERIFY( !g3Element.isNull() );
  QCOMPARE( g3Element.attribute( u"stroke-width"_s ), u".265"_s );
  QCOMPARE( g3Element.attribute( u"text-anchor"_s ), u"middle"_s );
  QCOMPARE( g3Element.attribute( u"alignment-baseline"_s ), u"middle"_s );

  const QDomElement text1Element = g3Element.firstChildElement( u"text"_s );
  QVERIFY( !text1Element.isNull() );
  QCOMPARE( text1Element.attribute( u"x"_s ), u"98"_s );
  QCOMPARE( text1Element.attribute( u"y"_s ), u"147.5"_s );
  QCOMPARE( text1Element.attribute( u"font-size"_s ), u"6px"_s );
  QCOMPARE( text1Element.attribute( u"font-family"_s ), u"QGIS Vera Sans"_s );
  QCOMPARE( text1Element.attribute( u"font-weight"_s ), u"bold"_s );
  QCOMPARE( text1Element.text(), u"green?"_s );

  const QDomElement text2Element = text1Element.nextSiblingElement( u"text"_s );
  QVERIFY( !text2Element.isNull() );
  QCOMPARE( text2Element.attribute( u"x"_s ), u"98"_s );
  QCOMPARE( text2Element.attribute( u"y"_s ), u"156.3"_s );
  QCOMPARE( text2Element.attribute( u"font-size"_s ), u"4.5px"_s );
  QCOMPARE( text2Element.attribute( u"font-family"_s ), u"QGIS Vera Sans"_s );
  QCOMPARE( text2Element.attribute( u"font-weight"_s ), u"bold"_s );
  QCOMPARE( text2Element.text(), u"supergreen"_s );
}

void TestQgsSvgCache::aspectRatio()
{
  // test rendering SVGs with manual aspect ratio
  QgsSvgCache cache;
  bool inCache = false;

  const QString originalImage = TEST_DATA_DIR + u"/test_symbol_svg.svg"_s;
  QImage img = cache.svgAsImage( originalImage, 200, QColor( 0, 0, 0 ), QColor( 0, 0, 0 ), 1.0, 1.0, inCache, 0.5 );
  QVERIFY( imageCheck( u"svgcache_aspect_ratio"_s, img, 30 ) );
}

void TestQgsSvgCache::noViewBox()
{
  // if a source SVG has no viewbox but it does have width/height, use that as a backup so that
  // we can correctly determine the svg's aspect ratio
  const QString originalImage = TEST_DATA_DIR + u"/svg/no_viewbox.svg"_s;
  QgsSvgCache cache;
  const double size = 12;
  const QColor fill = QColor( 0, 0, 0 );
  const QColor stroke = QColor( 0, 0, 0 );
  const double strokeWidth = 1;
  const double widthScaleFactor = 1;
  const QSizeF viewBoxSize = cache.svgViewboxSize( originalImage, size, fill, stroke, strokeWidth, widthScaleFactor );
  QGSCOMPARENEAR( viewBoxSize.width(), 1.329267, 0.0001 );
  QGSCOMPARENEAR( viewBoxSize.height(), 6.358467, 0.0001 );
}

bool TestQgsSvgCache::imageCheck( const QString &testName, QImage &image, int mismatchCount )
{
  //draw background
  QImage imageWithBackground( image.width(), image.height(), QImage::Format_RGB32 );
  QgsRenderChecker::drawBackground( &imageWithBackground );
  QPainter painter( &imageWithBackground );
  painter.drawImage( 0, 0, image );
  painter.end();

  const QString tempDir = QDir::tempPath() + '/';
  const QString fileName = tempDir + testName + ".png";
  imageWithBackground.save( fileName, "PNG" );
  QgsRenderChecker checker;
  checker.setControlName( "expected_" + testName );
  checker.setRenderedImage( fileName );
  checker.setColorTolerance( 2 );
  const bool resultFlag = checker.compareImages( testName, mismatchCount );
  mReport += checker.report();
  return resultFlag;
}

QGSTEST_MAIN( TestQgsSvgCache )
#include "testqgssvgcache.moc"
