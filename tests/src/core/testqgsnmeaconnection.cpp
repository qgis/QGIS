...
class ReplayNmeaConnection : public QgsNmeaConnection
{
  ...
  public:
    ...
    {
      const QString messageTerminated = message + u"\r\n"_s;
...
void TestQgsNmeaConnection::testBasic()
{
  ...
  QgsGpsInformation info = connection.push( u"$%1GSV,3,1,12,07,41,181,26,05,34,292,30,16,34,060,36,26,24,031,24*7B"_s.arg( talkerId ) );
  ...
  QCOMPARE( info.qualityDescription(), u"Unknown (-1)"_s );
  QCOMPARE( info.latitude, 0 );
  QCOMPARE( info.longitude, 0 );
  QCOMPARE( info.elevation, 0 );
  QVERIFY( std::isnan( info.direction ) );

  info = connection.push( u"$%1GSV,3,2,12,02,61,115,,21,53,099,,03,51,111,,19,32,276,*72"_s.arg( talkerId ) );
  QVERIFY( info.isValid() );
  QVERIFY( !info.satInfoComplete );
  QCOMPARE( info.bestFixStatus( constellation ), Qgis::GpsFixStatus::NoData );
  QCOMPARE( info.qualityDescription(), u"Unknown (-1)"_s );
  ...
  info = connection.push( u"$%1GSV,3,3,12,17,31,279,,28,27,320,,23,23,026,,14,22,060,*7B"_s.arg( talkerId ) );
  QVERIFY( info.isValid() );
  QVERIFY( !info.satInfoComplete );
  QCOMPARE( info.bestFixStatus( constellation ), Qgis::GpsFixStatus::NoData );
  QCOMPARE( info.qualityDescription(), u"Unknown (-1)"_s );
  ...
  info = connection.push( u"$%1GGA,084112.185,6938.6532,N,01856.8526,E,1,04,1.4,35.0,M,29.4,M,,0000*63"_s.arg( talkerId ) );
  QVERIFY( info.isValid() );
  QVERIFY( info.satInfoComplete );
  QCOMPARE( info.bestFixStatus( constellation ), Qgis::GpsFixStatus::NoData );
  QCOMPARE( info.qualityDescription(), u"Autonomous"_s );
  ...
  info = connection.push( u"$%1RMC,084111.185,A,6938.6531,N,01856.8527,E,0.16,2.00,220120,,,A*6E"_s.arg( talkerId ) );
  QVERIFY( info.isValid() );
  QVERIFY( info.satInfoComplete );
  QCOMPARE( info.bestFixStatus( constellation ), Qgis::GpsFixStatus::NoData );
  QCOMPARE( info.qualityDescription(), u"Autonomous"_s );
  ...
  info = connection.push( u"$%1GSA,A,3,07,05,16,26,,,,,,,,,3.4,1.4,3.1*33"_s.arg( talkerId ) );
  QVERIFY( info.isValid() );
  QVERIFY( !info.satInfoComplete );
  ...
  info = connection.push( u"$%1RMC,084112.185,A,6938.6532,N,01856.8526,E,0.08,2.00,220120,,,A*60"_s.arg( talkerId ) );
  QVERIFY( info.isValid() );
  QVERIFY( info.satInfoComplete );
  QCOMPARE( info.bestFixStatus( constellation ), Qgis::GpsFixStatus::Fix3D );
  QCOMPARE( info.qualityDescription(), u"Autonomous"_s );
  ...
  info = connection.push( u"$%1GGA,084113.185,6938.6532,N,01856.8526,E,1,04,1.4,34.0,M,29.4,M,,0000*63"_s.arg( talkerId ) );
  QVERIFY( info.isValid() );
  QVERIFY( info.satInfoComplete );
  QCOMPARE( info.bestFixStatus( constellation ), Qgis::GpsFixStatus::Fix3D );
  QCOMPARE( info.qualityDescription(), u"Autonomous"_s );
  ...
  info = connection.push( u"$%1RMC,084113.185,A,6938.6532,N,01856.8526,E,0.05,2.00,220120,,,A*6C"_s.arg( talkerId ) );
  QVERIFY( info.isValid() );
  QVERIFY( info.satInfoComplete );
  QCOMPARE( info.bestFixStatus( constellation ), Qgis::GpsFixStatus::Fix3D );
  QCOMPARE( info.qualityDescription(), u"Autonomous"_s );
...
void TestQgsNmeaConnection::testVtg()
{
  ...
  QgsGpsInformation info = connection.push( u"$%1VTG,,T,,M,0.003,N,0.005,K,D*20"_s.arg( talkerId ) );
  QVERIFY( info.isValid() );
  QVERIFY( std::isnan( info.direction ) );
  QCOMPARE( info.speed, 0.005 );
  info = connection.push( u"$%1VTG,224.592,T,224.492,M,0.003,N,0.006,K,D*20"_s.arg( talkerId ) );
  QVERIFY( info.isValid() );
  // must be direction to true north, not magnetic north
  QCOMPARE( info.direction, 224.592 );
  QCOMPARE( info.speed, 0.006 );
  info = connection.push( u"$%1VTG,139.969,T,139.969,M,0.007,N,0.013,K,D*3D"_s.arg( talkerId ) );
  QVERIFY( info.isValid() );
  QCOMPARE( info.direction, 139.969 );
  QCOMPARE( info.speed, 0.013 );
  // direction should not be overwritten with nan
  info = connection.push( u"$%1VTG,,T,,M,0.003,N,0.005,K,D*20"_s.arg( talkerId ) );
  QVERIFY( info.isValid() );
  QCOMPARE( info.direction, 139.969 );
  QCOMPARE( info.speed, 0.005 );
}
...
void TestQgsNmeaConnection::testFixStatus()
{
  ...
  QgsGpsInformation info = connection.push( u"$%1RMC,220516,V,5133.82,N,00042.24,W,173.8,231.8,130694,004.2,W*70"_s.arg( talkerId ) );
  QVERIFY( !info.isValid() );
  Qgis::GnssConstellation constellation = Qgis::GnssConstellation::Unknown;
  QCOMPARE( info.bestFixStatus( constellation ), Qgis::GpsFixStatus::NoData );
  QCOMPARE( constellation, Qgis::GnssConstellation::Unknown );
  ...
  connection.push( u"$%1RMC,220516,V,5133.82,N,00042.24,W,173.8,231.8,130694,004.2,W*70"_s.arg( talkerId ) );
  QCOMPARE( statusSpy.count(), 0 );

  // simulate 3d fix
  connection.push( u"$%1GSA,A,3,,,,,,16,18,,22,24,,,3.6,2.1,2.2*3C"_s.arg( talkerId ) );
  info = connection.push( u"$%1RMC,220516,A,5133.82,N,00042.24,W,173.8,231.8,130694,004.2,W*70"_s.arg( talkerId ) );
  QVERIFY( info.isValid() );
  QCOMPARE( info.bestFixStatus( constellation ), Qgis::GpsFixStatus::Fix3D );
  ...
  // simulate 2d fix
  connection.push( u"$%1GSA,A,2,,,,,,16,18,,22,24,,,3.6,2.1,2.2*3C"_s.arg( talkerId ) );
  info = connection.push( u"$%1RMC,220516,A,5133.82,N,00042.24,W,173.8,231.8,130694,004.2,W*70"_s.arg( talkerId ) );
  QVERIFY( info.isValid() );
  ...
  connection.push( u"$%1GSA,A,1,,,,,,16,18,,22,24,,,3.6,2.1,2.2*3C"_s.arg( talkerId ) );
  info = connection.push( u"$%1RMC,220516,A,5133.82,N,00042.24,W,173.8,231.8,130694,004.2,W*70"_s.arg( talkerId ) );
  QVERIFY( !info.isValid() );
  ...
  info = connection.push( u"$%1RMC,220516,A,9933.82,N,00042.24,W,173.8,231.8,130694,004.2,W*70"_s.arg( talkerId ) );
  ...
  info = connection.push( u"$%1RMC,220516,A,9933.82,S,00042.24,W,173.8,231.8,130694,004.2,W*70"_s.arg( talkerId ) );
  QGSCOMPARENEAR( info.latitude, -99.563666, 0.00001 );
  QGSCOMPARENEAR( info.longitude, -0.70400000, 0.00001 );
  QVERIFY( !info.isValid() );

  // longitude 192 degrees => out of range
  info = connection.push( u"$%1RMC,220516,A,1933.82,N,19192.24,W,173.8,231.8,130694,004.2,W*70"_s.arg( talkerId ) );
  QGSCOMPARENEAR( info.latitude, 19.5636666667, 0.00001 );
  QGSCOMPARENEAR( info.longitude, -192.5373333333, 0.00001 );
  QVERIFY( !info.isValid() );

  info = connection.push( u"$%1RMC,220516,A,1933.82,N,19192.24,E,173.8,231.8,130694,004.2,W*70"_s.arg( talkerId ) );
  QGSCOMPARENEAR( info.latitude, 19.5636666667, 0.00001 );
  QGSCOMPARENEAR( info.longitude, 192.5373333333, 0.00001 );
  QVERIFY( !info.isValid() );
...
void TestQgsNmeaConnection::testFixStatusAcrossConstellations()
{
  ...
  QgsGpsInformation info = connection.push( u"$GPGSA,A,1,7,9,16,27,30,9,7,1,6,5,,,4.2,3.4,2.4*07"_s );
  ...
  info = connection.push( u"$GLGSA,A,1,7,9,16,27,30,9,7,1,6,5,,,4.2,3.4,2.4*07"_s );
  QCOMPARE( info.bestFixStatus( bestConstellation ), Qgis::GpsFixStatus::NoFix );
  QCOMPARE( statusSpy.count(), 1 );
  QCOMPARE( statusSpy.constLast().at( 0 ).value<Qgis::GpsFixStatus>(), Qgis::GpsFixStatus::NoFix );
  QCOMPARE( bestConstellation, Qgis::GnssConstellation::Gps );

  info = connection.push( u"$GLGSA,A,2,7,9,16,27,30,9,7,1,6,5,,,4.2,3.4,2.4*07\r\n$GPGSA,A,1,7,9,16,27,30,9,7,1,6,5,,,4.2,3.4,2.4*07"_s );
  ...
  info = connection.push( u"$GNGSA,A,3,7,9,16,27,30,9,7,1,6,5,,,4.2,3.4,2.4*07"_s );
  ...
  info = connection.push( u"$GNGSA,A,1,7,9,16,27,30,9,7,1,6,5,,,4.2,3.4,2.4*07"_s );
  QCOMPARE( info.bestFixStatus( bestConstellation ), Qgis::GpsFixStatus::Fix2D );
  QCOMPARE( statusSpy.count(), 4 );
  QCOMPARE( statusSpy.constLast().at( 0 ).value<Qgis::GpsFixStatus>(), Qgis::GpsFixStatus::Fix2D );
  QCOMPARE( bestConstellation, Qgis::GnssConstellation::Glonass );
  ...
  info = connection.push( u"$GNGSA,A,3,6,11,12,19,20,25,29,10,20,19,7,10,7.5,5.5,5.1*24\r\n$GNGSA,A,3,11,12,24,25,31,,,,,,,,7.5,5.5,5.1*2A\r\n$GNGSA,A,1,194,195,,,,,,,,,,,7.5,5.5,5.1*29"_s );
  QCOMPARE( info.bestFixStatus( bestConstellation ), Qgis::GpsFixStatus::Fix3D );
  QVERIFY( info.isValid() );
  QCOMPARE( statusSpy.count(), 5 );
  QCOMPARE( statusSpy.constLast().at( 0 ).value<Qgis::GpsFixStatus>(), Qgis::GpsFixStatus::Fix3D );
...
void TestQgsNmeaConnection::testConstellation()
{
  ...
  QgsGpsInformation info = connection.push( u"$GPGSV,8,5,30,06,33,224,00,08,19,298,00,09,21,234,00,13,18,286,00*7E"_s );
  ...
  info = connection.push( u"$GLGSV,8,5,30,06,33,224,00,08,19,298,00,09,21,234,00,13,18,286,00*7E"_s );
  ...
  info = connection.push( u"$GAGSV,8,5,30,31,33,224,00,08,19,298,00,09,21,234,00,13,18,286,00*7E"_s );
  ...
  info = connection.push( u"$GQGSV,8,5,30,7,33,224,00,08,19,298,00,09,21,234,00,13,18,286,00*7E"_s );
  ...
  info = connection.push( u"$GBGSV,8,5,30,64,33,224,00,08,19,298,00,09,21,234,00,13,18,286,00*7E"_s );
...
void TestQgsNmeaConnection::testPosition()
{
  ...
  connection.push( u"$%1GGA,084112.185,6900.0,N,01800.0,E,1,04,1.4,35.0,M,29.4,M,,0000*63"_s.arg( talkerId ) );
  ...
  connection.push( u"$%1GGA,084112.185,6900.0,N,01800.0,E,1,04,1.4,35.0,M,29.6,M,,0000*63"_s.arg( talkerId ) );
  QCOMPARE( spy.count(), 1 );

  // new location
  connection.push( u"$%1GGA,084112.185,6900.0,N,01900.0,E,1,04,1.4,35.0,M,29.4,M,,0000*63"_s.arg( talkerId ) );
  ...
  connection.push( u"$%1GGA,084112.185,9900.0,N,01900.0,E,1,04,1.4,35.0,M,29.4,M,,0000*63"_s.arg( talkerId ) );
...
void TestQgsNmeaConnection::testComponent()
{
  ...
  QgsGpsInformation info = connection.push( u"$%1GGA,084112.185,6900.0,N,01800.0,E,1,04,1.4,35.0,M,29.4,M,,0000*63"_s.arg( talkerId ) );
...
void TestQgsNmeaConnection::testIncompleteMessage()
{
  ...
  connection.pushString( u"$GPGGA,"_s );
  ...
  connection.pushString( u"084112.185,6900.0,N,01800.0,E,1,04,1.4,35.0,M,29.4,M,,0000*63\r\n"_s );
