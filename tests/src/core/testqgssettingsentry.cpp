...
void TestQgsSettingsEntry::enumValue()
{
  ...
  QgsSettingsEntryBase::userSettings().remove( fullSettingsKey );
...
void TestQgsSettingsEntry::flagValue()
{
  ...
  // Check get value
  QgsSettingsEntryBase::userSettings().setValue( fullSettingsKey, u"PointLayer|LineLayer"_s );
  ...
  // auto conversion of old settings (int to str)
  QSettings().setValue( fullSettingsKey, static_cast<int>( pointAndPolygon ) );
  QCOMPARE( settingsEntryFlag.valueAsVariant().toInt(), pointAndPolygon );
  QCOMPARE( settingsEntryFlag.value(), pointAndPolygon );

  // Explicit zero flags must round-trip as zero instead of falling back to the default.
  QgsSettings settings;
  settings.remove( fullSettingsKey );
  settings.setFlagValue( fullSettingsKey, Qgis::LayerFilters() );
  QCOMPARE( settings.value( fullSettingsKey ).toString(), u"0"_s );
  QCOMPARE( settings.flagValue( fullSettingsKey, pointAndLine ), Qgis::LayerFilters() );
}
