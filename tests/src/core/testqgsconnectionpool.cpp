...
void TestQgsConnectionPool::layersFromSameDatasetGPX()
{
  ...
  {
    const QgsPointV2* geom = dynamic_cast<const QgsPointV2*>( layer1Features[i].constGeometry()->geometry() );
  ...
  {
    const QgsLineStringV2* geom = dynamic_cast<const QgsLineStringV2*>( layer2Features[i].constGeometry()->geometry() );
    QVERIFY( geom != nullptr );
