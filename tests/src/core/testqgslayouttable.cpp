...
class TestQgsLayoutTable : public QgsTest
{
  ...
  public:
    TestQgsLayoutTable() : QgsTest( QStringLiteral( "Layout Table Tests" ), QStringLiteral( "composer_table" ) ) {}
...
void TestQgsLayoutTable::testDataDefinedTextFormatForCell()
{
  ...
  textFormat.dataDefinedProperties().setProperty( static_cast< int >( QgsPalLayerSettings::Property::Size ), QgsProperty::fromExpression( QStringLiteral( "if(@column_number = 1,35,15)" ) ) );
...
void TestQgsLayoutTable::dataDefinedSource()
{
  ...
  table->dataDefinedProperties().setProperty( static_cast< int >( QgsLayoutObject::DataDefinedProperty::AttributeTableSourceLayer ), layer1->id() );
  table->refresh();
  QCOMPARE( table->contents().length(), 1 );
  QCOMPARE( table->contents().at( 0 ), QVector< QVariant >() << 1 << 2 << 3 );

  // by layer name
  table->dataDefinedProperties().setProperty( static_cast< int >( QgsLayoutObject::DataDefinedProperty::AttributeTableSourceLayer ), QStringLiteral( "l2" ) );
  table->refresh();
  QCOMPARE( table->contents().length(), 1 );
  QCOMPARE( table->contents().at( 0 ), QVector< QVariant >() << 11 << 12 << 13 );

  // by layer name (case insensitive)
  table->dataDefinedProperties().setProperty( static_cast< int >( QgsLayoutObject::DataDefinedProperty::AttributeTableSourceLayer ), QStringLiteral( "L3" ) );
  table->refresh();
  QCOMPARE( table->contents().length(), 1 );
