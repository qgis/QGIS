#!/usr/bin/env bash
# Incremental GeoAI Desktop app build (assumes qgis_core libs already exist).
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
BUILD="${GEOAI_BUILD_DIR:-${ROOT}/build}"
JOBS="${GEOAI_BUILD_JOBS:-$(sysctl -n hw.ncpu)}"
...
echo "Build finished. Launch with: ${ROOT}/scripts/run-geoai-dev.sh"
