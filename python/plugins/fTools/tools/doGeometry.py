...
class GeometryDialog(QDialog, Ui_Dialog):
    ...
    def __init__(self, iface, function):
        ...
        if self.myFunction == 1:
            QObject.connect(
                self.inShape, SIGNAL("currentIndexChanged( QString )"), self.update)
        elif self.myFunction == 5:
            QObject.connect(
                self.chkWriteShapefile, SIGNAL("stateChanged( int )"), self.updateGui)
            self.updateGui()
        self.manageGui()
        self.success = False
    ...
    def manageGui(self):
        ...
        if self.myFunction == 1:  # Singleparts to multipart
            ...
            if self.myFunction == 8:  # Delaunay triangulation
                ...
                self.label.setVisible(False)
                self.lineEdit.setVisible(False)
            elif self.myFunction == 12:  # Minimum Bounding Box
                self.setWindowTitle(self.tr("Minimum Bounding Box from Layer"))
                ...
                self.chkByFeatures.setVisible(True)
                self.chkUseSelection.setVisible(True)
            self.lblOutputShapefile.setText(
                self.tr("Output polygon shapefile"))
        self.resize(381, 100)
        self.populateLayers()
...
# 12: Minimum Bounding Box from layer
    ...
    def geometry(self, myLayer, myParam, myField):
        ...
        QObject.connect(self.testThread, SIGNAL(
            "runFinished( PyQt_PyObject )"), self.runFinishedFromThread)
        QObject.connect(self.testThread, SIGNAL(
            "runStatus( PyQt_PyObject )"), self.runStatusFromThread)
        QObject.connect(self.testThread, SIGNAL(
            "runRange( PyQt_PyObject )"), self.runRangeFromThread)
        self.cancel_close.setText(self.tr("Cancel"))
        QObject.connect(
            self.cancel_close, SIGNAL("clicked()"), self.cancelThread)
    ...
    def runFinishedFromThread(self, success):
        ...
        if success == "math_error":
            ...
            if success:
                if (self.myFunction == 5 and self.chkWriteShapefile.isChecked()) or self.myFunction != 5:
                    if self.addToCanvasCheck.isChecked():
                        ...
                        if not addCanvasCheck:
                            QMessageBox.warning(self, self.tr("Geometry"), self.tr(
                                "Error loading output shapefile:\n%s") % (unicode(self.shapefileName)))
                        self.populateLayers()
                    else:
                        QMessageBox.information(self, self.tr("Geometry"), self.tr(
                ...
                QMessageBox.warning(
                    self, self.tr("Geometry"), self.tr("Error writing output shapefile."))
...
class geometryThread(QThread):
    ...
    def run(self):
        ...
        elif self.myFunction == 12:  # Minimum Bounding Box from Layer
            if self.byFeatures:
                success = self.feature_minBBox()
            else:
                success = self.layer_minBBox()
    ...
    def single_to_multi(self):
        ...
        if not len(unique) == self.vlayer.featureCount() or merge_all:
            for i in unique:
                ...
                # TODO: Make this optional (opt-out to keep it easy for
                # beginners)
    ...
    def export_geometry_info(self):
        ...
        if self.myCalcType == 2:
            ...
            mapCRS = self.parent.iface.mapCanvas(
            ).mapRenderer().destinationCrs()
        ...
        self.emit(SIGNAL("runStatus( PyQt_PyObject )"), 0)
        self.emit(
            SIGNAL("runRange( PyQt_PyObject )"), (0, vprovider.featureCount()))

        (fields, index1, index2) = self.checkMeasurementFields(
            self.vlayer, not self.writeShape)
        ...
        while fit.nextFeature(inFeat):
            ...
            (attr1, attr2) = self.simpleMeasure(
                inGeom, self.myCalcType, ellips, crs)
    ...
    def delaunay_triangulation(self):
        ...
        for triangle in triangles:
            ...
            for index in indicies:
                vprovider.getFeatures(
                    QgsFeatureRequest().setFilterFid(ptDict[ids[index]])).nextFeature(inFeat)
                geom = QgsGeometry(inFeat.geometry())
    ...
    def voronoi_polygons(self):
        ...
        ids = [pts.index(item) for item in uniqueSet]
        sl = voronoi.SiteList(
            [voronoi.Site(i[0], i[1], sitenum=j) for j, i in enumerate(uniqueSet)])
        voronoi.voronoi(sl, c)
        inFeat = QgsFeature()
        ...
        for site, edges in c.polygons.iteritems():
            vprovider.getFeatures(
                QgsFeatureRequest().setFilterFid(ptDict[ids[site]])).nextFeature(inFeat)
    ...
    def clip_voronoi(self, edges, c, width, height, extent, exX, exY):
        ...
        for edge in edges:
            if edge[1] >= 0 and edge[2] >= 0:       # two vertices
                [x1, y1, x2, y2] = clip_line(c.vertices[edge[1]][0], c.vertices[edge[1]][
                                             1], c.vertices[edge[2]][0], c.vertices[edge[2]][1], width, height, exX, exY)
                ...
                if c.lines[edge[0]][1] == 0:      # vertical line
                    ...
                    xtemp = width + exX
                    ytemp = (
                        c.lines[edge[0]][2] - (width + exX) * c.lines[edge[0]][0]) / c.lines[edge[0]][1]
                [x1, y1, x2, y2] = clip_line(c.vertices[edge[1]][0], c.vertices[edge[1]][
                                             1], xtemp, ytemp, width, height, exX, exY)
                ...
                [x1, y1, x2, y2] = clip_line(xtemp, ytemp, c.vertices[edge[2]][0], c.vertices[
                                             edge[2]][1], width, height, exX, exY)
            if x1 or x2 or y1 or y2:
                lines.append(
                    QgsPoint(x1 + extent.xMinimum(), y1 + extent.yMinimum()))
                lines.append(
                    QgsPoint(x2 + extent.xMinimum(), y2 + extent.yMinimum()))
                if 0 - exX in (x1, x2):
                    hasXMin = True
                if 0 - exY in (y1, y2):
                    hasYMin = True
        ...
        if hasXMin:
            if hasYMax:
                lines.append(
                    QgsPoint(extent.xMinimum() - exX, height + extent.yMinimum() + exY))
            if hasYMin:
                lines.append(
                    QgsPoint(extent.xMinimum() - exX, extent.yMinimum() - exY))
        if hasXMax:
            if hasYMax:
                lines.append(
                    QgsPoint(width + extent.xMinimum() + exX, height + extent.yMinimum() + exY))
            if hasYMin:
                lines.append(
                    QgsPoint(width + extent.xMinimum() + exX, extent.yMinimum() - exY))
        return lines
    ...
    def feature_extent(self):
        ...
        if self.useSelection:
            self.emit(SIGNAL("runRange( PyQt_PyObject )"),
                      (0, self.vlayer.selectedFeatureCount()))
    ...
    def layer_minBBox(self):
        ...
        if self.useSelection:
            self.emit(SIGNAL("runRange( PyQt_PyObject )"),
                      (0, self.vlayer.selectedFeatureCount()))
            first = True
            features = self.vlayer.selectedFeatures()
        ...
        geometry, area, angle, width, height = OMBBox(newgeometry)
    ...
    def feature_minBBox(self):
        ...
        if self.useSelection:
            ...
            for inFeat in features:
                self.emit(SIGNAL("runStatus( PyQt_PyObject )"),  nElement)
                nElement += 1
                geometry, area, angle, width, height = OMBBox(inFeat.geometry())
            ...
            while fit.nextFeature(inFeat):
                ...
                geometry, area, angle, width, height = OMBBox(
                    inFeat.geometry())
...
###############
    ...
    def doubleFieldIndex(self, name, desc, fieldList):
        ...
        fieldList.append(
            QgsField(name, QVariant.Double, "double precision", 21, 6, desc))
    ...
    def checkMeasurementFields(self, vlayer, add):
        ...
        if geomType == QGis.Polygon:
            (index1, fieldList) = self.doubleFieldIndex(
                "AREA", self.tr("Polygon area"), fieldList)
            (index2, fieldList) = self.doubleFieldIndex(
                "PERIMETER", self.tr("Polygon perimeter"), fieldList)
        elif geomType == QGis.Line:
            (index1, fieldList) = self.doubleFieldIndex(
                "LENGTH", self.tr("Line length"), fieldList)
            index2 = index1
        else:
            (index1, fieldList) = self.doubleFieldIndex(
                "XCOORD", self.tr("Point x ordinate"), fieldList)
            (index2, fieldList) = self.doubleFieldIndex(
                "YCOORD", self.tr("Point y ordinate"), fieldList)
...
def OMBBox(geom):
    ...
    while i < l - 1:
        x = QgsGeometry(g)
        angle = GetAngleOfLineBetweenTwoPoints(r[i], r[i + 1])
        x.rotate(angle, p0)
        bbox = x.boundingBox()
    ...
    return OMBBox, OMBBox_area, OMBBox_angle, OMBBox_width, OMBBox_heigth
