...
class PerlAlgorithm(GeoAlgorithm):

    PERL_CONSOLE_OUTPUT = 'PERL_CONSOLE_OUTPUT'
    PERLPLOTS = 'PERLPLOTS'

    def getCopy(self):
        newone = PerlAlgorithm(self.descriptionFile)
    ...
    def getIcon(self):
        if self._icon is None:
            self._icon = QgsApplication.getThemeIcon("/providerPerl.svg")
        return self._icon

    def defineCharacteristicsFromScript(self):
        lines = self.script.split('\n')
        self.name, self.i18n_name = self.trAlgorithm('[Unnamed algorithm]')
        self.group, self.i18n_group = self.trAlgorithm('User Perl scripts')
    ...
    def defineCharacteristicsFromFile(self):
        ...
        self.group, self.i18n_group = self.trAlgorithm('User Perl scripts')
        with open(self.descriptionFile, 'perl') as f:
        ...
        while ender < 10:
            if line.startswith('##'):
                ...
                if not self.showConsoleOutput:
                    self.addOutput(OutputHTML(PerlAlgorithm.PERL_CONSOLE_OUTPUT,
                                              self.tr('Perl Console Output')))
    ...
    def checkBeforeOpeningParametersDialog(self):
        msg = PerlUtils.checkRIsInstalled()
        if msg is not None:
            html = self.tr(
