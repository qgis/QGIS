...
class translate(GdalAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.COMPRESSOR = ["JPEG", "LZW", "PACKBITS", "DEFLATE", "LZMA", "ZSTD", "LERC",
                           "LERC_DEFLATE", "LERC_ZSTD", "WEBP", "CCITTRLE", "CCITTFAX3", "CCITTFAX4", "NONE"]
        ...
        self.addParameter(QgsProcessingParameterNumber(self.PREDICTOR,
                                                       self.tr(
                                                           'Sets the LZW, DEFLATE and ZSTD compression.'),
                                                       type=QgsProcessingParameterNumber.Integer,
        ...
        sparse = QgsProcessingParameterBoolean(self.SPARSE,
                                               self.tr(
                                                   'Make output sparse (save space, possibly not supported by other reader)'),
                                               defaultValue=False)
        sparse.setFlags(sparse.flags() |
                        QgsProcessingParameterDefinition.FlagAdvanced)
        self.addParameter(sparse)

        self.addParameter(QgsProcessingParameterNumber(self.QUALITY,
                                                       self.tr(
                                                           'Control Quality/compression, 100 is best quality, 1 is most compression, default is 75'),
                                                       type=QgsProcessingParameterNumber.Integer,
        ...
        tables = QgsProcessingParameterNumber(self.JPEGTABLESMODE,
                                              self.tr(
                                                  'Configure JPEG quantisation'),
                                              type=QgsProcessingParameterNumber.Integer,
                                              defaultValue=0, minValue=0, maxValue=3,
                                              optional=True)
        tables.setFlags(tables.flags() |
                        QgsProcessingParameterDefinition.FlagAdvanced)
    ...
    def getConsoleCommands(self, parameters, context, feedback, executing=True):
        ...
        predictor = self.parameterAsInteger(
            parameters, self.PREDICTOR, context)
