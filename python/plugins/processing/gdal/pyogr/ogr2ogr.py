...
# /* -------------------------------------------------------------------- */
    ...
    while iArg < nArgc:
        if EQUAL(args[iArg], '-f') and iArg < nArgc - 1:
            ...
            iArg = iArg + 1
        elif (EQUAL(args[iArg], '-tg') or EQUAL(args[iArg], '-gt')) and iArg \
            < nArgc - 1:
            ...
            for token in tokens:
                if EQUAL(token, 'Integer') or EQUAL(token, 'Real') \
                    or EQUAL(token, 'String') or EQUAL(token, 'Date') \
                    or EQUAL(token, 'Time') or EQUAL(token, 'DateTime') \
                    or EQUAL(token, 'Binary') or EQUAL(token, 'IntegerList') \
                    or EQUAL(token, 'RealList') or EQUAL(token, 'StringList'):
        ...
        # */
            ...
            if IsNumber(args[iArg + 1]) and iArg < nArgc - 4:
                ...
                iArg = iArg + 4
            elif len(args[iArg + 1]) >= 7 and EQUAL((args[iArg + 1])[0:7],
                    'POLYGON') or len(args[iArg + 1]) >= 12 \
...
'''
    ...
    if bClipSrc and pszClipSrcDS is not None:
        poClipSrc = LoadGeometry(pszClipSrcDS, pszClipSrcSQL, pszClipSrcLayer,
                                 pszClipSrcWhere)
        if poClipSrc is None:
    ...
    if pszClipDstDS is not None:
        poClipDst = LoadGeometry(pszClipDstDS, pszClipDstSQL, pszClipDstLayer,
                                 pszClipDstWhere)
        if poClipDst is None:
...
# /* -------------------------------------------------------------------- */
    ...
    if not bUpdate:
        ...
        if poDriver.TestCapability(ogr.ODrCCreateDataSource) == False:
            print '%s driver does not support data source creation.' \
                % pszFormat
            return False
...
# /* -------------------------------------------------------------------- */
        ...
        if EQUAL(poDriver.GetName(), 'ESRI Shapefile') and pszSQLStatement \
            is None and (len(papszLayers) > 1 or len(papszLayers) == 0
                         and poDS.GetLayerCount() > 1) and pszNewLayerName \
            is None and EQUAL(os.path.splitext(pszDestDataSource)[1], '.SHP'):
...
# /* -------------------------------------------------------------------- */
        ...
        if poODS is None:
            print '%s driver failed to create %s' % (pszFormat,
                    pszDestDataSource)
            return False
...
# /* -------------------------------------------------------------------- */
            ...
            if EQUAL(poDriver.GetName(), 'ESRI Shapefile') and pszNewLayerName \
                is None:
                try:
                    mode = os.stat(pszDestDataSource).st_mode
                    if mode & stat.S_IFDIR == 0:
                        pszNewLayerName = \
                            os.path.splitext(os.path.basename(pszDestDataSource))[0]
                except:
                    pass

            if not TranslateLayer(
                poDS,
                poResultSet,
                poODS,
                papszLCO,
                ...
                print 'Terminating translation prematurely after failed\n' \
...
# /* -------------------------------------------------------------------- */
            ...
            for layername in papszLayers:
                ...
                if poLayer is None:
                    print "FAILURE: Couldn't fetch advertised layer %s!" \
                        % layername
                    return False
        ...
        for iLayer in range(nLayerCount):
            ...
            if bDisplayProgress:
                ...
                pProgressData = ScaledProgressObject(nAccCountFeatures * 1.0
                        / nCountLayersFeatures, (nAccCountFeatures
                        + panLayerCountFeatures[iLayer]) * 1.0
                        / nCountLayersFeatures, progress_func, progress_data)
...
# /* -------------------------------------------------------------------- */
            ...
            if EQUAL(poDriver.GetName(), 'ESRI Shapefile') and nLayerCount \
                == 1 and pszNewLayerName is None:
                try:
                    mode = os.stat(pszDestDataSource).st_mode
                    if mode & stat.S_IFDIR == 0:
                        pszNewLayerName = \
...
def Usage():
    ...
    print 'Usage: ogr2ogr [--help-general] [-skipfailures] [-append] [-update] [-gt n]\n' \
        ...
        + '\n' \
        + ' -f format_name: output file format name, possible values are:'  # // "               [-segmentize max_dist] [-fieldTypeToString All|(type1[,type2]*)]\n" + \
    ...
    print ' -append: Append to existing layer instead of creating new if it exists\n' \
        ...
        + '      StringList. Special value All can be used to convert all fields to strings.'  # //" -segmentize max_dist: maximum distance between 2 nodes.\n" + \
...
def SetZ(poGeom, dfZ):
    ...
    elif eGType == ogr.wkbPolygon or eGType == ogr.wkbMultiPoint or eGType \
        == ogr.wkbMultiLineString or eGType == ogr.wkbMultiPolygon or eGType \
...
# /* -------------------------------------------------------------------- */
    ...
    if poDstLayer is not None:
        ...
        for iLayer in range(nLayerCount):
            ...
            if poLayer is not None and poLayer.GetName() \
                == poDstLayer.GetName():
                break
...
# /* -------------------------------------------------------------------- */
    ...
    if poDstLayer is None:
        ...
        if poDstDS.TestCapability(ogr.ODsCCreateLayer) == False:
            print 'Layer ' + pszNewLayerName \
                + 'not found, and CreateLayer not supported by driver.'
            return False

        gdal.ErrorReset()

        poDstLayer = poDstDS.CreateLayer(pszNewLayerName, poOutputSRS, eGType,
                papszLCO)
...
# /* -------------------------------------------------------------------- */
        ...
        print 'FAILED: Layer ' + pszNewLayerName \
    ...
    if papszSelFields is not None and not bAppend:
        ...
        for iField in range(len(papszSelFields)):
            ...
            if iSrcField >= 0:
                ...
                if papszFieldTypesToString is not None \
                    and (CSLFindString(papszFieldTypesToString, 'All') != -1
                         or CSLFindString(papszFieldTypesToString,
                ...
                if iDstField >= 0:
                    ...
                    if poDstFDefn is not None and poDstFDefn.GetFieldCount() \
                        != nDstFieldCount + 1:
                        print 'The output driver has claimed to have added the %s field, but it did not!' \
                ...
                print "Field '" + papszSelFields[iField] \
    ...
    while True:
        ...
        if bExplodeCollections:
            ...
            if poSrcGeometry is not None:
                ...
                if eSrcType == ogr.wkbMultiPoint or eSrcType \
                    == ogr.wkbMultiLineString or eSrcType \
                    == ogr.wkbMultiPolygon or eSrcType \
                    == ogr.wkbGeometryCollection:
        ...
        for iPart in range(nIters):
            ...
            if poDstFeature.SetFromWithMap(poFeature, 1, panMap) != 0:
                ...
                print 'Unable to translate feature %d from layer %s' \
            ...
            if poDstGeometry is not None:
                ...
                if eGeomOp == GeomOperation.SEGMENTIZE:
                    pass
                elif eGeomOp == GeomOperation.SIMPLIFY_PRESERVE_TOPOLOGY \
                    and dfGeomOpParam > 0:
                ...
                if poClipSrc is not None:
                    ...
                    if poClipped is None or poClipped.IsEmpty():
                        ...
                        if pfnProgress is not None:
                            pfnProgress(nCount * 1.0 / nCountLayerFeatures, ''
                                    , pProgressData)
                        continue
