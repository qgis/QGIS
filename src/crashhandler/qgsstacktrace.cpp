  ...
  switch ( basicType )
  {
    case btChar:
    {
      if ( !whilePrintingPointer )
      {
        ...
        stackTrace->written += swprintf_s( stackTrace->message + stackTrace->written, sizeof( stackTrace->message ) / sizeof( stackTrace->message[0] ) - stackTrace->written,
                                           L"'%c'", value );
      }
      else
      {
...
void getStackTrace( StackTrace *stackTrace, QString symbolPath, QgsStackTrace *trace )
{
  ...
  for ( int i = 0;; i++ )
  {
    ...
    if ( SymFromAddrW( stackTrace->process, stackTrace->currentStackFrame.AddrPC.Offset, NULL, symbol ) )
    {
      ...
      stackTrace->written += swprintf_s( &stackTrace->message[stackTrace->written], sizeof( stackTrace->message ) / sizeof( stackTrace->message[0] ) - stackTrace->written - 1,
