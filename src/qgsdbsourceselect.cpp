...
void QgsDbSourceSelect::editConnection ()
{
  ...
  QgsNewConnection *nc =
    new QgsNewConnection (cmbConnections->currentText ());
...
void QgsDbSourceSelect::dbConnect ()
{
  ...
  if (pd->Status () == CONNECTION_OK)
	...
	{
	  QString msg;
	  QTextOStream (&msg) << "Fetched " << pd->
	    Tuples () << " tables from database";
	  qDebug (msg);
	...
	{
	  qDebug
	    ("Unable to get list of spatially enabled tables from geometry_columns table");
