...
{
  mTerrain = t;
  mTileCrsToMapCrs = QgsCoordinateTransform( mMetadata->mCrs, mTerrain->mapSettings()->crs(), mTerrain->mapSettings()->transformContext() );
}
...
QFuture<QgsChunkLoaderResult> QgsQuantizedMeshTerrainGenerator::loadChunk( QgsChunkNode *node )
{
  ...
  return QtFuture::whenAll( //
      ...
      QtConcurrent::run( [this, node, vertScale, chunkOrigin, shadingEnabled, context, tileId]() -> QgsTerrainTileEntity * {
        ...
        QgsTiledSceneTile tile = mIndex.getTile( tileId );
    ...
    .then( [this]( QList<std::variant<QFuture<QgsTerrainGenerator::TerrainTextureResources>, QFuture<QgsTerrainTileEntity *>>> results ) {
      QgsTerrainGenerator::TerrainTextureResources textureResources = std::get<0>( results[0] ).result();
      QgsTerrainTileEntity *entity = std::get<1>( results[1] ).result();
      ...
      return QgsChunkLoaderResult { [this, entity, textureResources]( Qt3DCore::QEntity *parent ) {
        if ( entity )
        {
          ...
          Qt3DRender::QTexture2D *texture = createTexture( entity, QgsMaterialContext::fromRenderContext( context ), textureResources );
...
QVector<QgsChunkNode *> QgsQuantizedMeshTerrainGenerator::createChildren( QgsChunkNode *node ) const
{
  ...
  {
    QgsChunkNodeId childId( node->tileId().d + 1, node->tileId().x * 2 + offset.first, node->tileId().y * 2 + offset.second );
    ...
    children.push_back( new QgsChunkNode( childId, QgsBox3D( corner1, corner2 ), mMetadata->geometricErrorAtZoom( tile.zoomLevel() ), node ) );
  }
...
QgsTileXYZ QgsQuantizedMeshTerrainGenerator::nodeIdToTile( QgsChunkNodeId nodeId ) const
{
  ...
  if ( nodeId.d == 0 )
    return { 0, 0, -1 };
  return { nodeId.x, mMetadata->mTileScheme == "tms"_L1 ? ( 1 << ( nodeId.d - 1 ) ) - nodeId.y - 1 : nodeId.y, nodeId.d - 1 };
}
