/***************************************************************************
  qgsmaskeddiffusespecularematerial.cpp
  --------------------------------------
  Date                 : March 2024
...
///@cond PRIVATE
QgsMaskedDiffuseSpecularMaterial::QgsMaskedDiffuseSpecularMaterial( QNode *parent )
  : QMaterial( parent )
...
QgsMaskedDiffuseSpecularMaterial::~QgsMaskedDiffuseSpecularMaterial() = default;


void QgsMaskedDiffuseSpecularMaterial::init()
{
  connect( mAmbientParameter, &Qt3DRender::QParameter::valueChanged,
           this, &QgsMaskedDiffuseSpecularMaterial::handleAmbientChanged );
  connect( mDiffuseParameter, &Qt3DRender::QParameter::valueChanged,
           this, &QgsMaskedDiffuseSpecularMaterial::handleDiffuseChanged );
  connect( mSpecularParameter, &Qt3DRender::QParameter::valueChanged,
           this, &QgsMaskedDiffuseSpecularMaterial::handleSpecularChanged );
  connect( mShininessParameter, &Qt3DRender::QParameter::valueChanged,
           this, &QgsMaskedDiffuseSpecularMaterial::handleShininessChanged );
  ...
  mGL3Shader->setVertexShaderCode( Qt3DRender::QShaderProgram::loadSource( QUrl( QStringLiteral( "qrc:/shaders/maskeddiffusespecular.vert" ) ) ) );
...
void QgsMaskedDiffuseSpecularMaterial::setAmbient( const QColor &ambient )
{
  mAmbientParameter->setValue( ambient );
}

void QgsMaskedDiffuseSpecularMaterial::setDiffuse( const QVariant &diffuse )
{
  mDiffuseParameter->setValue( diffuse );
  mDiffuseTextureParameter->setValue( diffuse );

  QByteArray fragmentShaderCode = Qt3DRender::QShaderProgram::loadSource( QUrl( QStringLiteral( "qrc:/shaders/maskeddiffusespecular.frag" ) ) );
...
void QgsMaskedDiffuseSpecularMaterial::setSpecular( const QColor &specular )
{
  mSpecularParameter->setValue( specular );
}

void QgsMaskedDiffuseSpecularMaterial::setShininess( float shininess )
{
  mShininessParameter->setValue( shininess );
}

QColor QgsMaskedDiffuseSpecularMaterial::ambient() const
{
  return mAmbientParameter->value().value<QColor>();
}

QVariant QgsMaskedDiffuseSpecularMaterial::diffuse() const
{
  return mDiffuseParameter->value();
}

QColor QgsMaskedDiffuseSpecularMaterial::specular() const
{
  return mSpecularParameter->value().value<QColor>();
}

float QgsMaskedDiffuseSpecularMaterial::shininess() const
{
  return mShininessParameter->value().toFloat();
}

void QgsMaskedDiffuseSpecularMaterial::handleAmbientChanged( const QVariant &var )
{
  emit ambientChanged( var.value<QColor>() );
}

void QgsMaskedDiffuseSpecularMaterial::handleDiffuseChanged( const QVariant &var )
{
  emit diffuseChanged( var );
}

void QgsMaskedDiffuseSpecularMaterial::handleSpecularChanged( const QVariant &var )
{
  emit specularChanged( var.value<QColor>() );
}

void QgsMaskedDiffuseSpecularMaterial::handleShininessChanged( const QVariant &var )
{
