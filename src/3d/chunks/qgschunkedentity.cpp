...
void QgsChunkedEntity::update( QgsChunkNode *root, const SceneContext &sceneContext )
{
  ...
  while ( !pq.empty() && renderedCount <= mPrimitivesBudget )
  {
    ...
    if ( !node->hasChildrenPopulated() )
    {
      ...
      // TODO(dvdkon): Just make createChildren async
      if ( mChunkLoader->canCreateChildren( node ) )
      {
        node->populateChildren( mChunkLoader->createChildren( node ) );
      }
      else
      {
        mChunkLoader->prepareChildren( node );
      }
    }
...
void QgsChunkedEntity::onActiveJobFinished( QgsChunkQueueJob &job, QgsChunkLoaderResult result )
{
  int oldJobsCount = pendingJobsCount();
  ...
  else
  {
    ...
    // with QgsChunkUpdaterFactory passed to updatedNodes(). The result will
    // give us a completely new QEntity, so we just delete the old one and use
    // the new one.
    {
...
{
  mActiveJobs.push_back( std::make_unique<QgsChunkQueueJob>( node ) );
  auto &jobPtr = mActiveJobs.back();
  jobPtr->node = node;
  jobPtr->future = mChunkLoader->loadChunk( node ).then( this, [this, &job = *jobPtr]( QgsChunkLoaderResult res ) { this->onActiveJobFinished( job, res ); } );
...
{
  while ( mActiveJobs.size() )
  {
    cancelActiveJob( *mActiveJobs.back() );
    mActiveJobs.pop_back();
  }
}
