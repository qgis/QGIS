  ...
  , mMapSettings( settings )
{
  ...
  mLabelsLayer->setObjectName( objectName() + "::LabelsLayer" );
  ...
  // render pass for the axis labels
  ...
  // this ensures to have the labels (Text2DEntity) rendered after the other objects and therefore
  // avoid any transparency issue on the labels.
  Qt3DRender::QSortPolicy *labelsSortPolicy = new Qt3DRender::QSortPolicy( labelsCameraSelector );
  QVector<Qt3DRender::QSortPolicy::SortType> labelsSortTypes = QVector<Qt3DRender::QSortPolicy::SortType>();
  labelsSortTypes << Qt3DRender::QSortPolicy::BackToFront;
  labelsSortPolicy->setSortTypes( labelsSortTypes );

  Qt3DRender::QClearBuffers *clearBuffers = new Qt3DRender::QClearBuffers( labelsSortPolicy );
...
Qt3DRender::QLayer *Qgs3DAxisRenderView::labelsLayer() const
{
  return mLabelsLayer;
}
...
void Qgs3DAxisRenderView::onViewportSizeUpdate( int )
{
  ...
  {
    QgsMapSettings set;
    QgsDebugMsgLevel( QString( "onViewportSizeUpdate window w/h: %1px / %2px" )
                      .arg( windowWidth ).arg( windowHeight ), 2 );
    QgsDebugMsgLevel( QString( "onViewportSizeUpdate window physicalDpi %1 (%2, %3)" )
                      .arg( mCanvas->screen()->physicalDotsPerInch() )
                      .arg( mCanvas->screen()->physicalDotsPerInchX() )
                      .arg( mCanvas->screen()->physicalDotsPerInchY() ), 2 );
    QgsDebugMsgLevel( QString( "onViewportSizeUpdate window logicalDotsPerInch %1 (%2, %3)" )
                      .arg( mCanvas->screen()->logicalDotsPerInch() )
                      .arg( mCanvas->screen()->logicalDotsPerInchX() )
                      .arg( mCanvas->screen()->logicalDotsPerInchY() ), 2 );

    QgsDebugMsgLevel( QString( "onViewportSizeUpdate window pixel ratio %1" )
                      .arg( mCanvas->screen()->devicePixelRatio() ), 2 );

    QgsDebugMsgLevel( QString( "onViewportSizeUpdate set pixel ratio %1" )
                      .arg( set.devicePixelRatio() ), 2 );
    QgsDebugMsgLevel( QString( "onViewportSizeUpdate set outputDpi %1" )
                      .arg( set.outputDpi() ), 2 );
    QgsDebugMsgLevel( QString( "onViewportSizeUpdate set dpiTarget %1" )
                      .arg( set.dpiTarget() ), 2 );
  }
  ...
  if ( heightRatio * windowHeight < viewportPixelSize )
  {
    ...
    QgsDebugMsgLevel( QString( "3DAxis viewport, height too small, ratios adjusted to width: %1% / height: %2%" )
                      .arg( widthRatio * 100.0 )
                      .arg( heightRatio * 100.0 ), 2 );
  }
  ...
  if ( heightRatio > settings.maxViewportRatio() || widthRatio > settings.maxViewportRatio() )
  {
    ...
    QgsDebugMsgLevel( QString( "3DAxis viewport takes too much place into the 3d view, disabling it (maxViewportRatio: %1)." )
                      .arg( settings.maxViewportRatio() ), 2 );
  ...
  else
  {
    ...
    if ( settings.mode() == Qgs3DAxisSettings::Mode::Crs )
    {
      ...
      mLabelsCamera->lens()->setOrthographicProjection(
        -halfWidthSize, halfWidthSize,
        -halfHeightSize, halfHeightSize,
        mLabelsCamera->lens()->nearPlane(), mLabelsCamera->lens()->farPlane() );
    }
  }
}
