...
void QgsCategorized3DRenderer::updateSymbols( QgsAbstract3DSymbol *symbol )
{
  ...
  {
    std::unique_ptr<QgsAbstract3DSymbol> newSymbol( symbol->clone() );
    Qgs3DSymbolUtils::copyVectorSymbolMaterial( cat.symbol(), newSymbol.get() );
    updateCategorySymbol( catIndex, newSymbol.release() );
    ++catIndex;
  }
...
Qgs3DCategoryList QgsCategorized3DRenderer::createCategories( const QList<QVariant> &values, const QgsAbstract3DSymbol *symbol, QgsVectorLayer *layer, const QString &attributeName )
{
  ...
  if ( layer && !attributeName.isNull() )
  {
    ...
    for ( const QVariant &value : vals )
    {
      ...
      {
        std::unique_ptr<QgsAbstract3DSymbol> newSymbol( symbol->clone() );
        cats.append( Qgs3DRendererCategory( value, newSymbol.release(), true ) );
      }
    }
  }
  ...
  std::unique_ptr<QgsAbstract3DSymbol> newSymbol( symbol->clone() );
  cats.append( Qgs3DRendererCategory( QVariant(), newSymbol.release(), true ) );
