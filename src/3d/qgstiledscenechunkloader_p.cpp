...
void QgsTiledSceneChunkLoader::start()
{
  ...
  const QFuture<void> future = QtConcurrent::run( [this, tileId, boundsTransform, chunkOrigin, isGlobe] {
    ...
    else if ( format == "cesiumtiles"_L1 )
    {
      ...
      const auto instancedPrimitives = QgsGltfUtils::resolveInstancing( model, tileContent.instancing, entityTransform.gltfUpAxis, rawTileTransform, tileContent.rtcCenter );
