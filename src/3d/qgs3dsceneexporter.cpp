...
QVector<T> getAttributeData( Qt3DQAttribute *attribute, const QByteArray &data )
{
  ...
  if ( bytesStride == 0 )
  {
    QgsDebugMsgLevel( QStringLiteral( "bytesStride==0, the attribute probably was not set properly" ), 2 );
    return result;
  }
...
QVector<uint> getIndexData( Qt3DQAttribute *indexAttribute, const QByteArray &data )
{
  switch ( indexAttribute->vertexBaseType() )
  {
    ...
    default:
      QgsDebugMsgLevel( QStringLiteral( "Probably trying to get index data using an attribute that has vertex data" ), 2 );
      break;
  }
  return QVector<uint>();
}
...
QByteArray getData( Qt3DQBuffer *buffer )
{
  ...
  if ( bytes.isNull() )
  {
    QgsDebugMsgLevel( QStringLiteral( "QBuffer is null" ), 3 );
  }
  return bytes;
}
...
void Qgs3DSceneExporter::parseFlatTile( QgsTerrainTileEntity *tileEntity, const QString &layerName )
{
  ...
  {
    QgsDebugMsgLevel( QStringLiteral( "Qt3DExtras::QPlaneGeometry* is expected but something else was given" ), 2 );
    return;
  }
...
void Qgs3DSceneExporter::parseDemTile( QgsTerrainTileEntity *tileEntity, const QString &layerName )
{
  ...
  if ( tileGeometry == nullptr )
  {
    QgsDebugMsgLevel( QStringLiteral( "DemTerrainTileGeometry* is expected but something else was given" ), 2 );
    return;
  }
...
Qgs3DExportObject *Qgs3DSceneExporter::processGeometryRenderer( Qt3DRender::QGeometryRenderer *geomRenderer, const QString &objectNamePrefix, float sceneScale, QVector3D sceneTranslation )
{
  ...
  // In polygon cases, we have multiple feature geometries within the same geometry object (QgsTessellatedPolygonGeometry).
  // The QgsTessellatedPolygonGeometry class holds the list of all feature ids included.
  // To avoid to export the same geometry (can be included in multiple QgsTessellatedPolygonGeometry) more than once,
  ...
  if ( positionAttribute == nullptr )
  {
    QgsDebugMsgLevel( QStringLiteral( "Geometry renderer with null data was being processed" ), 2 );
    return nullptr;
  }
...
QVector<Qgs3DExportObject *> Qgs3DSceneExporter::processLines( Qt3DCore::QEntity *entity, const QString &objectNamePrefix )
{
  ...
  for ( Qt3DRender::QGeometryRenderer *renderer : renderers )
  {
    ...
    if ( positionAttribute == nullptr || indexAttribute == nullptr )
    {
      QgsDebugMsgLevel( QStringLiteral( "Position or index attribute was not found" ), 2 );
      continue;
    }
