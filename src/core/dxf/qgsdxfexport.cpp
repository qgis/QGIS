...
void QgsDxfExport::writeHeader( const QString &codepage )
{
  ...
  writeGroup( 0, QgsPoint( QgsWkbTypes::Type::PointZ, mExtent.xMinimum(), mExtent.yMinimum(), 0.0 ) );

  // EXTMAX
  writeGroup( 9, QStringLiteral( "$EXTMAX" ) );
  writeGroup( 0, QgsPoint( QgsWkbTypes::Type::PointZ, mExtent.xMaximum(), mExtent.yMaximum(), 0.0 ) );
...
void QgsDxfExport::writeTables()
{
  ...
  writeGroup( 6, QgsPoint( QgsWkbTypes::Type::PointZ, 0.0, 0.0, 1.0 ) );  // view direction from target point
  ...
  writeGroup( 100, QgsPoint( QgsWkbTypes::Type::PointZ, 0.0, 0.0, 0.0 ) );// UCS origin
  writeGroup( 101, QgsPoint( QgsWkbTypes::Type::PointZ, 1.0, 0.0, 0.0 ) );// UCS x axis
  writeGroup( 102, QgsPoint( QgsWkbTypes::Type::PointZ, 0.0, 1.0, 0.0 ) );// UCS y axis
...
void QgsDxfExport::writeBlocks()
{
  ...
  for ( const QString &block : blockStrings )
  {
    ...
    writeGroup( 2, block );
    writeGroup( 70, 0 );
    writeGroup( 0, QgsPoint( QgsWkbTypes::Type::PointZ, 0.0, 0.0, 0.0 ) );
    writeGroup( 3, block );
...
void QgsDxfExport::writePoint( const QgsPoint &pt, const QString &layer, const QColor &color, QgsSymbolRenderContext &ctx, const QgsSymbolLayer *symbolLayer, const QgsSymbol *symbol, double angle )
{
  ...
  if ( msl )
  {
    ...
    writeGroup( 8, layer );
    writeGroup( 62, 1 );
    writeGroup( 0, QgsPoint( QgsWkbTypes::Type::PointZ, pt.x() - halfSize, pt.y() - halfSize ) );
    writeGroup( 1, QgsPoint( QgsWkbTypes::Type::PointZ, pt.x() + halfSize, pt.y() - halfSize ) );
    writeGroup( 2, QgsPoint( QgsWkbTypes::Type::PointZ, pt.x() - halfSize, pt.y() + halfSize ) );
    writeGroup( 3, QgsPoint( QgsWkbTypes::Type::PointZ, pt.x() + halfSize, pt.y() + halfSize ) );
  }
#endif // 0
...
void QgsDxfExport::writePolygon( const QgsRingSequence &polygon, const QString &layer, const QString &hatchPattern, const QColor &color )
{
  ...
  writeGroup( 0, QgsPoint( QgsWkbTypes::Type::PointZ, 0.0, 0.0, 0.0 ) ); // Elevation point (in OCS)
  writeGroup( 200, QgsPoint( QgsWkbTypes::Type::PointZ, 0.0, 0.0, 1.0 ) );
...
void QgsDxfExport::writeFilledCircle( const QString &layer, const QColor &color, const QgsPoint &pt, double radius )
{
  ...
  writeGroup( 0, QgsPoint( QgsWkbTypes::Type::Point, pt.x() - radius, pt.y() ) );
  writeGroup( 42, 1.0 );
...
void QgsDxfExport::addFeature( QgsSymbolRenderContext &ctx, const QgsCoordinateTransform &ct, const QString &layer, const QgsSymbolLayer *symbolLayer, const QgsSymbol *symbol )
{
  ...
  // single point
  if ( QgsWkbTypes::flatType( geometryType ) == QgsWkbTypes::Type::Point )
  {
    writePoint( geom.constGet()->coordinateSequence().at( 0 ).at( 0 ).at( 0 ), layer, penColor, ctx, symbolLayer, symbol, angle );
    return;
  }

  if ( QgsWkbTypes::flatType( geometryType ) == QgsWkbTypes::Type::MultiPoint )
  {
