...
{
  QgsDebugMsgLevel( QStringLiteral( "theWidth= %1 height = %2 dataType = %3" ).arg( width ).arg( height ).arg( static_cast<int>( dataType ) ), 4 );
  ...
  if ( typeIsNumeric( dataType ) )
  {
    ...
    qgssize tSize = typeSize( static_cast<int>( dataType ) );
  ...
  mValid = true;
  mDataType = dataType;
  mTypeSize = QgsRasterBlock::typeSize( static_cast<int>( mDataType ) );
  mWidth = width;
  mHeight = height;
...
Qgis::DataType QgsRasterBlock::typeWithNoDataValue( Qgis::DataType dataType, double *noDataValue )
{
  ...
  QgsDebugMsgLevel( QStringLiteral( "newDataType = %1 noDataValue = %2" ).arg( static_cast<int>( newDataType ) ).arg( *noDataValue ), 4 );
  return newDataType;
}
...
bool QgsRasterBlock::setIsNoData()
{
  ...
  {
    const size_t dataTypeSize = typeSize( static_cast<int>( mDataType ) );
    if ( mHasNoDataValue )
    {
      if ( !mData )
      {
...
{
  if ( mData )
    return QByteArray::fromRawData( static_cast<const char *>( mData ), typeSize( static_cast<int>( mDataType ) ) * mWidth * mHeight );
...
void QgsRasterBlock::setData( const QByteArray &data, int offset )
{
  ...
  if ( mData )
  {
    int len = std::min( data.size(), typeSize( static_cast<int>( mDataType ) ) * mWidth * mHeight - offset );
...
bool QgsRasterBlock::convert( Qgis::DataType destDataType )
{
  ...
  if ( typeIsNumeric( mDataType ) && typeIsNumeric( destDataType ) )
  {
    ...
    mTypeSize = typeSize( static_cast<int>( mDataType ) );
  }
...
{
  int destDataTypeSize = typeSize( static_cast<int>( destDataType ) );
...
{
  qgssize size = QgsRasterBlock::typeSize( static_cast<int>( dataType ) );
  QByteArray ba;
...
QString  QgsRasterBlock::toString() const
{
  return QStringLiteral( "dataType = %1 width = %2 height = %3" )
         .arg( static_cast<int>( mDataType ) ).arg( mWidth ).arg( mHeight );
}
