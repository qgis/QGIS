    ...
    const QgsCoordinateReferenceSystem &crs, const QgsCoordinateTransformContext &transformContext, QgsRasterBlockFeedback *feedback )
{
  ...
  int dataTypeSize = QgsRasterBlock::typeSize( static_cast<int>( srcProvider->sourceDataType( 1 ) ) );
  for ( int i = 2; i <= nBands; ++i )
  {
    if ( QgsRasterBlock::typeSize( static_cast<int>( srcProvider->sourceDataType( 1 ) ) ) != dataTypeSize )
    {
  ...
  for ( int bandNo = 1; bandNo <= nBands; bandNo++ )
  {
    ...
    QgsDebugMsgLevel( QStringLiteral( "bandNo = %1 destDataType = %2 destHasNoDataValue = %3 destNoDataValue = %4" ).arg( bandNo ).arg( static_cast<int>( destDataType ) ).arg( destHasNoDataValue ).arg( destNoDataValue ), 4 );
