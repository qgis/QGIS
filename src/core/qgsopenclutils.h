...
class CORE_EXPORT QgsOpenClUtils
{
  ...
  public:
     ...
     */
    template<typename T> struct CPLAllocator
    {
      public:
        explicit CPLAllocator( unsigned long size )
          : mMem( static_cast<T *>( CPLMalloc( sizeof( T ) * size ) ) )
        {}

        ~CPLAllocator() { CPLFree( static_cast<void *>( mMem ) ); }
        ...
        void reset( unsigned long size ) { reset( static_cast<T *>( CPLMalloc( sizeof( T ) * size ) ) ); }
        ...
        T &operator[]( const int index ) { return mMem[index]; }
