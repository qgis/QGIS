...
void QgsExpression::buildVariableHelp()
{
  ...
  sVariableHelpTexts()->insert( QStringLiteral( "row_number" ), QCoreApplication::translate( "variable_help", "Stores the number of the current row. For attribute table data it's respecting order of features from data source. If used from attribute table field calculator index starts at 1, otherwise (e.g. from processing tools) index starts from 0." ) );
