  ...
  , mSymbolType( QgsSymbol::symbolTypeForGeometryType( QgsWkbTypes::geometryType( source->mWkbType ) ) )
{
   ...
   */
  mAllowResetReading = !transaction || ( source->mDriverName != QLatin1String( "GPKG" ) && source->mDriverName != QLatin1String( "SQLite" ) ) || ( mRequest.filterType() != Qgis::FeatureRequestFilterType::Fid && mRequest.filterType() != Qgis::FeatureRequestFilterType::Fids );
  ...
  if ( !constOrderBy.isEmpty() && source->mDriverName == QLatin1String( "GPKG" ) && ( request.filterType() == Qgis::FeatureRequestFilterType::NoFilter || request.filterType() == Qgis::FeatureRequestFilterType::Expression ) && ( mSource->mSubsetString.isEmpty() || !mSource->mSubsetString.startsWith( QLatin1String( "SELECT " ), Qt::CaseInsensitive ) ) )
  {
  ...
  if ( request.filterType() == Qgis::FeatureRequestFilterType::Expression && !filterExpressionAlreadyTakenIntoAccount )
  {
    ...
    if ( result == QgsSqlExpressionCompiler::Complete || result == QgsSqlExpressionCompiler::Partial )
    {
      ...
      {
        whereClause = QStringLiteral( "(" ) + mSource->mSubsetString + QStringLiteral( ") AND (" ) + whereClause + QStringLiteral( ")" );
      }
  ...
  if ( QgsApplication::databaseQueryLog()->enabled() && mConn )
  {
    GDALDatasetSetQueryLoggerFunc(
      mConn->ds, []( const char *pszSQL, const char *pszError, int64_t lNumRecords, int64_t lExecutionTimeMilliseconds, void *pQueryLoggerArg ) {
  ...
  if ( mAllowResetReading && !mSource->mCanDriverShareSameDatasetAmongLayers )
  {
    ...
    for ( int passNumber = 0; passNumber < 2; passNumber++ )
    {
      while ( fet.reset( GDALDatasetGetNextFeature(
                mConn->ds, &nextFeatureBelongingLayer, nullptr, nullptr, nullptr ) ),
              fet )
      {
