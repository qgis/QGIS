    ...
    if ( !layerName.isEmpty() )
    {
      ...
      if ( hDS )
      {
        ...
        if ( hLayer )
        {
          ...
          firstFieldIsFid = !( EQUAL( OGR_L_GetFIDColumn( hLayer ), "" ) ) && OGR_FD_GetFieldIndex( OGR_L_GetLayerDefn( hLayer ), ogrFidColumnName.toUtf8() ) < 0 && cleanedFields.indexFromName( ogrFidColumnName.toUtf8() ) < 0;
...
void QgsOgrProvider::loadFields()
{
  ...
  for ( int i = 0; i < fdef.GetFieldCount(); ++i )
  {
    ...
    if ( defaultValue == QLatin1String( "FILEGEODATABASE_SHAPE_LENGTH" ) || defaultValue == QLatin1String( "FILEGEODATABASE_SHAPE_AREA" ) )
    {
...
QgsRectangle QgsOgrProvider::extent() const
{
  ...
  if ( !mExtent2D && !mExtent3D )
  {
    ...
    if ( mForceRecomputeExtent && mValid && mWriteAccess && ( mGDALDriverName == QLatin1String( "GPKG" ) || mGDALDriverName == QLatin1String( "ESRI Shapefile" ) ) && mOgrOrigLayer )
    {
    ...
    {
      if ( ( mGDALDriverName == QLatin1String( "OAPIF" ) || mGDALDriverName == QLatin1String( "WFS3" ) ) && !mOgrLayer->TestCapability( OLCFastGetExtent ) )
      {
...
QVariant QgsOgrProvider::defaultValue( int fieldId ) const
{
  ...
  if ( mOgrOrigLayer && mTransaction && mDefaultValues.value( fieldId, QString() ) == tr( "Autogenerate" ) && providerProperty( EvaluateDefaultValues, false ).toBool() && ( mGDALDriverName == QLatin1String( "GPKG" ) || mGDALDriverName == QLatin1String( "SQLite" ) ) && mFirstFieldIsFid && fieldId == 0 )
  {
...
QString QgsOgrProvider::defaultValueClause( int fieldIndex ) const
{
  ...
  if ( mTransaction && mDefaultValues.value( fieldIndex, QString() ) == tr( "Autogenerate" ) && providerProperty( EvaluateDefaultValues, false ).toBool() && ( mGDALDriverName == QLatin1String( "GPKG" ) || mGDALDriverName == QLatin1String( "SQLite" ) ) && mFirstFieldIsFid && fieldIndex == 0 )
    return QString();
  else
...
bool QgsOgrProvider::skipConstraintCheck( int fieldIndex, QgsFieldConstraints::Constraint constraint, const QVariant &value ) const
{
  ...
  {
    // stricter check
    return mDefaultValues.contains( fieldIndex ) && !QgsVariantUtils::isNull( value ) && ( mDefaultValues.value( fieldIndex ) == value.toString() || value.userType() == qMetaTypeId<QgsUnsetAttributeValue>() );
  }
}
...
OGRGeometryH QgsOgrProvider::ConvertGeometryIfNecessary( OGRGeometryH hGeom )
{
  ...
  if ( flattenLayerGeomType == wkbPolygon && flattenGeomType == wkbMultiPolygon && mGDALDriverName == QLatin1String( "ESRI Shapefile" ) )
  {
...
bool QgsOgrProvider::addFeatures( QgsFeatureList &flist, Flags flags )
{
  ...
  if ( !( flags & QgsFeatureSink::FastInsert ) && ( mGDALDriverName == QLatin1String( "CSV" ) || mGDALDriverName == QLatin1String( "XLSX" ) || mGDALDriverName == QLatin1String( "ODS" ) ) )
  {
    QRecursiveMutex *mutex = nullptr;
    OGRLayerH layer = mOgrOrigLayer->getHandleAndMutex( mutex );
    {
...
bool QgsOgrProvider::changeAttributeValues( const QgsChangedAttributesMap &attr_map )
{
  ...
  if ( mGDALDriverName == QLatin1String( "ESRI Shapefile" ) || mGDALDriverName == QLatin1String( "GPKG" ) || mGDALDriverName == QLatin1String( "CSV" ) )
  {
  ...
  if ( static_cast<size_t>( attr_map.size() ) >= THRESHOLD_UPDATE_OPTIM && ( mGDALDriverName == QLatin1String( "GPKG" ) || mGDALDriverName == QLatin1String( "SQLite" ) ) && mOgrLayer->TestCapability( OLCFastFeatureCount ) && attr_map.size() == mOgrLayer->GetFeatureCount() )
  {
...
bool QgsOgrProvider::createSpatialIndexImpl()
{
  ...
  else if ( mGDALDriverName == QLatin1String( "GPKG" ) || mGDALDriverName == QLatin1String( "SQLite" ) )
  {
    QRecursiveMutex *mutex = nullptr;
    OGRLayerH layer = mOgrOrigLayer->getHandleAndMutex( mutex );
...
void QgsOgrProvider::open( OpenMode mode )
{
  ...
  else
  {
    ...
    if ( mGDALDriverName == QLatin1String( "FlatGeobuf" ) && !options.contains( QStringLiteral( "VERIFY_BUFFERS=YES" ) ) )
    {
  ...
  if ( mValid && mode == OpenModeInitial && mWriteAccess && ( mGDALDriverName == QLatin1String( "ESRI Shapefile" ) || mGDALDriverName == QLatin1String( "MapInfo File" )
#if GDAL_VERSION_NUM >= GDAL_COMPUTE_VERSION( 3, 4, 2 )
...
{
  if ( mFilePath.startsWith( QLatin1String( "/vsicurl/" ) ) || mFilePath.startsWith( QLatin1String( "/vsis3/" ) ) || mFilePath.startsWith( QLatin1String( "/vsigs/" ) ) || mFilePath.startsWith( QLatin1String( "/vsiaz/" ) ) || mFilePath.startsWith( QLatin1String( "/vsiadls/" ) ) )
  {
