...
namespace GDAL_MINMAXELT_NS
{
  namespace detail
  {
    ...
    template<class T, bool IS_MAX> size_t extremum_element( const T *v, size_t size, T noDataValue )
    {
      ...
      for ( ; i < size; ++i )
      {
        if ( v[i] != noDataValue && ( compScalar<T, IS_MAX>( v[i], extremum ) || extremum_is_nodata ) )
        {
          extremum = v[i];
          idx_of_extremum = i;
    ...
    template<class T, bool IS_MAX, class SSE_T> static inline __m128i comp( SSE_T x, SSE_T y )
    {
      if constexpr ( IS_MAX )
      {
        if constexpr ( std::is_same_v<T, uint8_t> )
          return _mm_cmpgt_epi8( _mm_add_epi8( x, _mm_set1_epi8( std::numeric_limits<int8_t>::min() ) ), y );
        else if constexpr ( std::is_same_v<T, int8_t> )
          return _mm_cmpgt_epi8( x, y );
        else if constexpr ( std::is_same_v<T, uint16_t> )
          return _mm_cmpgt_epi16( _mm_add_epi16( x, _mm_set1_epi16( std::numeric_limits<int16_t>::min() ) ), y );
        else if constexpr ( std::is_same_v<T, int16_t> )
          return _mm_cmpgt_epi16( x, y );
        else if constexpr ( std::is_same_v<T, uint32_t> )
          return _mm_cmpgt_epi32( _mm_add_epi32( x, _mm_set1_epi32( std::numeric_limits<int32_t>::min() ) ), y );
        else if constexpr ( std::is_same_v<T, int32_t> )
      ...
      {
        if constexpr ( std::is_same_v<T, uint8_t> )
          return _mm_cmplt_epi8( _mm_add_epi8( x, _mm_set1_epi8( std::numeric_limits<int8_t>::min() ) ), y );
        else if constexpr ( std::is_same_v<T, int8_t> )
          return _mm_cmplt_epi8( x, y );
        else if constexpr ( std::is_same_v<T, uint16_t> )
          return _mm_cmplt_epi16( _mm_add_epi16( x, _mm_set1_epi16( std::numeric_limits<int16_t>::min() ) ), y );
        else if constexpr ( std::is_same_v<T, int16_t> )
          return _mm_cmplt_epi16( x, y );
        else if constexpr ( std::is_same_v<T, uint32_t> )
          return _mm_cmplt_epi32( _mm_add_epi32( x, _mm_set1_epi32( std::numeric_limits<int32_t>::min() ) ), y );
        else if constexpr ( std::is_same_v<T, int32_t> )
    ...
    // Using SSE2
    template<class T, bool IS_MAX, bool HAS_NODATA> inline size_t extremum_element_with_nan( const T *v, size_t size, T noDataValue )
    {
      static_assert(
      ...
      const auto update = [v, noDataValue, &extremum, &idx_of_extremum, &extremum_is_invalid]( size_t idx ) {
        if constexpr ( HAS_NODATA )
        {
      ...
      for ( ; i < sse_iter_count; i += VALS_PER_ITER )
      {
        ...
        if constexpr ( HAS_NODATA )
        {
          ...
          {
            const auto replaceNoDataByNeutral = [sse_neutral, sse_nodata]( auto sse_val ) {
              const auto eq_nodata = _mm_cmpeq_epi8( sse_val, sse_nodata );
              return _mm_or_si128( _mm_and_si128( eq_nodata, sse_neutral ), _mm_andnot_si128( eq_nodata, sse_val ) );
            };
          ...
          else if constexpr ( std::is_same_v<T, float> )
          {
            const auto replaceNoDataByNeutral = [sse_neutral, sse_nodata]( auto sse_val ) {
              ...
              return _mm_or_ps( _mm_and_ps( eq_nodata, sse_neutral ), _mm_andnot_ps( eq_nodata, sse_val ) );
            };
          ...
          else if constexpr ( std::is_same_v<T, double> )
          {
            const auto replaceNoDataByNeutral = [sse_neutral, sse_nodata]( auto sse_val ) {
              ...
              return _mm_or_pd( _mm_and_pd( eq_nodata, sse_neutral ), _mm_andnot_pd( eq_nodata, sse_val ) );
            };
        ...
        if ( _mm_movemask_epi8(
               _mm_or_si128( _mm_or_si128( comp<T, IS_MAX>( sse_val0, sse_extremum ), comp<T, IS_MAX>( sse_val1, sse_extremum ) ), _mm_or_si128( comp<T, IS_MAX>( sse_val2, sse_extremum ), comp<T, IS_MAX>( sse_val3, sse_extremum ) ) )
             )
             != 0 )
        {
    ...
    template<class T, bool IS_MAX, bool HAS_NODATA> inline size_t extremum_element_with_nan( const T *v, size_t size, T /* nodata */ )
    {
      ...
      for ( ; i < size; ++i )
      {
        if ( compScalar<T, IS_MAX>( v[i], extremum ) || ( extremum_is_nan && !std::isnan( v[i] ) ) )
        {
          extremum = v[i];
          idx_of_extremum = i;
    ...
    template<> size_t extremum_element<uint8_t, true>( const uint8_t *v, size_t size, uint8_t noDataValue )
    {
      return extremum_element_with_nan<uint8_t, true, true>( v, size, noDataValue );
    }

    template<> size_t extremum_element<uint8_t, false>( const uint8_t *v, size_t size, uint8_t noDataValue )
    {
      return extremum_element_with_nan<uint8_t, false, true>( v, size, noDataValue );
    }

    template<> size_t extremum_element<uint8_t, true>( const uint8_t *v, size_t size )
    {
      return extremum_element_with_nan<uint8_t, true, false>( v, size, 0 );
    }

    template<> size_t extremum_element<uint8_t, false>( const uint8_t *v, size_t size )
    {
      return extremum_element_with_nan<uint8_t, false, false>( v, size, 0 );
    }

    template<> size_t extremum_element<int8_t, true>( const int8_t *v, size_t size, int8_t noDataValue )
    {
      return extremum_element_with_nan<int8_t, true, true>( v, size, noDataValue );
    }

    template<> size_t extremum_element<int8_t, false>( const int8_t *v, size_t size, int8_t noDataValue )
    {
    ...
    template<> size_t extremum_element<uint16_t, true>( const uint16_t *v, size_t size, uint16_t noDataValue )
    {
      return extremum_element_with_nan<uint16_t, true, true>( v, size, noDataValue );
    }

    template<> size_t extremum_element<uint16_t, false>( const uint16_t *v, size_t size, uint16_t noDataValue )
    {
      return extremum_element_with_nan<uint16_t, false, true>( v, size, noDataValue );
    }

    template<> size_t extremum_element<uint16_t, true>( const uint16_t *v, size_t size )
    {
      return extremum_element_with_nan<uint16_t, true, false>( v, size, 0 );
    }

    template<> size_t extremum_element<uint16_t, false>( const uint16_t *v, size_t size )
    {
      return extremum_element_with_nan<uint16_t, false, false>( v, size, 0 );
    }

    template<> size_t extremum_element<int16_t, true>( const int16_t *v, size_t size, int16_t noDataValue )
    {
      return extremum_element_with_nan<int16_t, true, true>( v, size, noDataValue );
    }

    template<> size_t extremum_element<int16_t, false>( const int16_t *v, size_t size, int16_t noDataValue )
    {
      return extremum_element_with_nan<int16_t, false, true>( v, size, noDataValue );
    }

    template<> size_t extremum_element<int16_t, true>( const int16_t *v, size_t size )
    {
      return extremum_element_with_nan<int16_t, true, false>( v, size, 0 );
    }

    template<> size_t extremum_element<int16_t, false>( const int16_t *v, size_t size )
    {
      return extremum_element_with_nan<int16_t, false, false>( v, size, 0 );
    }

    template<> size_t extremum_element<uint32_t, true>( const uint32_t *v, size_t size, uint32_t noDataValue )
    {
      return extremum_element_with_nan<uint32_t, true, true>( v, size, noDataValue );
    }

    template<> size_t extremum_element<uint32_t, false>( const uint32_t *v, size_t size, uint32_t noDataValue )
    {
      return extremum_element_with_nan<uint32_t, false, true>( v, size, noDataValue );
    }

    template<> size_t extremum_element<uint32_t, true>( const uint32_t *v, size_t size )
    {
      return extremum_element_with_nan<uint32_t, true, false>( v, size, 0 );
    }

    template<> size_t extremum_element<uint32_t, false>( const uint32_t *v, size_t size )
    {
      return extremum_element_with_nan<uint32_t, false, false>( v, size, 0 );
    }

    template<> size_t extremum_element<int32_t, true>( const int32_t *v, size_t size, int32_t noDataValue )
    {
      return extremum_element_with_nan<int32_t, true, true>( v, size, noDataValue );
    }

    template<> size_t extremum_element<int32_t, false>( const int32_t *v, size_t size, int32_t noDataValue )
    {
      return extremum_element_with_nan<int32_t, false, true>( v, size, noDataValue );
    }

    template<> size_t extremum_element<int32_t, true>( const int32_t *v, size_t size )
    {
      return extremum_element_with_nan<int32_t, true, false>( v, size, 0 );
    }

    template<> size_t extremum_element<int32_t, false>( const int32_t *v, size_t size )
    {
      return extremum_element_with_nan<int32_t, false, false>( v, size, 0 );
    }

    template<> size_t extremum_element<float, true>( const float *v, size_t size, float noDataValue )
    {
      return extremum_element_with_nan<float, true, true>( v, size, noDataValue );
    }

    template<> size_t extremum_element<float, false>( const float *v, size_t size, float noDataValue )
    {
      return extremum_element_with_nan<float, false, true>( v, size, noDataValue );
    }

    template<> size_t extremum_element<double, true>( const double *v, size_t size, double noDataValue )
    {
      return extremum_element_with_nan<double, true, true>( v, size, noDataValue );
    }

    template<> size_t extremum_element<double, false>( const double *v, size_t size, double noDataValue )
    {
    ...
    template<class T, bool IS_MAX> inline size_t extremum_element_with_nan( const T *v, size_t size, T noDataValue )
    {
      ...
      size_t idx_of_extremum = 0;
      auto extremum = v[0];
      bool extremum_is_nan_or_nodata = std::isnan( extremum ) || ( extremum == noDataValue );
      size_t i = 1;
      for ( ; i < size; ++i )
      {
        if ( v[i] != noDataValue && ( compScalar<T, IS_MAX>( v[i], extremum ) || ( extremum_is_nan_or_nodata && !std::isnan( v[i] ) ) ) )
        {
          extremum = v[i];
          idx_of_extremum = i;
    ...
    template<class T, bool IS_MAX> inline size_t extremum_element( const T *buffer, size_t size, bool bHasNoData, T noDataValue )
    {
      if ( bHasNoData )
      ...
      if ( bHasNoData )
      {
        if constexpr ( std::is_floating_point_v<T> )
        {
          if ( std::isnan( noDataValue ) )
          {
            if constexpr ( IS_MAX )
            {
              return std::max_element( buffer, buffer + size, []( T a, T b ) { return std::isnan( b ) ? false : std::isnan( a ) ? true : a < b; } ) - buffer;
            }
            else
            {
              return std::min_element( buffer, buffer + size, []( T a, T b ) { return std::isnan( b ) ? true : std::isnan( a ) ? false : a < b; } ) - buffer;
            }
          }
          else
          {
            if constexpr ( IS_MAX )
            {
              return std::max_element(
                       buffer, buffer + size, [noDataValue]( T a, T b ) { return std::isnan( b )        ? false
                                                                                 : std::isnan( a )      ? true
                                                                                 : ( b == noDataValue ) ? false
                                                                                 : ( a == noDataValue ) ? true
                                                                                                        : a < b; }
            ...
            else
            {
              return std::min_element(
                       buffer, buffer + size, [noDataValue]( T a, T b ) { return std::isnan( b )        ? true
                                                                                 : std::isnan( a )      ? false
                                                                                 : ( b == noDataValue ) ? true
                                                                                 : ( a == noDataValue ) ? false
                                                                                                        : a < b; }
        ...
        else
        {
          if constexpr ( IS_MAX )
          {
            return std::max_element( buffer, buffer + size, [noDataValue]( T a, T b ) { return ( b == noDataValue ) ? false : ( a == noDataValue ) ? true : a < b; } ) - buffer;
          }
          else
          {
            return std::min_element( buffer, buffer + size, [noDataValue]( T a, T b ) { return ( b == noDataValue ) ? true : ( a == noDataValue ) ? false : a < b; } ) - buffer;
          }
        }
      }
      else
    ...
    template<bool IS_MAX> size_t extremum_element( const void *buffer, size_t nElts, GDALDataType eDT, bool bHasNoData, double dfNoDataValue )
    {
      switch ( eDT )
      {
        ...
        case GDT_Int8:
        {
          ...
          return extremum_element<T, IS_MAX>( static_cast<const T *>( buffer ), nElts, bHasNoData, bHasNoData ? static_cast<T>( dfNoDataValue ) : 0 );
        }
#endif
        case GDT_Byte:
        {
      ...
      CPLError( CE_Failure, CPLE_NotSupported, "%s not supported for this data type.", __FUNCTION__ );
      return 0;
    }
 ...
 * @since GDAL 3.11
 */
  inline size_t max_element( const void *buffer, size_t nElts, GDALDataType eDT, bool bHasNoData, double dfNoDataValue )
  {
    return detail::extremum_element<true>( buffer, nElts, eDT, bHasNoData, dfNoDataValue );
  }
 ...
 * @since GDAL 3.11
 */
  inline size_t min_element( const void *buffer, size_t nElts, GDALDataType eDT, bool bHasNoData, double dfNoDataValue )
  {
    return detail::extremum_element<false>( buffer, nElts, eDT, bHasNoData, dfNoDataValue );
  }
  ...
  namespace detail
  {
    ...
    template<class T> std::pair<size_t, size_t> minmax_element( const T *v, size_t size, T noDataValue )
    {
      static_assert( !( std::is_floating_point_v<T> ) );
      if ( size == 0 )
        return std::pair( 0, 0 );
    ...
    template<class T> std::pair<size_t, size_t> minmax_element( const T *v, size_t size )
    {
      static_assert( !( std::is_floating_point_v<T> ) );
      if ( size == 0 )
        return std::pair( 0, 0 );
    ...
    template<class T> inline std::pair<size_t, size_t> minmax_element_with_nan( const T *v, size_t size )
    {
      if ( size == 0 )
        return std::pair( 0, 0 );
      size_t idx_of_min = 0;
    ...
    template<> std::pair<size_t, size_t> minmax_element<float>( const float *v, size_t size )
    {
      return minmax_element_with_nan<float>( v, size );
    }

    template<> std::pair<size_t, size_t> minmax_element<double>( const double *v, size_t size )
    {
      return minmax_element_with_nan<double>( v, size );
    }

    template<class T> inline std::pair<size_t, size_t> minmax_element( const T *buffer, size_t size, bool bHasNoData, T noDataValue )
    {
      if ( bHasNoData )
      {
    ...
    template<class T> inline std::pair<size_t, size_t> minmax_element( const T *buffer, size_t size, bool bHasNoData, T noDataValue )
    {
      ...
      if constexpr ( !std::is_floating_point_v<T> )
      {
        if ( !bHasNoData )
        {
          auto [min_iter, max_iter] = std::minmax_element( buffer, buffer + size );
      ...
      return std::pair( extremum_element<T, false>( buffer, size, bHasNoData, noDataValue ), extremum_element<T, true>( buffer, size, bHasNoData, noDataValue ) );
 ...
 * @since GDAL 3.11
 */
  inline std::pair<size_t, size_t> minmax_element( const void *buffer, size_t nElts, GDALDataType eDT, bool bHasNoData, double dfNoDataValue )
  {
    switch ( eDT )
    {
#if GDAL_VERSION_NUM >= GDAL_COMPUTE_VERSION( 3, 7, 0 )
      case GDT_Int8:
      {
        using T = int8_t;
        bHasNoData = bHasNoData && GDALIsValueExactAs<T>( dfNoDataValue );
        return detail::minmax_element<T>( static_cast<const T *>( buffer ), nElts, bHasNoData, bHasNoData ? static_cast<T>( dfNoDataValue ) : 0 );
      }
#endif
      case GDT_Byte:
      {
