...
template<class Tin, class Tout> inline void GDALGetDataLimits( Tin &tMaxValue, Tin &tMinValue )
{
  ...
  if constexpr ( std::numeric_limits<Tout>::is_signed && std::numeric_limits<Tout>::is_integer )
  {
    // the minimum value is less than zero
    if constexpr ( std::numeric_limits<Tout>::digits < std::numeric_limits<Tin>::digits || !std::numeric_limits<Tin>::is_integer )
    {
 ...
 */
template<class T> inline T GDALClampValue( const T tValue, const T tMax, const T tMin )
{
 ...
 *
 */
template<class T2, class T3> inline bool GDALClampDoubleValue( double &tValue, const T2 tMin, const T3 tMax )
{
...
{
  return dfValue >= static_cast<double>( std::numeric_limits<T>::lowest() ) && dfValue <= static_cast<double>( std::numeric_limits<T>::max() );
}
...
{
  return std::isinf( dfValue ) || ( dfValue >= -std::numeric_limits<float>::max() && dfValue <= std::numeric_limits<float>::max() );
}
...
template<> inline bool GDALIsValueInRange<int64_t>( double dfValue )
{
  ...
  return dfValue >= static_cast<double>( std::numeric_limits<int64_t>::min() ) && dfValue < static_cast<double>( std::numeric_limits<int64_t>::max() );
}
...
template<> inline bool GDALIsValueInRange<uint64_t>( double dfValue )
{
  ...
  return dfValue >= 0 && dfValue < static_cast<double>( std::numeric_limits<uint64_t>::max() );
}
...
{
  return GDALIsValueInRange<T>( dfValue ) && static_cast<double>( static_cast<T>( dfValue ) ) == dfValue;
}

template<> inline bool GDALIsValueExactAs<float>( double dfValue )
{
  return std::isnan( dfValue ) || ( GDALIsValueInRange<float>( dfValue ) && static_cast<double>( static_cast<float>( dfValue ) ) == dfValue );
}
...
{
    static inline void f( const Tin tValueIn, float &fValueOut ) { fValueOut = static_cast<float>( tValueIn ); }
};

template<class Tin> struct sGDALCopyWord<Tin, double>
{
    static inline void f( const Tin tValueIn, double &dfValueOut ) { dfValueOut = static_cast<double>( tValueIn ); }
};

template<> struct sGDALCopyWord<double, double>
{
    static inline void f( const double dfValueIn, double &dfValueOut ) { dfValueOut = dfValueIn; }
};

template<> struct sGDALCopyWord<float, float>
{
    static inline void f( const float fValueIn, float &fValueOut ) { fValueOut = fValueIn; }
};

template<> struct sGDALCopyWord<float, double>
{
    static inline void f( const float fValueIn, double &dfValueOut ) { dfValueOut = fValueIn; }
};
...
template<class Tout> struct sGDALCopyWord<float, Tout>
{
    static inline void f( const float fValueIn, Tout &tValueOut )
    {
      ...
      tValueOut = static_cast<Tout>( GDALClampValue( fValueIn + 0.5f, fMaxVal, fMinVal ) );
    }
};
...
template<> struct sGDALCopyWord<float, short>
{
    static inline void f( const float fValueIn, short &nValueOut )
    {
      ...
      nValueOut = static_cast<short>( GDALClampValue( fValue, fMaxVal, fMinVal ) );
    }
};
...
template<> struct sGDALCopyWord<float, signed char>
{
    static inline void f( const float fValueIn, signed char &nValueOut )
    {
      ...
      float fValue = fValueIn >= 0.0f ? fValueIn + 0.5f : fValueIn - 0.5f;
      nValueOut = static_cast<signed char>( GDALClampValue( fValue, fMaxVal, fMinVal ) );
    }
};
...
template<class Tout> struct sGDALCopyWord<double, Tout>
{
    static inline void f( const double dfValueIn, Tout &tValueOut )
    {
      ...
      tValueOut = static_cast<Tout>( GDALClampValue( dfValueIn + 0.5, dfMaxVal, dfMinVal ) );
    }
};
...
template<> struct sGDALCopyWord<double, int>
{
    static inline void f( const double dfValueIn, int &nValueOut )
    {
      ...
      nValueOut = static_cast<int>( GDALClampValue( dfValue, dfMaxVal, dfMinVal ) );
    }
};
...
template<> struct sGDALCopyWord<double, std::int64_t>
{
    static inline void f( const double dfValueIn, std::int64_t &nValueOut )
    {
      ...
      else if ( dfValueIn >= static_cast<double>( std::numeric_limits<std::int64_t>::max() ) )
      {
        nValueOut = std::numeric_limits<std::int64_t>::max();
      }
      else if ( dfValueIn <= static_cast<double>( std::numeric_limits<std::int64_t>::min() ) )
      {
        nValueOut = std::numeric_limits<std::int64_t>::min();
      }
      else
      {
        nValueOut = static_cast<std::int64_t>( dfValueIn > 0.0f ? dfValueIn + 0.5f : dfValueIn - 0.5f );
      }
    }
};
...
template<> struct sGDALCopyWord<double, std::uint64_t>
{
    static inline void f( const double dfValueIn, std::uint64_t &nValueOut )
    {
      ...
      else if ( dfValueIn > static_cast<double>( std::numeric_limits<uint64_t>::max() ) )
      {
...
template<> struct sGDALCopyWord<double, short>
{
    static inline void f( const double dfValueIn, short &nValueOut )
    {
      ...
      double dfValue = dfValueIn > 0.0 ? dfValueIn + 0.5 : dfValueIn - 0.5;
      nValueOut = static_cast<short>( GDALClampValue( dfValue, dfMaxVal, dfMinVal ) );
    }
};
...
template<> struct sGDALCopyWord<double, signed char>
{
    static inline void f( const double dfValueIn, signed char &nValueOut )
    {
      ...
      double dfValue = dfValueIn > 0.0 ? dfValueIn + 0.5 : dfValueIn - 0.5;
      nValueOut = static_cast<signed char>( GDALClampValue( dfValue, dfMaxVal, dfMinVal ) );
    }
};
...
template<> struct sGDALCopyWord<float, int>
{
    static inline void f( const float fValueIn, int &nValueOut )
    {
      ...
      else if ( fValueIn >= static_cast<float>( std::numeric_limits<int>::max() ) )
      {
      ...
      {
        nValueOut = static_cast<int>( fValueIn > 0.0f ? fValueIn + 0.5f : fValueIn - 0.5f );
      }
    }
};
...
template<> struct sGDALCopyWord<float, unsigned int>
{
    static inline void f( const float fValueIn, unsigned int &nValueOut )
    {
      ...
      else if ( fValueIn >= static_cast<float>( std::numeric_limits<unsigned int>::max() ) )
      {
...
template<> struct sGDALCopyWord<float, std::int64_t>
{
    static inline void f( const float fValueIn, std::int64_t &nValueOut )
    {
      ...
      else if ( fValueIn >= static_cast<float>( std::numeric_limits<std::int64_t>::max() ) )
      {
        nValueOut = std::numeric_limits<std::int64_t>::max();
      }
      ...
      {
        nValueOut = static_cast<std::int64_t>( fValueIn > 0.0f ? fValueIn + 0.5f : fValueIn - 0.5f );
      }
    }
};
...
template<> struct sGDALCopyWord<float, std::uint64_t>
{
    static inline void f( const float fValueIn, std::uint64_t &nValueOut )
    {
      ...
      else if ( fValueIn >= static_cast<float>( std::numeric_limits<std::uint64_t>::max() ) )
      {
...
template<class Tin, class Tout> inline void GDALCopyWord( const Tin tValueIn, Tout &tValueOut )
{
...
template<class Tin, class Tout> inline void GDALCopy4Words( const Tin *pValueIn, Tout *const pValueOut )
{
...
template<> inline void GDALCopy4Words( const float *pValueIn, GByte *const pValueOut )
{
...
template<> inline void GDALCopy4Words( const float *pValueIn, GInt16 *const pValueOut )
{
  __m128 xmm = _mm_loadu_ps( pValueIn );
  ...
  xmm = _mm_add_ps( xmm, _mm_or_ps( _mm_and_ps( mask, p0d5 ), _mm_andnot_ps( mask, m0d5 ) ) );
...
template<> inline void GDALCopy4Words( const float *pValueIn, GUInt16 *const pValueOut )
{
  __m128 xmm = _mm_loadu_ps( pValueIn );
...
template<> inline void GDALCopy8Words( const float *pValueIn, GByte *const pValueOut )
{
...
template<> inline void GDALCopy8Words( const float *pValueIn, GUInt16 *const pValueOut )
{
  __m256 ymm = _mm256_loadu_ps( pValueIn );
...
template<> inline void GDALCopy4Words( const double *pValueIn, float *const pValueOut )
{
