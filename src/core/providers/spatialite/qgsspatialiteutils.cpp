  ...
  if ( ( spatialiteDbInfo ) && ( spatialiteDbInfo->isDbSqlite3() ) )
  {
    ...
    if ( QgsSqliteHandle::sqlite3_open_v2( spatialiteDbInfo->getDatabaseFileName().toUtf8().constData(), &sqlite_handle, open_flags, nullptr )  == SQLITE_OK )
    {
      ...
      if ( spatialiteDbInfo->attachQSqliteHandle( handle ) )
      {
        ...
        {
          QgsDebugMsg( QString( "Connection to the database was successful [for QgsOgrProvider or QgsGdalProvider only] (%1,%2,%3,%4) Layers-Loaded[%5] VectorLayers-Found[%6] dbPath[%7] " ).arg( handle->getRef() ).arg( bShared ).arg( spatialiteDbInfo->isDbReadOnly() ).arg( spatialiteDbInfo->dbSpatialMetadataString() ).arg( spatialiteDbInfo->dbLoadedLayersCount() ).arg( spatialiteDbInfo->dbVectorLayersCount() ).arg( sDatabaseFileName ) );
        }
        else
        {
          QgsDebugMsg( QString( "Connection to the database was successful [for QgsSpatiaLiteProvider] (%1,%2,%3,%4) Layers-Loaded[%5] VectorLayers-Found[%6] dbPath[%7] " ).arg( handle->getRef() ).arg( bShared ).arg( spatialiteDbInfo->isDbReadOnly() ).arg( spatialiteDbInfo->dbSpatialMetadataString() ).arg( spatialiteDbInfo->dbLoadedLayersCount() ).arg( spatialiteDbInfo->dbVectorLayersCount() ).arg( sDatabaseFileName ) );
        }
      }
      else
      {
...
//----------------------------------------------------------
  ...
  //----------------------------------------------------------
  mVectorLayersMissing.clear();
  if ( dbLoadedLayersCount() != getDbVectorLayers().count() )
  {
  ...
  if ( imageResult.byteCount() > 0 )
  {
    ...
    if ( ( iPixelType == 21 ) || ( iPixelType == 22 ) )
    {
      ...
      for ( int j = 0; j < ( imageResult.byteCount() / iQImageBands ); j++ )
      {
        ...
        qDebug().noquote() << QString( "height_retrieve: %1: %2" ).arg( height_retrieve ).arg( sRGB );
      }
      ...
      qDebug().noquote() << QString( "input: %1: %2" ).arg( ( float )bits[0] ).arg( sRGB );
      ...
      qDebug().noquote() << QString( "sample: %1: %2" ).arg( ( float )iheight_pixel ).arg( sRGB );
      ...
      qDebug().noquote() << QString( "float_bytes: %1: %2" ).arg( ( float )iheight_pixel ).arg( sRGB );
      ...
      if ( bits )
      {
        for ( int j = 0; j < ( imageResult.byteCount() / iQImageBands ); j++ )
        {
          ...
          if ( j < 10 )
          {
            ...
            qDebug().noquote() << QString( "%1: %2" ).arg( j ).arg( sRGB );
          }
#endif
          for ( int iBand = 0; iBand < iQImageBands; iBand++ )
          {
    ...
    QString styleName, QString mimeType, QString bgColor, bool bTransparent, int quality, bool bReaspect )
{
  ...
  if ( ( layerHasRasterlite2() ) && ( ( !viewExtent.isEmpty() ) && ( width > 0 ) && ( height > 0 ) ) )
  {
    ...
    qDebug().noquote() << QString( "sql[%1]" ).arg( sql );
    if ( sqlite3_prepare_v2( dbSqliteHandle(), sql.toUtf8().constData(), -1, &stmt, nullptr ) == SQLITE_OK )
    {
      //-----------------------------------------------------------
...
//-----------------------------------------------------------
SpatialiteDbInfo *QgsSpatiaLiteUtils::CreateSpatialiteConnection( QString dbPath,  bool bLoadLayers,  bool bShared, SpatialiteDbInfo::SpatialMetadata dbCreateOption )
{
