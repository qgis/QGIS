...
QgsPdalFeatureIterator::QgsPdalFeatureIterator( QgsPdalFeatureSource *source, bool ownSource, const QgsFeatureRequest &request )
  : QgsAbstractFeatureIteratorFromSource<QgsPdalFeatureSource>( source, ownSource, request )
...
///https://blog.csdn.net/tszhangjunqiao/article/details/38518483
  ...
  {
	 // QgsPdalProviderUtils::setRelevantFields(mOgrLayer, mSource->mFields.count(), mFetchGeometry, attrs, mSource->mFirstFieldIsFid, mSource->mSubsetString);
...
QgsPdalFeatureIterator::~QgsPdalFeatureIterator()
{
  close();
}

bool QgsPdalFeatureIterator::fetchFeature(QgsFeature &feature)
{
...
bool QgsPdalFeatureIterator::nextFeatureFilterExpression( QgsFeature &f )
{
...
bool QgsPdalFeatureIterator::checkFeatureSinglePoint(std::shared_ptr<Geometry> geom, QgsFeature &feature)
{
...
check multipoint 
*/
bool QgsPdalFeatureIterator::checkFeature(std::shared_ptr<Geometry> geom, QgsFeature &feature )
{  
	//QgsMultiPoint
...
bool QgsPdalFeatureIterator::rewind()
{
  return true;
}


bool QgsPdalFeatureIterator::close()
{
...
bool QgsPdalFeatureIterator::readFeature( ) const
{
...
{
  return QgsFeatureIterator( new QgsPdalFeatureIterator( this, false, request ) );
}
