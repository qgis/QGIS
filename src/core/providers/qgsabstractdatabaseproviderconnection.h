...
class CORE_EXPORT QgsAbstractDatabaseProviderConnection : public QgsAbstractProviderConnection
{
  ...
  public:
    ...
    struct CORE_EXPORT TableProperty
    {
        ...
        % MethodCode
            QString str
          = QStringLiteral( "<QgsAbstractDatabaseProviderConnection.TableProperty: '%1'>" ).arg( sipCpp->tableName() );
          ...
          struct CORE_EXPORT GeometryColumnType
        {
            ...
            % MethodCode
                QString str
              = QStringLiteral( "<QgsAbstractDatabaseProviderConnection.TableProperty.GeometryColumnType: '%1, %2'>" ).arg( QgsWkbTypes::displayString( sipCpp->wkbType ), sipCpp->crs.authid() );
            sipRes = PyUnicode_FromString( str.toUtf8().constData() );
            % End
#endif
...
#else
        SIP_PYOBJECT
      supportedRelationshipStrengths() const SIP_TYPEHINT( List[Qgis.RelationshipStrength] );
    % MethodCode
      // adapted from the qpymultimedia_qlist.sip file from the PyQt6 sources
