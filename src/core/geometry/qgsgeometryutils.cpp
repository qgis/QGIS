...
bool QgsGeometryUtils::isGeometryCoplanar( const QgsAbstractGeometry *geom, double tolerance )
{
  ...
  normal.normalize();
  const double planeOffset = -QgsVector3D::dotProduct( normal, QgsVector3D( pt1.x(), pt1.y(), pt1.z() ) );
  ...
  while ( vertexIt.hasNext() )
  {
    ...
    const double distance = std::abs( QgsVector3D::dotProduct( normal, QgsVector3D( currentPoint.x(), currentPoint.y(), currentPoint.z() ) ) + planeOffset );
