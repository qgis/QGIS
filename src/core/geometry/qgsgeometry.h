...
class CORE_EXPORT QgsGeometry
{
  ...
  public:
    ...
    */
     ...
     *
     * \param index_at_closest_vertex will return the vertex index of the closest found vertex
     * \param index_before_closest_vertex will return the vertex index of the previous vertex from the closest one. Will be set to -1 if
     * not present.
     * \param index_after_closest_vertex will return the vertex index of the next vertex after the closest one. Will be set to -1 if
     * not present.
     ...
     * \param index_after_closest_segment Returns index of the closest vertex
     * \returns The squared Cartesian distance (returned in sqrDist) of the closest vertex to the input point, negative number on error
     */
    double closestVertexWithContext( const QgsPointXY &point, int &index_after_closest_segment SIP_OUT ) const;
     ...
     *
     * \param minDistPoint Returns the closest point on the segment with regards to the input point. The closest 
     * point is the perpendicular point if an end of the segment is not the shortest distance
     * \param index_after_closest_segment Returns the index of the vertex after the closest segment. The vertex
     * before the closest segment is always afterVertex - 1
     * \param left_or_right_of_line Returns if the point is located on the left or right side of the geometry ( < 0 means left, > 0 means right, 0 indicates
     * that the test was unsuccessful, e.g. for a point exactly on the line)
     * \returns The squared Cartesian distance (returned in sqrDist) of the input point to the minDistPoint, negative number on error
     */
    double closestSegmentWithContext( const QgsPointXY &point, QgsPointXY &minDistPoint SIP_OUT, int &index_after_closest_segment SIP_OUT, int *left_or_right_of_line SIP_OUT = nullptr, double epsilon = DEFAULT_SEGMENT_EPSILON ) const;
