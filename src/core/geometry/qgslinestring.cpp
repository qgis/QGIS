...
std::vector<std::unique_ptr<QgsLineString>> QgsLineString::splitToDisjointXYParts() const
{
  ...
  std::vector<std::unique_ptr<QgsLineString>> disjointParts;
  for ( size_t i = 0; i < allPointsCount; i++ )
  {
    ...
    if ( partPointSet.contains( point ) )
    {
      ...
      // current part
      disjointParts.push_back( std::make_unique<QgsLineString>( partX, partY ) );
