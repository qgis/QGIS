...
QImage QgsImageCache::pathAsImage( const QString &f, const QSize size, const bool keepAspectRatio, const double opacity, bool &fitsInCache, bool synchrone )
{
  ...
  if ( currentEntry->image.isNull() )
  {
    ...
    result = renderImage( file, size, keepAspectRatio, opacity, synchrone );
...
QSize QgsImageCache::originalSize( const QString &path, bool synchrone ) const
{
  if ( path.isEmpty() )
    return QSize();
  ...
  {
    QByteArray ba = getContent( path, QByteArray( "broken" ), QByteArray( "fetching" ), synchrone );
...
QImage QgsImageCache::renderImage( const QString &path, QSize size, const bool keepAspectRatio, const double opacity, bool synchrone ) const
{
  QImage im;
  // direct read if path is a file -- maybe more efficient than going the bytearray route? (untested!)
  if ( !path.startsWith( QStringLiteral( "base64:" ) ) && QFile::exists( path ) )
