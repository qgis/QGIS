  ...
  : QgsAuthConfigurationStorage( settings )
{
  ...
  QgsDebugMsgLevel( QStringLiteral( "Auth db connection settings: driver=%1, database='%2', host=%3, port=%4, user='%5', schema=%6, options=%7" )
                      .arg( mDriver, mDatabase, mHost, QString::number( mPort ), mUser, mConnectOptions, mConfiguration.value( QStringLiteral( "schema" ) ).toString() ),
                    2 );
}
  ...
  if ( !QSqlDatabase::contains( connectionName ) )
  {
    ...
    if ( QCoreApplication::instance() && QThread::currentThread() != QCoreApplication::instance()->thread() )
    {
      ...
      QMetaObject::Connection connection = connect(
        QThread::currentThread(), &QThread::finished, QThread::currentThread(), [connectionName, this] {
          QMutexLocker locker( &mMutex );
  ...
  if ( query.isActive() && query.isSelect() )
  {
    ...
    if ( query.next() )
    {
      ...
      emit messageLog( tr( "Authentication database contains more than one SSL cert custom configs for host:port, id: %1, %2" )
                         .arg( hostport, id ),
                       loggerTag(), Qgis::MessageLevel::Warning );
      return false;
    }
  }
  return res;
...
const QMap<QString, QVariant> QgsAuthConfigurationStorageDb::uriToSettings( const QString &uri )
{
  ...
  if ( url.isValid() )
  {
    ...
    if ( path.startsWith( QLatin1Char( '/' ) ) && !( settings.value( QStringLiteral( "driver" ) ) == QLatin1String( "QSQLITE" ) || settings.value( QStringLiteral( "driver" ) ) == QLatin1String( "QSPATIALITE" ) ) )
    {
      path = path.mid( 1 );
    }
