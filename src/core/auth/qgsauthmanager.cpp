...
const QgsSettingsEntryBool *QgsAuthManager::settingsUsingGeneratedRandomPassword
  = new QgsSettingsEntryBool( u"using-generated-random-password"_s, QgsSettingsTree::sTreeAuthentication, false, u"True if the user is using an autogenerated random password stored in the system keychain."_s );
const QgsSettingsEntryBool *QgsAuthManager::settingsPasswordHelperInsecureFallback = new QgsSettingsEntryBool( u"password_helper_insecure_fallback"_s, QgsSettingsTree::sTreeAuthentication, false );
const QgsSettingsEntryBool *QgsAuthManager::settingsUsePasswordHelper = new QgsSettingsEntryBool( u"use_password_helper"_s, QgsSettingsTree::sTreeAuthentication, true );
const QgsSettingsEntryBool *QgsAuthManager::settingsPasswordHelperLogging = new QgsSettingsEntryBool( u"password_helper_logging"_s, QgsSettingsTree::sTreeAuthentication, false );
...
QgsAuthManager::QgsAuthManager()
{
  ...
  connect( this, &QgsAuthManager::messageLog, this, &QgsAuthManager::writeToConsole );
#endif
}
...
bool QgsAuthManager::resetMasterPassword( const QString &newpass, const QString &oldpass, bool keepbackup, QString *backuppath )
{
#ifdef HAVE_AUTH
  ensureInitialized();
...
QgsAuthMethodsMap QgsAuthManager::authMethodsMap( const QString &dataprovider )
{
  ...
  {
    if ( i.value() && ( i.value()->supportedDataProviders().contains( u"all"_s ) || i.value()->supportedDataProviders().contains( dataprovider ) ) )
    {
...
bool QgsAuthManager::updateNetworkRequest( QNetworkRequest &request, const QString &authcfg, const QString &dataprovider )
{
#ifdef HAVE_AUTH
  ensureInitialized();
...
bool QgsAuthManager::updateNetworkReply( QNetworkReply *reply, const QString &authcfg, const QString &dataprovider )
{
#ifdef HAVE_AUTH
  ensureInitialized();
...
bool QgsAuthManager::updateDataSourceUriItems( QStringList &connectionItems, const QString &authcfg, const QString &dataprovider )
{
#ifdef HAVE_AUTH
  ensureInitialized();
...
const QPair<QSslCertificate, QSslKey> QgsAuthManager::certIdentityBundle( const QString &id )
{
  ...
  for ( QgsAuthConfigurationStorage *storage : std::as_const( storages ) )
  {
    if ( storage->certIdentityExists( id ) )
    {
      ...
      QSslKey key( QgsAuthCrypto::decrypt( mMasterPass, masterPasswordCiv(), encryptedBundle.second ).toLatin1(), QSsl::Rsa, QSsl::Pem, QSsl::PrivateKey );
      if ( key.isNull() )
      {
...
bool QgsAuthManager::updateIgnoredSslErrorsCacheFromConfig( const QgsAuthConfigSslServer &config )
{
  ...
  QString shahostport( u"%1:%2"_s.arg( QgsAuthCertUtils::shaHexForCert( config.sslCertificate() ).trimmed(), config.sslHostPort().trimmed() ) );
  if ( mIgnoredSslErrorsCache.contains( shahostport ) )
  {
    mIgnoredSslErrorsCache.remove( shahostport );
  }
...
void QgsAuthManager::writeToConsole( const QString &message, const QString &tag, Qgis::MessageLevel level )
{
#ifdef HAVE_AUTH
  Q_UNUSED( tag )
...
void QgsAuthManager::tryToStartDbErase()
{
  ...
  {
    QgsDebugMsgLevel( u"authDatabaseEraseRequest attempt (%1 of %2)"_s.arg( mScheduledDbEraseRequestCount ).arg( trycutoff ), 2 );
  }
...
bool QgsAuthManager::passwordHelperDelete()
{
  ...
  job.setInsecureFallback( settingsPasswordHelperInsecureFallback->value() );
...
bool QgsAuthManager::passwordHelperEnabled()
{
  ...
  return settingsUsePasswordHelper->value();
#else
  return false;
#endif
}
...
{
#ifdef HAVE_AUTH
  settingsUsePasswordHelper->setValue( enabled );
  emit messageLog(
    enabled ? tr( "Your %1 will be <b>used from now</b> on to store and retrieve the master password." ).arg( passwordHelperDisplayName() )
            : tr( "Your %1 will <b>not be used anymore</b> to store and retrieve the master password." ).arg( passwordHelperDisplayName() )
  );
#else
  Q_UNUSED( enabled )
#endif
...
#ifdef HAVE_AUTH
  // Does the user want to store the password in the wallet?
  return settingsPasswordHelperLogging->value();
#else
  return false;
#endif
}
...
{
#ifdef HAVE_AUTH
  settingsPasswordHelperLogging->setValue( enabled );
#else
  Q_UNUSED( enabled )
#endif
}
...
void QgsAuthManager::passwordHelperProcessError()
{
       ...
       || mPasswordHelperErrorCode == QKeychain::NotImplemented )
  {
    ...
    mPasswordHelperErrorMessage = tr(
  ...
  for ( const QgsAuthConfigurationStorage *storage : std::as_const( storages ) )
  {
    ...
    try
    {
      ...
      for ( auto it = configs.cbegin(); it != configs.cend(); ++it )
      {
        ...
        if ( configstring.isEmpty() )
        {
          QgsDebugError( u"Verify password can decrypt configs FAILED, could not decrypt a config (id: %1) from storage %2"_s.arg( it.key(), storage->name() ) );
          return false;
        }
      }
    }
...
bool QgsAuthManager::reencryptAuthenticationIdentity( const QString &identid, const QString &prevpass, const QString &prevciv )
{
#ifdef HAVE_AUTH
  ensureInitialized();
...
void QgsAuthManager::insertCaCertInCache( QgsAuthCertUtils::CaCertSource source, const QList<QSslCertificate> &certs )
{
  ...
  for ( const auto &cert : certs )
  {
    mCaCertsCache.insert( QgsAuthCertUtils::shaHexForCert( cert ), QPair<QgsAuthCertUtils::CaCertSource, QSslCertificate>( source, cert ) );
  }
#else
  Q_UNUSED( source )
  Q_UNUSED( certs )
