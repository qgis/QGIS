...
bool QgsAuthManager::initPrivate( const QString &pluginPath )
{
  ...
  {
    authConfigurationStorageRegistry()->addStorage( new QgsAuthConfigurationStorageDb( mAuthDatabaseConnectionUri ) );
  }
...
QgsAuthMethodConfigsMap QgsAuthManager::availableAuthMethodConfigs( const QString &dataprovider )
{
  ...
  else
  {
    ...
    QgsDebugError( QStringLiteral( "No default storage found" ) );
  }
...
void QgsAuthManager::updateConfigAuthMethods()
{
  ...
  const QList<QgsAuthConfigurationStorage *> storages { authConfigurationStorageRegistry()->readyStorages() };
  for ( QgsAuthConfigurationStorage *storage : std::as_const( storages ) )
  {
...
bool QgsAuthManager::updateAuthenticationConfig( const QgsAuthMethodConfig &config )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    ...
    if ( !defaultStorage->storeMethodConfig( config, configstring ) )
    {
      emit messageLog( tr( "Store config: FAILED to store config in default storage: %1" ).arg( defaultStorage->lastError() ), authManTag(), Qgis::MessageLevel::Critical );
      return false;
    }
  }
  else
...
bool QgsAuthManager::loadAuthenticationConfig( const QString &authcfg, QgsAuthMethodConfig &config, bool full )
{
  ...
  {
    QString payload;
    config = defaultStorage->loadMethodConfig( authcfg, payload, full );

    if ( ! config.isValid( true ) || ( full && payload.isEmpty() ) )
    {
      emit messageLog( tr( "Load config: FAILED to load config %1 from default storage: %2" ).arg( authcfg, defaultStorage->lastError() ), authManTag(), Qgis::MessageLevel::Critical );
      return false;
    }
...
bool QgsAuthManager::removeAuthenticationConfig( const QString &authcfg )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    ...
    {
      QgsDebugMsgLevel( QStringLiteral( "FAILED to remove config for authcfg: %1" ).arg( authcfg ), 2 );
      return false;
    }
  }
  else
...
bool QgsAuthManager::removeAllAuthenticationConfigs()
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    if ( defaultStorage->clearMethodConfigs() )
    {
      ...
      updateConfigAuthMethods();
      QgsDebugMsgLevel( QStringLiteral( "REMOVED all configs" ), 2 );
      return true;
    }
    else
    {
      QgsDebugMsgLevel( QStringLiteral( "FAILED to remove all configs" ), 2 );
      return false;
    }
  }
  else
...
QVariant QgsAuthManager::authSetting( const QString &key, const QVariant &defaultValue, bool decrypt )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    QString storeval = defaultStorage->loadAuthSetting( key );
    if ( !storeval.isEmpty() )
    {
      if ( decrypt )
      {
...
bool QgsAuthManager::existsAuthSetting( const QString &key )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    return defaultStorage->authSettingExists( key );
  }
  else
  {
    emit messageLog( tr( "Could not connect to the default storage." ), authManTag(), Qgis::MessageLevel::Critical );
...
bool QgsAuthManager::removeAuthSetting( const QString &key )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    if ( defaultStorage->removeAuthSetting( key ) )
    {
    ...
    {
      QgsDebugMsgLevel( QStringLiteral( "FAILED to remove setting for key: %1" ).arg( key ), 2 );
      return false;
    }
  }
  else
...
const QSslCertificate QgsAuthManager::certIdentity( const QString &id )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    return defaultStorage->loadCertIdentity( id );
  }
  else
  {
    emit messageLog( tr( "Could not connect to the default storage." ), authManTag(), Qgis::MessageLevel::Critical );
...
const QPair<QSslCertificate, QSslKey> QgsAuthManager::certIdentityBundle( const QString &id )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    QPair<QSslCertificate, QString> encryptedBundle { defaultStorage->loadCertIdentityBundle( id ) };
...
const QList<QSslCertificate> QgsAuthManager::certIdentities()
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    certs = defaultStorage->certIdentities();
  }
...
QStringList QgsAuthManager::certIdentityIds() const
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    return defaultStorage->certIdentityIds();
  }
...
bool QgsAuthManager::existsCertIdentity( const QString &id )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    return defaultStorage->certIdentityExists( id );
  }
  else
  {
    emit messageLog( tr( "Could not connect to the default storage." ), authManTag(), Qgis::MessageLevel::Critical );
...
bool QgsAuthManager::removeCertIdentity( const QString &id )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    if ( defaultStorage->removeCertIdentity( id ) )
    {
    ...
    {
      QgsDebugMsgLevel( QStringLiteral( "FAILED to remove certificate identity for id: %1" ).arg( id ), 2 );
      return false;
    }
  }
  else
...
const QgsAuthConfigSslServer QgsAuthManager::sslCertCustomConfig( const QString &id, const QString &hostport )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    return defaultStorage->loadSslCertCustomConfig( id, hostport );
  }
  else
  {
    emit messageLog( tr( "Could not connect to the default storage." ), authManTag(), Qgis::MessageLevel::Critical );
...
const QgsAuthConfigSslServer QgsAuthManager::sslCertCustomConfigByHost( const QString &hostport )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    config = defaultStorage->loadSslCertCustomConfigByHost( hostport );
  }
  else
  {
    emit messageLog( tr( "Could not connect to the default storage." ), authManTag(), Qgis::MessageLevel::Critical );
...
const QList<QgsAuthConfigSslServer> QgsAuthManager::sslCertCustomConfigs()
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    configs = defaultStorage->sslCertCustomConfigs();
  }
  else
  {
    emit messageLog( tr( "Could not connect to the default storage." ), authManTag(), Qgis::MessageLevel::Critical );
...
bool QgsAuthManager::existsSslCertCustomConfig( const QString &id, const QString &hostport )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    return defaultStorage->sslCertCustomConfigExists( id, hostport );
  }
  else
  {
    emit messageLog( tr( "Could not connect to the default storage." ), authManTag(), Qgis::MessageLevel::Critical );
...
bool QgsAuthManager::removeSslCertCustomConfig( const QString &id, const QString &hostport )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    if ( defaultStorage->removeSslCertCustomConfig( id, hostport ) )
    {
    ...
    {
      QgsDebugMsgLevel( QStringLiteral( "FAILED to remove SSL cert custom config for host:port, id: %1, %2" ).arg( hostport, id ), 2 );
      return false;
    }
  }
  else
...
bool QgsAuthManager::rebuildIgnoredSslErrorCache()
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    const auto customConfigs { defaultStorage->sslCertCustomConfigs() };
...
const QSslCertificate QgsAuthManager::certAuthority( const QString &id )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    return defaultStorage->loadCertAuthority( id );
  }
  else
  {
    emit messageLog( tr( "Could not connect to the default storage." ), authManTag(), Qgis::MessageLevel::Critical );
...
bool QgsAuthManager::existsCertAuthority( const QSslCertificate &cert )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    return defaultStorage->certAuthorityExists( cert );
  }
  else
  {
    emit messageLog( tr( "Could not connect to the default storage." ), authManTag(), Qgis::MessageLevel::Critical );
...
bool QgsAuthManager::removeCertAuthority( const QSslCertificate &cert )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    return defaultStorage->removeCertAuthority( cert );
  }
  else
  {
    emit messageLog( tr( "Could not connect to the default storage." ), authManTag(), Qgis::MessageLevel::Critical );
...
QgsAuthCertUtils::CertTrustPolicy QgsAuthManager::certTrustPolicy( const QSslCertificate &cert )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    return defaultStorage->loadCertTrustPolicy( cert );
  }
  else
  {
    emit messageLog( tr( "Could not connect to the default storage." ), authManTag(), Qgis::MessageLevel::Critical );
...
bool QgsAuthManager::removeCertTrustPolicy( const QSslCertificate &cert )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    return defaultStorage->removeCertTrustPolicy( cert );
  }
  else
  {
    emit messageLog( tr( "Could not connect to the default storage." ), authManTag(), Qgis::MessageLevel::Critical );
...
bool QgsAuthManager::rebuildCertTrustCache()
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    const auto trustedCerts { defaultStorage->caCertsPolicy() };
...
bool QgsAuthManager::masterPasswordRowsInDb( int *rows ) const
{
  ...
  if ( QgsAuthConfigurationStorageDb *defaultStorage = defaultDbStorage() )
  {
    *rows = defaultStorage->masterPasswords( ).count();
    return true;
  }
  else
  {
...
bool QgsAuthManager::reencryptAuthenticationConfig( const QString &authcfg, const QString &prevpass, const QString &prevciv )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    ...
    QString payload;
    const QgsAuthMethodConfig config = defaultStorage->loadMethodConfig( authcfg, payload, true );
  ...
  const QString &prevciv )
{
  ...
  if ( QgsAuthConfigurationStorage *defaultStorage = defaultDbStorage() )
  {
    ...
    const QPair<QSslCertificate, QString> identityBundle = defaultStorage->loadCertIdentityBundle( identid );
    ...
    return defaultStorage->storeCertIdentity( identityBundle.first, keystring );
  }
  else
  {
