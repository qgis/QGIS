...
QgsAuthMethodsMap QgsAuthManager::authMethodsMap( const QString &dataprovider )
{
  ...
  {
    if ( i.value()
         && ( i.value()->supportedDataProviders().contains( u"all"_s ) || i.value()->supportedDataProviders().contains( dataprovider ) ) )
    {
...
bool QgsAuthManager::updateIgnoredSslErrorsCacheFromConfig( const QgsAuthConfigSslServer &config )
{
  ...
  QString shahostport( u"%1:%2"_s
                         .arg( QgsAuthCertUtils::shaHexForCert( config.sslCertificate() ).trimmed(), config.sslHostPort().trimmed() ) );
  if ( mIgnoredSslErrorsCache.contains( shahostport ) )
  {
    mIgnoredSslErrorsCache.remove( shahostport );
  }
...
void QgsAuthManager::setPasswordHelperEnabled( const bool enabled )
{
  ...
  emit messageLog( enabled ? tr( "Your %1 will be <b>used from now</b> on to store and retrieve the master password." ).arg( passwordHelperDisplayName() ) : tr( "Your %1 will <b>not be used anymore</b> to store and retrieve the master password." ).arg( passwordHelperDisplayName() ) );
#else
  Q_UNUSED( enabled )
#endif
}
