...
{
  mPayloadData = data;
  const QgsBlockingNetworkRequest::ErrorCode res = doRequest( Post, request, forceRefresh, feedback );
...
{
  mPayloadData = data;
  const QgsBlockingNetworkRequest::ErrorCode res = doRequest( Put, request, true, feedback );
  mPayloadData = nullptr;
  return res;
}
