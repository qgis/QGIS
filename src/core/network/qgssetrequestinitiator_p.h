...
#define QgsSetRequestInitiatorClass( request, _class )                                                                                   \
  ...
  ( request )                                                                                                                            \
    .setAttribute( static_cast< QNetworkRequest::Attribute >( QgsNetworkRequestParameters::AttributeInitiatorRequestId ), QString( QString( __FILE__ ).mid( sFilePrefixLength ) + ':' + QString::number( __LINE__ ) + " (" + ( __FUNCTION__ ) + ")" ) );
#define QgsSetRequestInitiatorId( request, str )                                                                                            \
  ( request ).setAttribute(                                                                                                                 \
    static_cast< QNetworkRequest::Attribute >( QgsNetworkRequestParameters::AttributeInitiatorRequestId ),                                  \
    QString( QString( __FILE__ ).mid( sFilePrefixLength ) + ':' + QString::number( __LINE__ ) + " (" + ( __FUNCTION__ ) + "): " + ( str ) ) \
  );
#else
#define QgsSetRequestInitiatorClass( request, _class ) \
  ( void ) ( request );                                \
