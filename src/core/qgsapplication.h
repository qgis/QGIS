...
class CORE_EXPORT QgsApplication : public QApplication {
  ...
  % TypeCode
      ...
      // array.
      static char **
      qtgui_ArgvToC(PyObject *argvlist, int &argc) {
    char **argv;
  ...
  % End
       ...
       */
      enum StyleSheetType {
        Qt,         //!< StyleSheet for Qt GUI widgets (based on QLabel or
                    //!< QTextBrowser), supports basic CSS and Qt extensions
   ...
   * for a console only application \param profileFolder optional string
   * representing the profile to load at startup \param platformName the QGIS
   * platform name, e.g., "desktop", "server", "qgis_process" or "external" (for
  ...
  /**
   * This method initializes paths etc for QGIS. Called by the ctor or call it
   * manually when your app does not extend the QApplication class. \note you
   * will probably want to call initQgis too to load the providers in the above
   ...
   * "/" but plugin writers etc can use themeName() as a basis for searching for
   * resources in their own datastores e.g. a Qt4 resource bundle. \note A basic
   * test will be carried out to ensure the theme search path based on the
   * supplied theme name exists. If it does not the theme name will be reverted
   * to 'default'.
   */
   ...
   * The theme name should be a single word e.g. 'default','classic'.
   * The theme search path usually will be pkgDataPath + "/themes/" + themName +
   * "/" but plugin writers etc can use this method as a basis for searching for
   * resources in their own datastores e.g. a Qt4 resource bundle.
   */
  static QString themeName();

  /**
   * \brief Set the current UI theme used to style the interface.  Use
   * uiThemes() to find valid themes to use. Variables found in variables.qss
   * will be added to the stylesheet on load. \param themeName The name of the
   ...
   * \returns A hash of theme name and theme path. Valid theme folders contain
   ...
   * where DATABASE is just the path to the file for SQLite databases. If DRIVER
   * is omitted, PSQLITE is assumed. Optional SCHEMA can be specified as a query
   * parameter. \since QGIS 3.40
   */
   ...
   *
   * Since QGIS 3.20, the optional \a fillColor and \a strokeColor arguments can
   * be used to control the color of parameter based SVG icons.
   */
   ...
   * \note This functionality was previously available as `shortNameRegExp` for
   * QGIS <= 3.20 \since QGIS 3.22
   */
  ...
  /**
   * Returns the QGIS platform name, e.g., "desktop", "server", "qgis_process"
   * or "external" (for external CLI scripts). \see osName()
   */
   ...
   * \note if styleSheetType equals StyleSheetType::Qt you can use the special
   * Qt extensions too, for example the gradient fills for backgrounds.
   */
  static QString
  reportStyleSheet(QgsApplication::StyleSheetType styleSheetType =
                       QgsApplication::StyleSheetType::Qt);
   ...
   * of registered drivers when QgsApplication::registerOgrDrivers was called
   * multiple times.
   */
  static void registerOgrDrivers();

  //! Converts absolute path to path relative to target
  static QString absolutePathToRelativePath(const QString &apath,
                                            const QString &targetPath);
  //! Converts path relative to target to an absolute path
  static QString relativePathToAbsolutePath(const QString &rpath,
                                            const QString &targetPath);
   ...
   * setting. \since QGIS 3.10
   */
  static void registerGdalDriversFromSettings();

  /**
   * Returns the list of gdal drivers that have been disabled in the current
   * session, and thus, for safety, should not be disabled right now, but at the
   ...
   * deferredSkippedGdalDrivers. This writes the "gdal/skipList" setting. \since
   * QGIS 3.10
   */
  static void
  setSkippedGdalDrivers(const QStringList &skippedGdalDrivers,
   ...
   * settings. \since QGIS 3.20 \deprecated QGIS 3.30. Use
   * QgsSettings::treeRoot() instead.
   */
  Q_DECL_DEPRECATED static QgsSettingsRegistryCore *
  settingsRegistryCore() SIP_KEEPREFERENCE SIP_DEPRECATED;
   ...
   * point cloud layer 2D renderers. \since QGIS 3.18
   */
   ...
   * tiled scene layer 2D renderers. \since QGIS 3.34
   */
   ...
   * handles known CRS definitions (including user-defined CRSes).
   *
   * \since QGIS 3.18
   */
   ...
   * temporary files during QGIS session \since QGIS 3.2
   */
   ...
   * validity checks. \since QGIS 3.6
   */
   ...
   * GPSBabel formats. \since QGIS 3.22
   */
   ...
   * graduated renderer \since QGIS 3.10
   */
   ...
   * installation-wide bookmarks. \since QGIS 3.10
   */
  static QgsBookmarkManager *bookmarkManager();

  /**
   * Returns the application's tile download manager, used for download of map
   * tiles when rendering. \note not available in Python bindings \since
   * QGIS 3.18
   */
   ...
   *
   * Using this shared model instead of creating a new QgsStyleModel improves
   * performance.
   *
   * \since QGIS 3.10
   */
   ...
   * \since QGIS 3.40
   */
  static QgsAuthConfigurationStorageRegistry *
   ...
   * provider connections. \since QGIS 3.14
   */
   ...
   *
   * \note In general, when passing values around, prefer to use an invalid
   * QVariant. The nullRepresentation() value should only be used in the final
   * presentation step when showing values in a widget or sending it to a web
   * browser.
   *
   ...
   * This does not include generated variables (like system name, user name
   * etc.)
   *
   ...
   *
   * \a standardSize should be set to a standard icon size, e.g. 16, 24, 48,
   * etc.
   *
   * \since QGIS 3.16
   */
...
private:
  ...
  // ... but in case QgsApplication is never instantiated (eg with custom
  // designer widgets), we fall back to static members
