  ...
  , mNewValue( value )
{
  ...
  std::function mapFn = [editIndex, attribute, index]( std::pair<const QgsPointCloudNodeId &, const QVector<int> &> pair )
  {
...
void QgsPointCloudLayerUndoCommandChangeAttribute::undoRedoPrivate( bool isUndo )
{
    ...
    )
  {
    ...
    if ( editIndex->mEditedNodeData.contains( node ) )
    {
      chunkData = editIndex->mEditedNodeData[node];
    }
    else
    {
      QPair<uint64_t, int32_t> offsetSizePair = copcIndex->mHierarchyNodePos[node];
    ...
    {
      editIndex->mEditedNodeData.remove( node );
    }
    else if ( isUndo )
    {
