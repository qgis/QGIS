...
QList<QColor> QgsSymbolLayerUtils::parseColorList( const QString &colorStr )
{
  ...
  QStringList components = colorStr.simplified().split( QRegularExpression( "(,|\\s)" ) );
  ...
  components = colorStr.split( QRegularExpression( "(,|\n)" ) );
  it = components.begin();
  for ( ; it != components.end(); ++it )
  {
    QColor result = parseColor( *it, true );
...
QgsNamedColorList QgsSymbolLayerUtils::importColorsFromGpl( QFile &file, bool &ok, QString &name )
{
  ...
  {
    QRegularExpressionMatch match = QRegularExpression( "Name:\\s*(\\S.*)$" ).match( line );
    if ( match.hasMatch() )
    {
  ...
  QRegularExpression rx( "^\\s*(\\d+)\\s+(\\d+)\\s+(\\d+)(\\s.*)?$" );
