...
bool QgsVectorFileWriter::driverMetadata( const QString &driverName, QgsVectorFileWriter::MetaData &driverMetadata )
{
  ...
  {
    if ( it.key() == QLatin1String( "PGDUMP" ) && driverName != QLatin1String( "PGDUMP" ) && driverName != QLatin1String( "PostgreSQL SQL dump" ) )
    {
...
QgsVectorFileWriter::WriterError QgsVectorFileWriter::writeAsVectorFormatV2( PreparedWriterDetails &details, const QString &fileName, const QgsCoordinateTransformContext &transformContext, const QgsVectorFileWriter::SaveVectorOptions &options, QString *newFilename, QString *newLayer, QString *errorMessage, SinkFlags sinkFlags )
{
  ...
  if ( details.providerType == QLatin1String( "ogr" ) && !details.dataSourceUri.isEmpty() )
  {
    ...
    if ( QFile::exists( srcFileName ) && QFileInfo( fileName ).canonicalFilePath() == QFileInfo( srcFileName ).canonicalFilePath() )
    {
      ...
      if ( !( ( options.driverName == QLatin1String( "GPKG" ) || options.driverName == QLatin1String( "SpatiaLite" ) || options.driverName == QLatin1String( "SQLite" ) ) && options.layerName != details.providerUriParams.value( QStringLiteral( "layerName" ) ) ) )
      {
        if ( errorMessage )
...
QString QgsVectorFileWriter::filterForDriver( const QString &driverName )
{
  ...
  return QStringLiteral( "%1 (%2 %3)" ).arg( metadata.trLongName, metadata.glob.toLower(), metadata.glob.toUpper() );
}
