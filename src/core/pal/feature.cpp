...
int FeaturePart::createCurvedCandidatesAlongLine( QList< LabelPosition * > &lPos, PointSet *mapShape )
{
  ...
  for ( double distanceAlongLineToStartCandidate = 0; distanceAlongLineToStartCandidate < total_distance; distanceAlongLineToStartCandidate += delta )
  {
    ...
    LabelPosition *slp = curvedPlacementAtOffset( mapShape, path_distances, orientation, distanceAlongLineToStartCandidate, reversed, flip );
    if ( !slp )
      continue;
    ...
    if ( slp->upsideDownCharCount() >= li->char_num / 2.0 )
    {
      ...
      if ( ( showUprightLabels() && !flip ) )
      {
        ...
        slp = curvedPlacementAtOffset( mapShape, path_distances, orientation, distanceAlongLineToStartCandidate, reversed, flip );
      }
    }
    if ( !slp )
      continue;
