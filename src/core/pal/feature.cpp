  ...
  for ( int i = 0; i < straightSegmentLengths.count(); ++i )
  {
    ...
    while ( currentDistanceAlongLine + labelWidth < distanceToEndOfSegment )
    {
      ...
      if ( mLF->layer()->arrangement() == QgsPalLayerSettings::Line )
      {
        ...
        {
          if ( !mLF->permissibleZonePrepared() || GeomFunction::containsCandidate( mLF->permissibleZonePrepared(), candidateStartX - ( mLF->originalDistLabel() + labelHeight ) * std::cos( beta ) / 2, candidateStartY - ( mLF->originalDistLabel() + labelHeight ) * std::sin( beta ) / 2, labelWidth, labelHeight, angle ) )
          {
            const double candidateCost = cost + 0.002;
            lPos.emplace_back( qgis::make_unique< LabelPosition >( i, candidateStartX - ( mLF->originalDistLabel() + labelHeight ) * std::cos( beta ) / 2, candidateStartY - ( mLF->originalDistLabel() + labelHeight ) * std::sin( beta ) / 2, labelWidth, labelHeight, angle, candidateCost, this, isRightToLeft, LabelPosition::QuadrantOver ) ); // Line
          }
        }
      }
...
std::size_t FeaturePart::createCurvedCandidatesAlongLine( std::vector< std::unique_ptr< LabelPosition > > &lPos, PointSet *mapShape, bool allowOverrun, Pal *pal )
{
  ...
  for ( ; distanceAlongLineToStartCandidate <= total_distance; distanceAlongLineToStartCandidate += delta )
  {
    ...
    for ( int i = 0; i <= 2; ++i )
    {
      ...
      {
        p = _createCurvedCandidate( slp.get(), angle_avg, mLF->originalDistLabel() );
