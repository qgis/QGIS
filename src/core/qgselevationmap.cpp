  ...
  for ( int i = 0; i < imgWidth; ++i )
  {
    for ( int j = 0; j < imgHeight; ++j )
    {
      ...
      imgPtr[index] = qRgba( static_cast<int>( static_cast<float>( qRed( c ) ) * shade ),
                             ...
                             static_cast<int>( static_cast<float>( qBlue( c ) ) * shade ), qAlpha( c ) );
    }
  }
}
    ...
    for ( int colC = 0; colC < imgWidth; ++colC )
    {
      ...
      if ( elevPtr[centerIndex] != 0 )
      {
        ...
        if ( !multiDirectional )
        {
          ...
          shade = std::clamp( ( sin_altRadian - ( derY * cos_az_mul_cos_alt_mul_z - derX * sin_az_mul_cos_alt_mul_z ) ) / std::sqrt( 1 + square_z * ( derX * derX + derY * derY ) ),
                              0.0, 1.0 );
        }
        else
        {
          ...
          else
          {
            ...
            const double cang_mul_127 = ( ( weight_225 * val225_mul_0_5 + weight_270 * val270_mul_0_5 + weight_315 * val315_mul_0_5 + weight_360 * val360_mul_0_5 ) / xx_plus_yy ) / ( 1 + square_z * xx_plus_yy );
