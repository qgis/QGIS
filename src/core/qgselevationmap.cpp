...
void QgsElevationMap::applyEyeDomeLighting( QImage &img, int distance, float strength, float rendererScale ) const
{
  ...
  for ( int i = 0; i < imgWidth; ++i )
  {
    for ( int j = 0; j < imgHeight; ++j )
    {
      ...
      QRgb c = imgPtr[index];
      imgPtr[index]
        = qRgba( static_cast<int>( static_cast<float>( qRed( c ) ) * shade ), static_cast<int>( static_cast<float>( qGreen( c ) ) * shade ), static_cast<int>( static_cast<float>( qBlue( c ) ) * shade ), qAlpha( c ) );
    }
  }
}
...
void QgsElevationMap::applyHillshading( QImage &img, bool multiDirectional, double altitude, double azimuth, double zFactor, double cellSizeX, double cellSizeY ) const
{
  ...
  auto colRowToIndex = [&]( int c, int r ) -> qgssize { return static_cast<qgssize>( r ) * imgWidth + c; };
    ...
    for ( int colC = 0; colC < imgWidth; ++colC )
    {
      ...
      if ( elevPtr[centerIndex] != 0 )
      {
        ...
        if ( !multiDirectional )
        {
          ...
          shade = std::clamp( ( sin_altRadian - ( derY * cos_az_mul_cos_alt_mul_z - derX * sin_az_mul_cos_alt_mul_z ) ) / std::sqrt( 1 + square_z * ( derX * derX + derY * derY ) ), 0.0, 1.0 );
        }
        else
        {
          ...
          else
          {
            ...
            double val225_mul_0_5 = sin_altRadian * 0.5 + ( derX - derY ) * cos225_az_mul_cos_alt_mul_z_mul_0_5;
            val225_mul_0_5 = ( val225_mul_0_5 <= 0.0 ) ? 0.0 : val225_mul_0_5;
            double val270_mul_0_5 = sin_altRadian * 0.5 - derX * cos_alt_mul_z * 0.5;
            val270_mul_0_5 = ( val270_mul_0_5 <= 0.0 ) ? 0.0 : val270_mul_0_5;
            double val315_mul_0_5 = sin_altRadian * 0.5 + ( derX + derY ) * cos225_az_mul_cos_alt_mul_z_mul_0_5;
            val315_mul_0_5 = ( val315_mul_0_5 <= 0.0 ) ? 0.0 : val315_mul_0_5;
            double val360_mul_0_5 = sin_altRadian * 0.5 - derY * cos_alt_mul_z * 0.5;
            ...
            const double cang_mul_127 = ( ( weight_225 * val225_mul_0_5 + weight_270 * val270_mul_0_5 + weight_315 * val315_mul_0_5 + weight_360 * val360_mul_0_5 ) / xx_plus_yy )
