...
{
  QDomElement valueElement = document.createElement( QStringLiteral( "property" ) );
...
bool QgsProjectPropertyKey::readXml( const QDomNode &keyNode )
{
  ...
  while ( i < subkeys.count() )
  {
    ...
    if ( subkey.nodeName() == QStringLiteral( "property" ) &&
...
bool QgsProjectPropertyKey::writeXml( QString const &nodeName, QDomElement &element, QDomDocument &document )
{
  ...
  QDomElement keyElement = document.createElement( "property" ); // Dom element for this property key
