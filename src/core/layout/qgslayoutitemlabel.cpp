...
void QgsLayoutItemLabel::draw( QgsLayoutItemRenderContext &context )
{
  ...
  switch ( mMode )
  {
    ...
    case ModeFont:
    {
      ...
      QgsTextRenderer::drawText( painterRect, 0, QgsTextRenderer::convertQtHAlignment( mHAlignment ), currentText(), context.renderContext(), mFormat, true, QgsTextRenderer::convertQtVAlignment( mVAlignment ) );
      break;
    }
  }
}
...
{
  const double textWidth = QgsLayoutUtils::textWidthMM( mFormat.font(), currentText() );
  const double fontHeight = QgsLayoutUtils::fontHeightMM( mFormat.font() );
