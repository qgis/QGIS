...
QgsCoordinateReferenceSystem::QgsCoordinateReferenceSystem( const QgsCoordinateReferenceSystem &srs )  //NOLINT
  : d( srs.d )
    =======
      QgsCoordinateReferenceSystem::QgsCoordinateReferenceSystem( const QgsCoordinateReferenceSystem &srs ) //NOLINT
        : d( srs.d )
        >>>>>>> fixes #21575. Used astyle.sh on \src\core\qgscoordinatereferencesystem.cpp to hopefully pass code style validation
        =======
          QgsCoordinateReferenceSystem::QgsCoordinateReferenceSystem( const QgsCoordinateReferenceSystem &srs ) //NOLINT
            : d( srs.d )
            >>>>>>> fixes #21575. Used astyle.sh - now with '--indent=spaces=2' on \src\core\qgscoordinatereferencesystem.cpp to hopefully pass code style validation
{
}
...
bool QgsCoordinateReferenceSystem::createFromId( const long id, CrsType type )
{
  ...
  return result;
  >>>>>>> fixes #21575. Used astyle.sh - now using astyle v31 on \src\core\qgscoordinatereferencesystem.cpp to hopefully pass code style validation
}
...
bool QgsCoordinateReferenceSystem::createFromOgcWmsCrs( const QString &crs )
{
  ...
  QRegExp reUri( "http://www\\.opengis\\.net/def/crs/([^/]+).+/(\\d+)", Qt::CaseInsensitive );
  QRegExp reUrn( "urn:ogc:def:crs:([^:]+).+([^:]+)", Qt::CaseInsensitive );
  if ( reUri.exactMatch( wmsCrs ) )
  {
    wmsCrs = reUri.cap( 1 ) + ':' + reUri.cap( 2 );
  }
  else if ( reUrn.exactMatch( wmsCrs ) )
  {
    wmsCrs = reUrn.cap( 1 ) + ':' + reUrn.cap( 2 );
  }
  else
  {
    reUrn.setPattern( QStringLiteral( "(user|custom|qgis):(\\d+)" ) );
    if ( reUrn.exactMatch( wmsCrs ) && createFromSrsId( reUrn.cap( 2 ).toInt() ) )
    {
