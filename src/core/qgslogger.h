...
class CORE_EXPORT QgsLogger
{
  public:
    ...
    *
    * Returns an empty string if the variable is not set, otherwise returns a
    * file name for writing log messages to.
    *
    * \note Calling logFile() also initializes file logging and reads related
    * environment variables such as QGIS_DEBUG and QGIS_DEBUG_FILE. As a result,
    * logFile() should be called before logMessageToFile() to ensure that messages
