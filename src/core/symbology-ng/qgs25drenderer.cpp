...
QDomElement Qgs25DRenderer::save( QDomDocument &doc, const QgsPathResolver &pathResolver )
{
  QDomElement rendererElem = doc.createElement( RENDERER_TAG_NAME );

  rendererElem.setAttribute( QStringLiteral( "type" ), QStringLiteral( "25dRenderer" ) );

  QDomElement symbolElem = QgsSymbolLayerUtils::saveSymbol( QStringLiteral( "symbol" ), mSymbol.get(), doc, pathResolver );
...
QgsFeatureRenderer *Qgs25DRenderer::create( QDomElement &element, const QgsPathResolver &pathResolver )
{
  ...
  if ( symbols.size() )
  {
    renderer->mSymbol.reset( QgsSymbolLayerUtils::loadSymbol( symbols.at( 0 ).toElement(), pathResolver ) );
  }
