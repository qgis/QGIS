...
class VectorStream
{
  public:
    ...
    size_t remaining() const { return mVec.size() - mOffset; }
...
static std::vector<uint32_t> parseU32OrU16Array( VectorStream &stream, bool isU32, size_t countMult )
{
  ...
  if ( isU32 )
  {
    ...
    values.resize( count );
    const uint32_t *valuesRaw = reinterpret_cast<const uint32_t *>( stream.read( sizeof( uint32_t ) * count ) );
  ...
  else
  {
    ...
    size_t count = static_cast<size_t>( *countBase ) * countMult;
    values.resize( count );
    const uint16_t *valuesRaw = reinterpret_cast<const uint16_t *>( stream.read( sizeof( uint16_t ) * count ) );
    std::copy( valuesRaw, valuesRaw + count, values.begin() );
  }
  return values;
}
...
QgsQuantizedMeshTile::QgsQuantizedMeshTile( const QByteArray &data )
{
  ...
  if ( reinterpret_cast<char *>( &endiannessCheck )[0] == 0x10 )
    throw QgsNotSupportedException( "Parsing quantized mesh tiles on big endian machines is not supported" );

  VectorStream stream( data );
  mHeader = *reinterpret_cast<const QgsQuantizedMeshHeader *>( stream.read( sizeof( QgsQuantizedMeshHeader ) ) );

  auto uArr = reinterpret_cast<const uint16_t *>( stream.read( sizeof( uint16_t ) * mHeader.vertexCount ) );
  auto vArr = reinterpret_cast<const uint16_t *>( stream.read( sizeof( uint16_t ) * mHeader.vertexCount ) );
  auto heightArr = reinterpret_cast<const uint16_t *>( stream.read( sizeof( uint16_t ) * mHeader.vertexCount ) );
  ...
  while ( stream.remaining() > 0 )
  {
    ...
    if ( extensionId == normalsExtensionId )
    {
      ...
      for ( size_t i = 0; i < mHeader.vertexCount; i++ )
      {
        auto normal = oct16Decode( *reinterpret_cast<const uint8_t *>( stream.read( sizeof( char ) ) ), *reinterpret_cast<const uint8_t *>( stream.read( sizeof( char ) ) ) );
...
tinygltf::Model QgsQuantizedMeshTile::toGltf( bool addSkirt, double skirtDepth, bool withTextureCoords )
{
  ...
  if ( addSkirt )
  {
    ...
    std::sort( mWestVertices.begin(), mWestVertices.end(), [&]( uint32_t a, uint32_t b ) { return mVertexCoords[a * 3 + 1] < mVertexCoords[b * 3 + 1]; } );
    std::sort( mSouthVertices.begin(), mSouthVertices.end(), [&]( uint32_t a, uint32_t b ) { return mVertexCoords[a * 3] > mVertexCoords[b * 3]; } );
    std::sort( mEastVertices.begin(), mEastVertices.end(), [&]( uint32_t a, uint32_t b ) { return mVertexCoords[a * 3 + 1] > mVertexCoords[b * 3 + 1]; } );
    std::sort( mNorthVertices.begin(), mNorthVertices.end(), [&]( uint32_t a, uint32_t b ) { return mVertexCoords[a * 3] < mVertexCoords[b * 3]; } );
