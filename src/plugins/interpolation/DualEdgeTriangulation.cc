...
int DualEdgeTriangulation::insertForcedSegment(int p1, int p2, bool breakline)
{
  ...
  while(true)//if it's an endless loop, something went wrong
	...
	{
	  if(mHalfEdge[mHalfEdge[actedge]->getNext()]->getForced()&&mForcedCrossBehaviour==Triangulation::SNAP_TO_VERTICE)//if the crossed edge is a forced edge, we have to snap the forced line to the next node
	    {
	      Point3D crosspoint;
	      int p3,p4;
  ...
  while(true)//if its an endless loop, something went wrong.
    {
      ...
      {
	if(mHalfEdge[mHalfEdge[mHalfEdge[crossedEdges.last()]->getDual()]->getNext()]->getForced()&&mForcedCrossBehaviour==Triangulation::SNAP_TO_VERTICE)//if the crossed edge is a forced edge and mForcedCrossBehaviour is SNAP_TO_VERTICE, we have to snap the forced line to the next node
	  {
	    Point3D crosspoint;
	    int p3,p4;
	...
	{
	  if(mHalfEdge[mHalfEdge[mHalfEdge[mHalfEdge[crossedEdges.last()]->getDual()]->getNext()]->getNext()]->getForced()&&mForcedCrossBehaviour==Triangulation::SNAP_TO_VERTICE)//if the crossed edge is a forced edge and mForcedCrossBehaviour is SNAP_TO_VERTICE, we have to snap the forced line to the next node
	    {
	      Point3D crosspoint;
	      int p3,p4;
...
void DualEdgeTriangulation::ruppertRefinement()
{
	...
	{
	    std::cout << "angle: " << it->first << std::endl << std::flush;
	}
	...
	while(!edge_angle.empty())
	{
	    ...
	    std::cout << "minangle: " << minangle << std::endl << std::flush;
	    ...
	    {
		std::cout << "warning, calculation of circumcenter failed in DualEdgeTriangulation::ruppertRefinement" << std::endl << std::flush; 
		...
		if(mHalfEdge[i]->getForced()||edgeOnConvexHull(i))
		{
			...
			encroached=true;
			//split segment
			std::cout << "segment split" << std::endl << std::flush;
			int pointno=splitHalfEdge(i,0.5);
		...
		if(pointno==-100)
		{
		    std::cout << "put circumcenter " << circumcenter.getX() << "//" << circumcenter.getY() << "on dontexamine list because of numerical instabilities" << std::endl << std::flush;
		}
		else if(pointno==mTwiceInsPoint)
		    {
			std::cout << "put circumcenter " << circumcenter.getX() << "//" << circumcenter.getY() << "on dontexamine list because it is  already inserted" << std::endl << std::flush;
	    ...
	    {
		std::cout << "circumcenter added" << std::endl << std::flush;
	...
	{
	    std::cout << "edge nr. " << *it << " is in dontexamine" << std::endl << std::flush;
	    }*/
}
