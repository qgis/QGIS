...
class QgsBarDiagramFactory: public QgsWKNDiagramFactory
{
  ...
  private:
    ...
    /**Calculates the maximum height of the bar chart (based on size for the
     scaling attribute)*/
    int getMaximumHeight( int size, const QgsAttributes& featureAttributes ) const;
    ...
    double sizeValueRatioBarChart( int size, const QgsAttributes& featureAttributes ) const;
};
