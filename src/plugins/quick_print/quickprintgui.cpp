...
QuickPrintGui::QuickPrintGui( QWidget* parent, Qt::WFlags fl )
: QDialog ( parent, fl )
{
  setupUi(this);
  readSettings();
...
void QuickPrintGui::on_buttonBox_accepted()
{
  ...
  if ( !myOutputFileName.isEmpty() )
  {
    if ( myFileDialog->selectedFilter() == tr ( "Portable Document Format (*.pdf)" ) )
    {
      ...
      // Text for inclusion on the page (to be softcoded via dialog)
      QString myTitleText = mySettings.value("quickprint/mapTitle", "Quantum GIS").toString();
      QString myNameText = mySettings.value("quickprint/mapName", "Quick Print").toString();
      QString myCopyrightText = mySettings.value("quickprint/mapCopyright", "(c) QGIS 2008").toString();
      ...
      int myPageTitleWidth = myTitleMetrics.width(myTitleText);
      ...
      myPrintPainter.drawText( myPageTitleRect, Qt::AlignCenter, myTitleText );
      ...
      int myNameWidth = myNameMetrics.width(myNameText);
      ...
      myPrintPainter.setPen( Qt::black );
      myPrintPainter.drawText( myNameRect, Qt::AlignCenter, myNameText );
      ...
      QPixmap myLegendPixmap ( myLegendDimensionX , myLegendDimensionY );      
      myLegendPixmap.fill ( myLegendBackgroundColour );
      QPainter myLegendPainter ( &myLegendPixmap );
      QFontMetrics myLegendFontMetrics(myLegendFont, &myLegendPixmap);
      int myLegendFontHeight = myLegendFontMetrics.height();
      int myLegendXPos = 0;
      int myLegendYPos = 0;
      int myLegendSpacer = 10; //for vertical and horizontal spacing
      int myIconWidth = 20;
      myLegendPainter.setFont( myLegendFont );
        ...
        if ( mypLayer )
        {
          ...
          if ( mypVectorLayer )
          {
            ...
            if ( 1 == mySymbolList.size() )
            {
              ...
              myLegendPainter.setPen( mypSymbol->pen() );
              myLegendPainter.setBrush( mypSymbol->brush() );
              ...
              if ( mypSymbol->type() == QGis::Point )
              {
                ...
                myLegendPainter.drawPixmap ( myLegendXPos, myLegendYPos, myPixmap);
              }
              else if ( mypSymbol->type() == QGis::Line )
              {
                myLegendPainter.drawLine( myLegendXPos, myLegendYPos,
                                          myLegendXPos + myIconWidth, 
                                          myLegendYPos + myIconWidth );
              }
              else //polygon
              {
                myLegendPainter.drawRect( myLegendXPos, myLegendYPos, myIconWidth, myIconWidth );
              }
              myLegendXPos += myIconWidth + myLegendSpacer;
              myLegendPainter.setPen( Qt::black );
              ...
              if (myLayerNameWidth > myMaximumLabelWidth)
              {
                ...
                myLegendPainter.drawText( 
                    myLegendItemRect, 
                    myLayerName,
                    myTextOption 
                    );
                myLegendYPos += myLegendSpacer + (myLegendFontHeight * myRowCount);
              }
              else //fits ok on a single line
              {
                QRect myLegendItemRect (myLegendXPos,
                    ...
                    myLayerNameWidth, 
                    myLegendFontHeight);
                myLegendPainter.drawText( myLegendItemRect, Qt::AlignCenter, myLayerName );
                myLegendYPos += myLegendSpacer + myLegendFontHeight;
              }
            }
            else  //class breaks
            {
              ...
              while ( myIterator.hasNext() )
              {
                ...
                else //fits ok on a single line
                {
                  QRect myLegendItemRect (myLegendXPos,
                      ...
                      myLabelWidth, 
                      myLegendFontHeight);
                  myLegendPainter.drawText( myLegendItemRect, Qt::AlignCenter, myLabel );
                  myLegendYPos += myLegendSpacer + myLegendFontHeight;
                }
              } //symbol loop
            } //class breaks
      ...
      QPixmap myLogo1;
      myLogo1.load ( QgsApplication::iconsPath() + "/qgis-icon.png" ); 
      ...
      myPrintPainter.drawText( myCopyrightRect, Qt::AlignCenter | Qt::TextWordWrap, myCopyrightText );
      ...
      //myLogo2.load ( quickprint::dataPath() + "/html/images/geologo2_small.png" ); 
      ...
      QSvgRenderer mySvgRenderer ( myNorthArrowFile );
...
void QuickPrintGui::scaleTextLabels( int theScaleFactor, SymbolScalingType theDirection)
{
  if (0 >= theScaleFactor)
  {
    QgsDebugMsg ("QuickPrintGui::scaleTextLabels invalid scale factor");
    return;
  }
  QStringList myLayerSet = mpMapCanvas->mapRender()->layerSet();
  QStringListIterator myLayerIterator ( myLayerSet );
...
void QuickPrintGui::scalePointSymbols( int theScaleFactor, SymbolScalingType theDirection)
{
  if (0 >= theScaleFactor)
  {
    QgsDebugMsg ("QuickPrintGui::scalePointSymbolsForPrint invalid scale factor");
    return;
  }
  QStringList myLayerSet = mpMapCanvas->mapRender()->layerSet();
  QStringListIterator myLayerIterator ( myLayerSet );
...
void QuickPrintGui::renderPrintScaleBar(QPainter * thepPainter, QgsMapCanvas * thepMapCanvas)
{
