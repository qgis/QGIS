  ...
  switch ( mTool )
  {
    case AddConnector:
      if ( mToolStep == 1 )
      {
        ...
        double d = sqrt( std::pow( ( double )( p.x() - p0.x() ), 2.0 )
                                        ...
                                        int end )
{
  ...
  if ( !connector->connected( In ) )
  {
    for ( int i = 0; i < mInputCount; i++ )
    {
      ...
      double d = sqrt( std::pow( ( double )( mInputPoints[i].x() + pos().x() - p.x() ), 2.0 )
  ...
  {
    double d = sqrt( std::pow( ( double )( mOutputPoint.x() + pos().x() - p.x() ), 2.0 )
...
void QgsGrassMapcalcConnector::selectEnd( QPoint point )
{
  ...
  double d0 = sqrt( std::pow( ( double )( point.x() - mPoints[0].x() ), 2.0 )
                    + std::pow( ( double )( point.y() - mPoints[0].y() ), 2.0 ) );

  double d1 = sqrt( std::pow( ( double )( point.x() - mPoints[1].x() ), 2.0 )
