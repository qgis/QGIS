/***************************************************************************
 qgssymbollayerwidgets.cpp - symbol layer widgets
...
#include "moc_qgssymbollayerwidgets.cpp"
...
QgsMarkerLineSymbolLayerWidget::QgsMarkerLineSymbolLayerWidget( QgsVectorLayer *vl, QWidget *parent )
  : QgsTemplatedLineSymbolLayerWidget( vl, parent )
{
}
...
QgsHashedLineSymbolLayerWidget::QgsHashedLineSymbolLayerWidget( QgsVectorLayer *vl, QWidget *parent )
  : QgsTemplatedLineSymbolLayerWidget( vl, parent )
{
  connect( mUi.mHashLengthUnitWidget, &QgsUnitSelectionWidget::changed, this, &QgsHashedLineSymbolLayerWidget::hashLengthUnitWidgetChanged );
  mUi.mHashLengthUnitWidget->setUnits( QgsUnitTypes::RenderUnitList() << Qgis::RenderUnit::Millimeters << Qgis::RenderUnit::MetersInMapUnits << Qgis::RenderUnit::MapUnits << Qgis::RenderUnit::Pixels << Qgis::RenderUnit::Points << Qgis::RenderUnit::Inches );

  mUi.mHashRotationSpinBox->setClearValue( 0 );

  connect( mUi.mSpinHashLength, static_cast<void ( QDoubleSpinBox::* )( double )>( &QDoubleSpinBox::valueChanged ), this, &QgsHashedLineSymbolLayerWidget::setHashLength );
  connect( mUi.mHashRotationSpinBox, static_cast<void ( QDoubleSpinBox::* )( double )>( &QDoubleSpinBox::valueChanged ), this, &QgsHashedLineSymbolLayerWidget::setHashAngle );
}

void QgsHashedLineSymbolLayerWidget::setSymbolLayer( QgsSymbolLayer *layer )
{
  QgsTemplatedLineSymbolLayerWidget::setSymbolLayer( layer );

  if ( layer->layerType() != QLatin1String( "HashLine" ) )
    return;

  // set values
  whileBlocking( mUi.mSpinHashLength )->setValue( mLayer->hashLength() );
  whileBlocking( mUi.mHashRotationSpinBox )->setValue( mLayer->hashAngle() );

  // set units
  whileBlocking( mUi.mHashLengthUnitWidget )->setUnit( mLayer->hashLengthUnit() );
  whileBlocking( mUi.mHashLengthUnitWidget )->setMapUnitScale( mLayer->hashLengthMapUnitScale() );

  registerDataDefinedButton( mUi.mHashLengthDDBtn, QgsSymbolLayer::Property::LineDistance );
  registerDataDefinedButton( mUi.mHashRotationDDBtn, QgsSymbolLayer::Property::LineAngle );
}
...
void QgsHashedLineSymbolLayerWidget::hashLengthUnitWidgetChanged()
{
  if ( mLayer )
  {
    mLayer->setHashLengthUnit( mUi.mHashLengthUnitWidget->unit() );
    mLayer->setHashLengthMapUnitScale( mUi.mHashLengthUnitWidget->getMapUnitScale() );
  }
  emit changed();
}
