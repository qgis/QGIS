...
void QgsElevationControllerLabels::paintEvent( QPaintEvent * )
{
  ...
  const int lowerY = !mInverted
                       ...
                       : ( std::max( static_cast<int>( std::round( rect().top() + sliderHeight * 0.5 + ( rect().height() - sliderHeight ) * lowerFraction - fm.descent() ) ),
                                     rect().top() + fm.ascent() ) );
  const int upperY = !mInverted ? ( std::max( static_cast<int>( std::round( rect().bottom() - sliderHeight * 0.5 - ( rect().height() - sliderHeight ) * upperFraction - fm.descent() ) ),
                                              rect().top() + fm.ascent() ) )
...
void QgsElevationControllerLabels::setLimits( const QgsDoubleRange &limits )
{
  ...
  const int maxChars = std::max( QLocale().toString( std::floor( limits.lower() ) ).length(),
                                 QLocale().toString( std::floor( limits.upper() ) ).length() )
                       + 3;
