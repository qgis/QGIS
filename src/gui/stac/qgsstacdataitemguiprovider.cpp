...
void QgsStacDataItemGuiProvider::downloadAssets( QgsDataItem *item, QgsDataItemGuiContext context )
{
  ...
  if ( dialog.exec() == QDialog::Accepted )
  {
    ...
    {
      QgsNetworkContentFetcherTask *fetcher = new QgsNetworkContentFetcherTask( url, itemItem->stacController()->authCfg(), QgsTask::CanCancel, tr( "Downloading STAC asset" ) );
      ...
      connect( fetcher, &QgsNetworkContentFetcherTask::errorOccurred, item, [context]( QNetworkReply::NetworkError, const QString &errorMsg ) {
        notify( tr( "Error downloading STAC asset" ), errorMsg, context, Qgis::MessageLevel::Critical );
      } );
      ...
      connect( fetcher, &QgsNetworkContentFetcherTask::fetched, item, [fetcher, folder, context] {
        ...
        else
        {
          ...
          if ( failed )
          {
            notify( tr( "Error downloading STAC asset" ), tr( "Could not write to file %1" ).arg( file.fileName() ), context, Qgis::MessageLevel::Critical );
          }
          else
          {
            notify( tr( "STAC asset downloaded" ), file.fileName(), context, Qgis::MessageLevel::Success );
          }
        }
      } );
