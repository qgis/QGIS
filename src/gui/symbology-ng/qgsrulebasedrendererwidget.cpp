...
QMimeData *QgsRuleBasedRendererModel::mimeData( const QModelIndexList &indexes ) const
{
  ...
  Q_FOREACH ( const QModelIndex &index, indexes )
  {
    ...
    QDomElement symbolsElem = QgsSymbolLayerUtils::saveSymbols( symbols, QStringLiteral( "symbols" ), doc, QgsPathResolver() );
    ...
    Qt::DropAction action, int row, int column, const QModelIndex &parent )
{
  ...
  while ( !stream.atEnd() )
  {
    ...
    if ( symbolsElem.isNull() )
      continue;
    QgsSymbolMap symbolMap = QgsSymbolLayerUtils::loadSymbols( symbolsElem, QgsPathResolver() );
