...
void QgsTableView::wheelEvent( QWheelEvent *event )
{
  if ( event->modifiers() & Qt::ShiftModifier )
  {
    ...
    QWheelEvent axisSwappedScrollEvent(
      event->position(), event->position(), invertedPixelDelta, invertedAngleDelta, event->buttons(), event->modifiers() & ~Qt::ShiftModifier, event->phase(), event->inverted(), event->source()
    );
