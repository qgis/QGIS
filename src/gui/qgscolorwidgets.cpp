...
void QgsColorBox::paintEvent( QPaintEvent *event )
{
  ...
  //draw cross lines
  double xPos = mMargin + ( width() - 2 * mMargin - 1 ) * static_cast<double>( xComponentValue() / ( double ) )valueRangeX();
  double yPos = mMargin + ( height() - 2 * mMargin - 1 ) - ( height() - 2 * mMargin - 1 ) * static_cast<double>( yComponentValue() / ( double ) )valueRangeY();
