...
/**
 * \ingroup gui
 * \brief A translucent overlay which renders visual feedback for a drag hovering a
 * widget that accepts map layers.
 *
 * The feedback depends on the payload being dragged (map layers, a project or an
 * unloadable file) and is shown over the widget which accepts such drops, e.g. the
 * layer tree view or the map canvas.
 *
