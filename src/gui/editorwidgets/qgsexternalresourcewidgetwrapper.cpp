...
void QgsExternalResourceWidgetWrapper::updateProperties( const QgsFeature &feature )
{
  if ( mQgsWidget && mPropertyCollection.hasActiveProperties() )
  {
    ...
    if ( mPropertyCollection.isActive( static_cast< int >( QgsEditorWidgetWrapper::Property::RootPath ) ) )
    {
      const QString path = mPropertyCollection.valueAsString( static_cast< int >( QgsEditorWidgetWrapper::Property::RootPath ), expressionContext, QString(), &ok );
      if ( ok )
      {
        mQgsWidget->setDefaultRoot( path );
      }
    }
    if ( mPropertyCollection.isActive( static_cast< int >( QgsEditorWidgetWrapper::Property::DocumentViewerContent ) ) )
    {
      const QString dvcString = mPropertyCollection.valueAsString( static_cast< int >( QgsEditorWidgetWrapper::Property::DocumentViewerContent ), expressionContext, QStringLiteral( "NoContent" ), &ok );
      if ( ok )
      {
...
void QgsExternalResourceWidgetWrapper::initWidget( QWidget *editor )
{
  ...
  if ( mQgsWidget )
  {
    ...
    mQgsWidget->fileWidget()->setStorageUrlExpression( mPropertyCollection.isActive( static_cast< int >( QgsWidgetWrapper::Property::StorageUrl ) ) ?
        mPropertyCollection.property( static_cast< int >( QgsWidgetWrapper::Property::StorageUrl ) ).asExpression() :
    ...
    if ( !mPropertyCollection.isActive( static_cast< int >( QgsWidgetWrapper::Property::RootPath ) ) )
    {
...
void QgsExternalResourceWidgetWrapper::widgetValueChanged( const QString &attribute, const QVariant &newValue, bool attributeChanged )
{
  ...
  if ( attributeChanged )
  {
    const QgsExpression documentViewerContentExp = QgsExpression( mPropertyCollection.property( static_cast< int >( QgsEditorWidgetWrapper::Property::DocumentViewerContent ) ).expressionString() );
    const QgsExpression rootPathExp = QgsExpression( mPropertyCollection.property( static_cast< int >( QgsEditorWidgetWrapper::Property::RootPath ) ).expressionString() );
