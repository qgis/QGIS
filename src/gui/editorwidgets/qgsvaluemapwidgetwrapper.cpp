...
void QgsValueMapWidgetWrapper::updateValues( const QVariant &value, const QVariantList & )
{
  ...
  if ( mComboBox )
  {
    ...
    {
      // add null value except if replace null value is checked (which is only checkable when a default value is set)
      // or if value is the autogenerated default null for the feature id field
      if ( v != "" && !layer()->defaultValueDefinition( fieldIdx() ).replaceNullValue() )
      {
        mComboBox->addItem( QgsApplication::nullRepresentation().prepend( '(' ).append( ')' ), QgsValueMapFieldFormatter::NULL_VALUE );
      }
      // add the default or the autogenerated null value
      if ( !QgsVariantUtils::isNull( value ) )
      {
