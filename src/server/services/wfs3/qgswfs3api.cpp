...
namespace QgsWfs3
{
  ...
  void Api::jsonDump( const json &data, QgsServerResponse &response, const QString &contentType )
  {
  ...
  void Api::htmlDump( const json &data, QgsServerResponse &response, const std::string &templatePath )
  {
    response.setHeader( QStringLiteral( "Content-Type" ), QStringLiteral( "text/html" ) );
    response.write( "<h1>" + templatePath + "</h1>" );
