...
void QgsServerSettings::initSettings()
{
  ...
  // options path
  {
    const Setting sOptPath
      = { QgsServerSettingsEnv::QGIS_OPTIONS_PATH, QgsServerSettingsEnv::DEFAULT_VALUE, u"Override the default path for user configuration"_s, QString(), QMetaType::Type::QString, QVariant( "" ), QVariant() };
    mSettings[sOptPath.envVar] = sOptPath;
  }

  // parallel rendering
  {
    const Setting sParRend = {
      QgsServerSettingsEnv::QGIS_SERVER_PARALLEL_RENDERING,
      QgsServerSettingsEnv::DEFAULT_VALUE,
      u"Activate/Deactivate parallel rendering for WMS getMap request"_s,
      u"/qgis/parallel_rendering"_s,
      QMetaType::Type::Bool,
      QVariant( false ),
      QVariant()
    };
  ...
  // max threads
  {
    const Setting sMaxThreads
      = { QgsServerSettingsEnv::QGIS_SERVER_MAX_THREADS, QgsServerSettingsEnv::DEFAULT_VALUE, u"Number of threads to use when parallel rendering is activated"_s, u"/qgis/max_threads"_s, QMetaType::Type::Int, QVariant( -1 ), QVariant() };
    mSettings[sMaxThreads.envVar] = sMaxThreads;
  }

  // log level
  {
    const Setting sLogLevel
      = { QgsServerSettingsEnv::QGIS_SERVER_LOG_LEVEL, QgsServerSettingsEnv::DEFAULT_VALUE, u"Log level"_s, QString(), QMetaType::Type::Int, QVariant::fromValue( Qgis::MessageLevel::NoLevel ), QVariant() };
  ...
  // log to stderr
  {
    const Setting sLogStderr
      = { QgsServerSettingsEnv::QGIS_SERVER_LOG_STDERR, QgsServerSettingsEnv::DEFAULT_VALUE, u"Activate/Deactivate logging to stderr"_s, QString(), QMetaType::Type::Bool, QVariant( false ), QVariant() };
  ...
  // cache directory
  {
    const Setting sCacheDir
      = { QgsServerSettingsEnv::QGIS_SERVER_CACHE_DIRECTORY, QgsServerSettingsEnv::DEFAULT_VALUE, u"Specify the cache directory"_s, u"/cache/directory"_s, QMetaType::Type::QString, QVariant( QgsApplication::qgisSettingsDirPath() + "cache" ), QVariant() };
    mSettings[sCacheDir.envVar] = sCacheDir;
  }

  // cache size
  {
    const Setting sCacheSize
      = { QgsServerSettingsEnv::QGIS_SERVER_CACHE_SIZE, QgsServerSettingsEnv::DEFAULT_VALUE, u"Specify the cache size (0 = automatic size)"_s, u"/cache/size-bytes"_s, QMetaType::Type::LongLong, 0, QVariant() };
    mSettings[sCacheSize.envVar] = sCacheSize;
  }

  // system locale override
  {
    const Setting sOverrideSystemLocale
      = { QgsServerSettingsEnv::QGIS_SERVER_OVERRIDE_SYSTEM_LOCALE, QgsServerSettingsEnv::DEFAULT_VALUE, u"Override system locale"_s, u"/locale/userLocale"_s, QMetaType::Type::QString, QVariant( "" ), QVariant() };
    mSettings[sOverrideSystemLocale.envVar] = sOverrideSystemLocale;
  }

  // bad layers handling
  {
    const Setting sIgnoreBadLayers
      = { QgsServerSettingsEnv::QGIS_SERVER_IGNORE_BAD_LAYERS, QgsServerSettingsEnv::DEFAULT_VALUE, u"Ignore bad layers"_s, QString(), QMetaType::Type::Bool, QVariant( false ), QVariant() };
    mSettings[sIgnoreBadLayers.envVar] = sIgnoreBadLayers;
  }

  {
    const Setting sIgnoreRenderingErrors
      = { QgsServerSettingsEnv::QGIS_SERVER_IGNORE_RENDERING_ERRORS, QgsServerSettingsEnv::DEFAULT_VALUE, u"Ignore rendering errors"_s, QString(), QMetaType::Type::Bool, QVariant( false ), QVariant() };
    mSettings[sIgnoreRenderingErrors.envVar] = sIgnoreRenderingErrors;
  }

  // retry bad layers
  {
    const Setting sRetryBadLayers
      = { QgsServerSettingsEnv::QGIS_SERVER_RETRY_BAD_LAYERS, QgsServerSettingsEnv::DEFAULT_VALUE, u"Retry bad layers"_s, QString(), QMetaType::Type::Bool, QVariant( false ), QVariant() };
    mSettings[sRetryBadLayers.envVar] = sRetryBadLayers;
  }

  // trust layer metadata
  {
    const Setting sTrustLayerMetadata
      = { QgsServerSettingsEnv::QGIS_SERVER_TRUST_LAYER_METADATA, QgsServerSettingsEnv::DEFAULT_VALUE, u"Trust layer metadata"_s, QString(), QMetaType::Type::Bool, QVariant( false ), QVariant() };
    mSettings[sTrustLayerMetadata.envVar] = sTrustLayerMetadata;
  }

  // force to open layers in read-only mode
  {
    const Setting sForceReadOnlyLayers
      = { QgsServerSettingsEnv::QGIS_SERVER_FORCE_READONLY_LAYERS, QgsServerSettingsEnv::DEFAULT_VALUE, u"Force to open layers in read-only mode"_s, QString(), QMetaType::Type::Bool, QVariant( false ), QVariant() };
    mSettings[sForceReadOnlyLayers.envVar] = sForceReadOnlyLayers;
  }

  // don't load layouts
  {
    const Setting sDontLoadLayouts
      = { QgsServerSettingsEnv::QGIS_SERVER_DISABLE_GETPRINT, QgsServerSettingsEnv::DEFAULT_VALUE, u"Don't load layouts"_s, QString(), QMetaType::Type::Bool, QVariant( false ), QVariant() };
    mSettings[sDontLoadLayouts.envVar] = sDontLoadLayouts;
  }

  // show group separator
  {
    const Setting sShowGroupSeparator
      = { QgsServerSettingsEnv::QGIS_SERVER_SHOW_GROUP_SEPARATOR, QgsServerSettingsEnv::DEFAULT_VALUE, u"Show group (thousands) separator"_s, u"/locale/showGroupSeparator"_s, QMetaType::Type::QString, QVariant( false ), QVariant() };
    mSettings[sShowGroupSeparator.envVar] = sShowGroupSeparator;
  }

  // max height
  {
    const Setting sMaxHeight = {
      QgsServerSettingsEnv::QGIS_SERVER_WMS_MAX_HEIGHT,
      QgsServerSettingsEnv::DEFAULT_VALUE,
      u"Maximum height for a WMS request. The lower one of this and the project configuration is used."_s,
      u"/qgis/max_wms_height"_s,
  ...
  // max width
  {
    const Setting sMaxWidth = {
      QgsServerSettingsEnv::QGIS_SERVER_WMS_MAX_WIDTH,
      QgsServerSettingsEnv::DEFAULT_VALUE,
      u"Maximum width for a WMS request. The most conservative between this and the project one is used"_s,
      u"/qgis/max_wms_width"_s,
      QMetaType::Type::LongLong,
      QVariant( -1 ),
      QVariant()
    };
  ...
  {
    const Setting sApiResourcesDirectory = {
      QgsServerSettingsEnv::QGIS_SERVER_API_RESOURCES_DIRECTORY,
      QgsServerSettingsEnv::DEFAULT_VALUE,
      u"Base directory where HTML templates and static assets (e.g. images, js and css files) are searched for"_s,
      u"/qgis/server_api_resources_directory"_s,
      QMetaType::Type::QString,
      QDir( QgsApplication::pkgDataPath() ).absoluteFilePath( u"resources/server/api"_s ),
      QString()
    };
  ...
  // API WFS3 max limit
  {
    const Setting sApiWfs3MaxLimit = {
      QgsServerSettingsEnv::QGIS_SERVER_API_WFS3_MAX_LIMIT,
      QgsServerSettingsEnv::DEFAULT_VALUE,
      u"Maximum value for \"limit\" in a features request, defaults to 10000"_s,
      u"/qgis/server_api_wfs3_max_limit"_s,
      QMetaType::Type::LongLong,
      QVariant( 10000 ),
      QVariant()
    };
  ...
  {
    const Setting sProjectsDirectories = {
      QgsServerSettingsEnv::QGIS_SERVER_LANDING_PAGE_PROJECTS_DIRECTORIES,
      QgsServerSettingsEnv::DEFAULT_VALUE,
      u"Directories used by the landing page service to find .qgs and .qgz projects"_s,
      u"/qgis/server_projects_directories"_s,
      QMetaType::Type::QString,
      QVariant( "" ),
      QVariant()
    };
  ...
  {
    const Setting sProjectsPgConnections = {
      QgsServerSettingsEnv::QGIS_SERVER_LANDING_PAGE_PROJECTS_PG_CONNECTIONS,
      QgsServerSettingsEnv::DEFAULT_VALUE,
      u"PostgreSQL connection strings used by the landing page service to find projects"_s,
      u"/qgis/server_projects_pg_connections"_s,
      QMetaType::Type::QString,
      QVariant( "" ),
      QVariant()
    };
  ...
  // landing page base URL prefix
  {
    const Setting sLandingPageBaseUrlPrefix
      = { QgsServerSettingsEnv::QGIS_SERVER_LANDING_PAGE_PREFIX, QgsServerSettingsEnv::DEFAULT_VALUE, u"Landing page base URL path prefix"_s, u"/qgis/server_landing_page_base_url_prefix"_s, QMetaType::Type::QString, QVariant( "" ), QVariant() };
    mSettings[sLandingPageBaseUrlPrefix.envVar] = sLandingPageBaseUrlPrefix;
  }

  // log profile
  {
    const Setting sLogProfile = {
      QgsServerSettingsEnv::QGIS_SERVER_LOG_PROFILE,
      QgsServerSettingsEnv::DEFAULT_VALUE,
      u"Add detailed profile information to the logs, only effective when QGIS_SERVER_LOG_LEVEL=0"_s,
      u"/qgis/server_log_profile"_s,
      QMetaType::Type::Bool,
      QVariant( false ),
      QVariant()
    };
  ...
  {
    const Setting sServiceUrl
      = { QgsServerSettingsEnv::QGIS_SERVER_SERVICE_URL, QgsServerSettingsEnv::DEFAULT_VALUE, u"The default service URL"_s, u"/qgis/server_service_url"_s, QMetaType::Type::QString, QVariant( "" ), QVariant() };
    mSettings[sServiceUrl.envVar] = sServiceUrl;
  }

  // the default WMS service URL.
  {
    const Setting sWmsServiceUrl
      = { QgsServerSettingsEnv::QGIS_SERVER_WMS_SERVICE_URL, QgsServerSettingsEnv::DEFAULT_VALUE, u"The default WMS service URL"_s, u"/qgis/server_wms_service_url"_s, QMetaType::Type::QString, QVariant( "" ), QVariant() };
    mSettings[sWmsServiceUrl.envVar] = sWmsServiceUrl;
  }

  // the default WFS service URL.
  {
    const Setting sWfsServiceUrl
      = { QgsServerSettingsEnv::QGIS_SERVER_WFS_SERVICE_URL, QgsServerSettingsEnv::DEFAULT_VALUE, u"The default WFS service URL"_s, u"/qgis/server_wfs_service_url"_s, QMetaType::Type::QString, QVariant( "" ), QVariant() };
    mSettings[sWfsServiceUrl.envVar] = sWfsServiceUrl;
  }

  // the default WCS service URL.
  {
    const Setting sWcsServiceUrl
      = { QgsServerSettingsEnv::QGIS_SERVER_WCS_SERVICE_URL, QgsServerSettingsEnv::DEFAULT_VALUE, u"The default WcS service URL"_s, u"/qgis/server_wcs_service_url"_s, QMetaType::Type::QString, QVariant( "" ), QVariant() };
    mSettings[sWcsServiceUrl.envVar] = sWcsServiceUrl;
  }

  // the default WMTS service URL.
  {
    const Setting sWmtsServiceUrl
      = { QgsServerSettingsEnv::QGIS_SERVER_WMTS_SERVICE_URL, QgsServerSettingsEnv::DEFAULT_VALUE, u"The default WMTS service URL"_s, u"/qgis/server_wmts_service_url"_s, QMetaType::Type::QString, QVariant( "" ), QVariant() };
  ...
  // the default config cache check interval
  {
    const Setting sConfigCacheCheckInterval = {
      ...
      QgsServerSettingsEnv::DEFAULT_VALUE,
      u"The default project cache check interval (in ms)"_s,
      u"/qgis/server_project_cache_check_interval"_s,
      QMetaType::Type::Int,
      QVariant( 0 ),
      QVariant()
    };
  ...
  {
    const Setting sProjectCacheStrategy = {
      QgsServerSettingsEnv::QGIS_SERVER_PROJECT_CACHE_STRATEGY,
      QgsServerSettingsEnv::DEFAULT_VALUE,
      u"Project's cache strategy. Possible values are 'off','filesystem' or 'periodic'"_s,
      u"/qgis/server_project_cache_strategy"_s,
      QMetaType::Type::QString,
      QVariant( "" ),
      QVariant()
    };
  ...
  {
    const Setting sProjectCacheSize
      = { QgsServerSettingsEnv::QGIS_SERVER_PROJECT_CACHE_SIZE, QgsServerSettingsEnv::DEFAULT_VALUE, u"Project's cache size, in number of projects."_s, u"/qgis/server_project_cache_size"_s, QMetaType::Type::QString, QVariant( 100 ), QVariant() };
...

  {
    const Setting sAllowedExtraSqlTokens = {
      ...
      QgsServerSettingsEnv::DEFAULT_VALUE,
      u"List of comma separated SQL tokens to be added to the list of allowed tokens that the services accepts when filtering features"_s,
      u"/qgis/server_allowed_extra_sql_tokens"_s,
      QMetaType::Type::QString,
      QVariant( "" ),
      QVariant()
    };
  ...
  {
    const Setting sCapabilitiesCacheSize
