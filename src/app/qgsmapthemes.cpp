...
void QgsMapThemes::addPreset()
{
  ...
  QgsNewNameDialog dlg( tr( "theme" ), tr( "Theme" ), QStringList(), existingNames, QRegularExpression(), QRegularExpression::CaseInsensitive, mMenu );
  ...
  for ( QAction *actionPreset : qgis::as_const( mMenuPresetActions ) )
  {
    if ( actionPreset->isChecked() )
    {
      QgsNewNameDialog dlg(
        ...
        QStringList(), existingNames, QRegularExpression(), QRegularExpression::CaseInsensitive, mMenu );
