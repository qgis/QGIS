...
QString QgsAiReindexLayersTool::description() const
{
  return QStringLiteral(
    ...
    "chunks (attributes + bounding boxes), and embeds them via the OpenAI "
    "embeddings endpoint. File chunks are preserved. The user's API key is used "
    "for billing; the data is uploaded to OpenAI. The caller MUST set "
...
{
  QJsonObject properties;
  properties.insert( u"confirm"_s, prop( u"boolean"_s, u"Must be true to acknowledge that layer data will be sent to OpenAI."_s ) );
  return schemaObject( properties, QJsonArray { u"confirm"_s } );
}
...
QgsAiToolResult QgsAiReindexLayersTool::execute( const QJsonObject &args )
{
  ...
  if ( !args.value( u"confirm"_s ).toBool( false ) )
    return QgsAiToolResult::error( u"Refusing to reindex layers without explicit 'confirm: true' (data would be sent to OpenAI)."_s );
