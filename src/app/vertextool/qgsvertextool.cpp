...
QgsPointLocator::Match QgsVertexTool::snapToEditableLayer( QgsMapMouseEvent *e )
{
    ...
    if ( currentVlayer->isEditable() )
    {
      ...
      for ( QgsMapLayer *layer : layers )
      {
        ...
        if ( existingSettings != oldLayerSettings.constEnd() )
        {
          ...
          layerSettings.setUnits( Qgis::MapUnitType::Project );
        }
        else
        {
          layerSettings = QgsSnappingConfig::IndividualLayerSettings(
                            vlayer == currentVlayer, static_cast<Qgis::SnappingTypes>( Qgis::SnappingType::Vertex | Qgis::SnappingType::Segment ), tol, Qgis::MapUnitType::Project, 0.0, 0.0 );
        }
  ...
  if ( !m.isValid() && mMode == AllLayers )
  {
    ...
    for ( QgsMapLayer *layer : layers )
    {
      ...
      {
        layerSettings = QgsSnappingConfig::IndividualLayerSettings( vlayer->isEditable(), static_cast<Qgis::SnappingTypes>( Qgis::SnappingType::Vertex | Qgis::SnappingType::Segment ), tol, Qgis::MapUnitType::Project, 0.0, 0.0 );
      }
      config.setIndividualLayerSettings( vlayer, layerSettings );
    }
