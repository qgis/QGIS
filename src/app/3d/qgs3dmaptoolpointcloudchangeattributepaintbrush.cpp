...
void Qgs3DMapToolPointCloudChangeAttributePaintbrush::activate()
{
  ...
  mSelectionRubberBand = std::make_unique<QgsRubberBand3D>( mCanvas->scene(), Qgis::GeometryType::Point );
  ...
  mIsActive = true;
  mHighlighterRubberBand = std::make_unique<QgsRubberBand3D>( mCanvas->scene(), Qgis::GeometryType::Polygon );
