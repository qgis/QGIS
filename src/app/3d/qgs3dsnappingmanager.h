 ...
 *
 * Snapped entities and points can be highlighted.
 * Highlighted entities are attached to the highlight root entity.
 * The snapped marker entity is attached to the rubberband root entity
...
class Qgs3DSnappingManager
{
  public:
     ...
     */
    QgsPoint screenToMap( const QPoint &screenPos, bool *success, bool highlightEntity = true, bool highlightSnappedPoint = true );

  private:
    static void computeFeatureColors( QColor &backgroundColor, QColor &edgeColor );
    /**
     *
     * \param screenPos screen position to start the search from
  ...
  private:
    ...
    QObjectUniquePtr<QgsRubberBand3D> mHighlightedPointBB = nullptr;
    QObjectUniquePtr<Qt3DCore::QEntity> mHighlightedFeature = nullptr;
