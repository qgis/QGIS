...
QIcon QgsCategorized3DRendererModel::symbolIcon( const QgsCategorized3DRenderer::Category &category ) const
{
...
void QgsCategorized3DRendererWidget::updateUiFromRenderer()
{
  ...
  if ( mLayer )
  {
    ...
    {
      mBtnUpdateCategoriesFrom2D->setEnabled( true );
      mBtnUpdateCategoriesFrom2D->setToolTip( QString() );
    }
    else
    {
      mBtnUpdateCategoriesFrom2D->setEnabled( false );
      mBtnUpdateCategoriesFrom2D->setToolTip( tr( "Requires a categorized 2D renderer" ) );
    }
  }
}
...
void QgsCategorized3DRendererWidget::addCategories()
{
  ...
  if ( uniqueValues.size() >= 1000 )
  {
    const int res = QMessageBox::warning(
      nullptr,
      tr( "Classify Categories" ),
      tr( "High number of classes. Classification would yield %n entries which might not be expected. Continue?", nullptr, static_cast<int>( uniqueValues.size() ) ),
...
void QgsCategorized3DRendererWidget::updateSymbolsFromWidget( QgsSymbol3DWidget *widget )
{
  mCategorizedSymbol = widget->symbol();
  applyChangeToSymbol();
}
