...
///888888888888888888888888888888888888888888888888888888888888888888888888
  ...
  connect(m_actionStartDianliXian, SIGNAL(triggered()), this, SLOT(StartCloseDianLiModel()));
...
void QgisApp::StartCloseProfileMode()
{
  ...
  2，设置成为正面视图
  3，如果电力线模式等和剖面窗口有关系的启动了，那么关闭
  */
  if (dianlixianwindowstarted == true)
  {
    StartCloseDianLiModel();
  }
  ...
  {   // todo:: 在二维平面 显示  这个四边形 
    ...
    if ( m_geometries->rowCount())
    {
      ...
      if (!ProfileViewerDock.get())
      {
        ProfileViewerDock = std::shared_ptr<QgsProfileWindowDockWidget>( new QgsProfileWindowDockWidget(tr("剖面图"), this)); //
      ...
      addDockWidget(Qt::BottomDockWidgetArea, ProfileViewerDock.get());
...
// 电力线模式::
//  剖面图功能基础上增加专门针对剖面图的功能
//  如果剖面图模式启动了，那么关闭剖面图
void QgisApp::StartCloseDianLiModel()
{
  /*
    1,完成隐藏，框选区域内的点显示，其余的显示。控制m_pointview
    2，设置成为正面视图
*/
  ...
  if (dianlixianwindowstarted == true)
  {
    mMapCanvas->setMapTool(mMapTools.mPan);
    m_pointProfileView->CloseProfileviewMode();
    dianlixianwindowstarted = false;
    DianliXianModuleDock->hide();
    mStatusBar->showMessage(tr("电力线模式关闭"));
  }
  else
  {   // todo:: 在二维平面 显示  这个四边形 
    const std::vector<Imath::V3f>& quadpoint = m_pointView->Get_Quadpoints();
    ...
    if (m_geometries->rowCount())
    {
      ...
      startProfile(tr("dianlixian dock"));
      if (!DianliXianModuleDock.get())
      {
        DianliXianModuleDock =std::shared_ptr<QgsDLWindowDockWidget>( new QgsDLWindowDockWidget(tr("电力线工具"), this)); //
        DianliXianModuleDock->setWindowFlags(Qt::FramelessWindowHint | Qt::WindowSystemMenuHint | Qt::WindowMinMaxButtonsHint);
        DianliXianModuleDock->setObjectName(QStringLiteral("ShowProfileWindow"));
        DianliXianModuleDock->setWhatsThis(tr("ShowProfileWindow"));
        DianliXianModuleDock->setAllowedAreas(Qt::AllDockWidgetAreas);
        m_pointProfileView->setMinimumWidth(int(this->centralwidget->size().width() / 5));
      ...
      setViewFront(m_pointProfileView);
      m_pointProfileView->LockXYZ(4);
      mStatusBar->showMessage(tr("电力线模式启动"));
      dianlixianwindowstarted = true;
      DianliXianModuleDock->show();
