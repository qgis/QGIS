...
bool QgisApp::openLayer( const QString &fileName, bool allowInteractive )
{
  ...
  SpatialiteDbInfo *spatialiteDbInfo = QgsSpatiaLiteUtils::CreateSpatialiteConnection( fileName, true, true );
  if ( spatialiteDbInfo )
  {
    ...
    if ( ( spatialiteDbInfo->isDbSpatialite() ) || ( spatialiteDbInfo->isDbGdalOgr() ) )
    {
      ...
      if ( ( bLoadGdalOrg ) && ( spatialiteDbInfo->dbLayersCount() > 0 ) )
      {
        ...
        {
          // When only 1 Layer, load directly through spatialiteDbInfo
          if ( spatialiteDbInfo->addDbMapLayers( QStringList( fileName ), QStringList( ) ) )
          {
            return true;
          }
        }
      }
      // - loading is not desired or possible, close connection and free
