...
bool QgsWcsCapabilities::retrieveServerCapabilities()
{
  ...
  foreach ( const QString& v, versions )
  {
...
bool QgsWcsCapabilities::parseCapabilitiesDom( QByteArray const &xml, QgsWcsCapabilitiesProperty &capabilities )
{
  ...
  else if ( mVersion.startsWith( "1.1" ) )
  {
    ...
    foreach ( const QDomElement& el, operationElements )
    {
...
QStringList QgsWcsCapabilities::domElementsTexts( const QDomElement &element, const QString &path )
{
  ...
  foreach ( const QDomElement& el, elems )
  {
    list << el.text();
  }
  return list;
...
{
  QList<int> list;
  foreach ( const QString& s, text.split( " " ) )
  {
    bool ok;
...
bool QgsWcsCapabilities::parseDescribeCoverageDom10( QByteArray const &xml, QgsWcsCoverageSummary *coverage )
{
  ...
  foreach ( const QDomElement& el, envelopeElements )
  {
  ...
  foreach ( const QDomElement& el, timePeriodElements )
  {
  ...
  if ( !coverage->nativeCrs.isEmpty() )
  {
    foreach ( const QString& srsName, coverage->boundingBoxes.keys() )
    {
  ...
  foreach ( const QString& text, domElementsTexts( coverageOfferingElement, "rangeSet.RangeSet.nullValue.singleValue" ) )
  {
    bool ok;
...
bool QgsWcsCapabilities::parseDescribeCoverageDom11( QByteArray const &xml, QgsWcsCoverageSummary *coverage )
{
  ...
  foreach ( const QDomElement& el, boundingBoxElements )
  {
  ...
  foreach ( const QString& text, domElementsTexts( docElem, "CoverageDescription.Range.Field.NullValue" ) )
  {
    bool ok;
    double val = text.toDouble( &ok );
    if ( ok )
  ...
  foreach ( const QString& crs, crss )
  {
