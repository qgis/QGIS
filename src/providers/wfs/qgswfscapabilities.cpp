...
{
  QRegularExpressionMatch match = QRegularExpression( "urn:ogc:def:crs:([^:]+).+([^:]+)", Qt::CaseInsensitive ).match( crsName );
  if ( match.hasMatch() )
  {
  ...
  QRegularExpressionMatch match2 = QRegularExpression( "urn:x-ogc:def:crs:([^:]+).+([^:]+)", Qt::CaseInsensitive ).match( crsName );
  if ( match2.hasMatch( )
  {
