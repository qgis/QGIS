    ...
    if ( QgsGeoNodeConnectionUtils::connectionList().contains( connectionName ) )
    {
      ...
      if ( !encodedUris.isEmpty() )
      {
        for ( const QString &encodedUri : encodedUris )
        {
          ...
          if ( QgsOwsConnection::settingsDpiMode->exists( serviceConnectionDetails ) )
          {
            uri.setParam( QStringLiteral( "dpiMode" ), QString::number( static_cast<int>( QgsOwsConnection::settingsDpiMode->value( serviceConnectionDetails ) ) ) );
          }
