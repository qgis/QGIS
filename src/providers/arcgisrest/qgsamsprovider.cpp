...
QImage QgsAmsProvider::draw( const QgsRectangle &viewExtent, int pixelWidth, int pixelHeight, QgsRasterBlockFeedback *feedback )
{
  ...
  if ( mTiled )
  {
    ...
    else if ( !requestsFinal.isEmpty() )
    {
      ...
      QgsAmsTiledImageDownloadHandler handler( authcfg, mRequestHeaders, mUrlPrefix, mTileReqNo, requestsFinal, &image, viewExtent, feedback );
  ...
  else
  {
    ...
    for ( int currentStepWidth = 0; currentStepWidth < nbStepWidth; currentStepWidth++ )
    {
      for ( int currentStepHeight = 0; currentStepHeight < nbStepHeight; currentStepHeight++ )
      {
        ...
        mErrorTitle.clear();
        QString contentType;
        QByteArray reply = QgsArcGisRestQueryUtils::queryService( requestUrl, authcfg, mErrorTitle, mError, mRequestHeaders, mUrlPrefix, feedback, &contentType );
        if ( !mError.isEmpty() )
        {
          p.end();
          mCachedImage = QImage();
...
QgsAmsTiledImageDownloadHandler::QgsAmsTiledImageDownloadHandler( const QString &auth,  const QgsHttpHeaders &requestHeaders, const QString &urlPrefix, int tileReqNo, const QgsAmsProvider::TileRequests &requests, QImage *image, const QgsRectangle &viewExtent, QgsRasterBlockFeedback *feedback )
  : mAuth( auth )
