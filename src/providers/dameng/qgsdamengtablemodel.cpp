...
QgsDamengTableModel::QgsDamengTableModel( QObject *parent )
...
QStringList QgsDamengTableModel::columns() const
{
  return mColumns;
}

int QgsDamengTableModel::defaultSearchColumn() const
{
  return static_cast<int>( DbtmTable );
}

bool QgsDamengTableModel::searchableColumn( int column ) const
{
...
void QgsDamengTableModel::addTableEntry( const QgsDamengLayerProperty &layerProperty )
{
  ...
  for ( int i = 0; i < layerProperty.size(); i++ )
  {
    ...
    QStandardItem *geomTypeItem = new QStandardItem( QgsDamengConn::displayStringForGeomType( layerProperty.geometryColType ) );
    ...
    QStringList defPk( QgsSettings().value( QStringLiteral( "/Dameng/connections/%1/keys/%2/%3" ).arg( mConnName, layerProperty.schemaName, layerProperty.tableName ), QStringList() ).toStringList() );
...
void QgsDamengTableModel::setSql( const QModelIndex &index, const QString &sql )
{
...
bool QgsDamengTableModel::setData( const QModelIndex &idx, const QVariant &value, int role )
{
  ...
  {
    const Qgis::WkbType wkbType = ( Qgis::WkbType ) idx.sibling( idx.row(), DbtmType ).data( Qt::UserRole + 2 ).toInt();
...
QString QgsDamengTableModel::layerURI( const QModelIndex &index, const QString &connInfo, bool useEstimatedMetadata )
{
  if ( !index.isValid() )
  {
  ...
  QgsSettings().setValue( QStringLiteral( "/Dameng/connections/%1/keys/%2/%3" ).arg( mConnName, schemaName, tableName ), QVariant( qgis::setToList( s1 ) ) );
