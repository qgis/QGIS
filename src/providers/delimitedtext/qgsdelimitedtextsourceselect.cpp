...
void QgsDelimitedTextSourceSelect::updateFieldLists()
{
  ...
  mBadRowCount = 0;
  QStringList values;
  QRegularExpression wktre( "^\\s*(?:MULTI)?(?:POINT|LINESTRING|POLYGON)\\s*Z?\\s*M?\\(", QRegularExpression::CaseInsensitiveOption );
    ...
    for ( int i = 0; i < tblSample->columnCount(); i++ )
    {
      ...
      if ( ! value.isEmpty() )
      {
        ...
        if ( isValidCoordinate[i] )
        {
          ...
          if ( xyDms )
          {
            ok = QgsDelimitedTextProvider::sCrdDmsRegexp.match( value ).capturedStart() == 0;
          }
          else
          {
