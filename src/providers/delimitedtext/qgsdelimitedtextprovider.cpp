...
QRegularExpression QgsDelimitedTextProvider::sWktPrefixRegexp( "^\\s*(?:\\d+\\s+|SRID\\=\\d+\\;)", QRegularExpression::CaseInsensitive );
QRegularExpression QgsDelimitedTextProvider::sCrdDmsRegexp( "^\\s*(?:([-+nsew])\\s*)?(\\d{1,3})(?:[^0-9.]+([0-5]?\\d))?[^0-9.]+([0-5]?\\d(?:\\.\\d+)?)[^0-9.]*([-+nsew])?\\s*$", QRegularExpression::CaseInsensitive );
...
QStringList QgsDelimitedTextProvider::readCsvtFieldTypes( const QString &filename, QString *message )
{
  ...
  QRegularExpressionMatch match = QRegularExpression( QRegularExpression::anchoredPattern( "^(?:\\s*(\\\"?)(?:integer|real|double|long|longlong|int8|string|date|datetime|time)(?:\\(\\d+(?:\\.\\d+)?\\))?\\1\\s*(?:,|$))+" ) ).globalMatch( strTypeList );
  if ( !match.hasMatch() )
  {
  ...
  int pos = 0;
  QRegularExpressionMatchIterator reType = QRegularExpressio( "(integer|real|double|string|date|datetime|time)" ).globalMatch( strTypeList );
  QString capted; 
