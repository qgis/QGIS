...
void QgsPgTableModel::addTableEntry( const QgsPostgresLayerProperty &layerProperty )
{
  ...
  for ( int i = 0; i < layerProperty.size(); i++ )
  {
    ...
    if ( ! layerProperty.isRaster )
    {
      ...
      else if ( wkbType != QgsWkbTypes::Type::NoGeometry && srid == std::numeric_limits<int>::min() )
      {
    ...
    {
      typeItem = new QStandardItem( iconForWkbType( wkbType ), wkbType == QgsWkbTypes::Type::Unknown ? tr( "Select…" ) : QgsPostgresConn::displayStringForWkbType( wkbType ) );
    }
    typeItem->setData( wkbType == QgsWkbTypes::Type::Unknown, Qt::UserRole + 1 );
    typeItem->setData( QVariant::fromValue( wkbType ), Qt::UserRole + 2 );
    ...
    QStandardItem *sridItem  = new QStandardItem( wkbType != QgsWkbTypes::Type::NoGeometry ? QString::number( srid ) : QString() );
    sridItem->setEditable( wkbType != QgsWkbTypes::Type::NoGeometry && srid == std::numeric_limits<int>::min() );
