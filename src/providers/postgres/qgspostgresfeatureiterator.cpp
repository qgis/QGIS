...
bool QgsPostgresFeatureIterator::declareCursor( const QString &whereClause, long limit, bool closeOnFail, const QString &orderBy )
{
  ...
  if ( mFetchGeometry )
  {
    ...
    QgsWkbTypes::Type usedGeomType = mSource->mRequestedGeomType != QgsWkbTypes::Type::Unknown
    ...
    if ( !mRequest.simplifyMethod().forceLocalOptimization() &&
         ...
         QgsWkbTypes::flatType( QgsWkbTypes::singleType( usedGeomType ) ) != QgsWkbTypes::Type::Point &&
...
bool QgsPostgresFeatureIterator::getFeature( QgsPostgresResult &queryResult, int row, QgsFeature &feature )
{
  ...
  if ( mFetchGeometry )
  {
    ...
    if ( returnedLength > 0 )
    {
      ...
      {
        // overwrite type
        unsigned int n = static_cast<unsigned int>( newType );
      ...
      if ( wkbType % 1000 == 16 )
      {
        ...
        {
          const unsigned int localType = static_cast<unsigned int>( QgsWkbTypes::singleType( newType ) ); // polygon(Z|M)
