...
void QgsMssqlProvider::loadMetadata()
{
  ...
  {
    QgsDebugError( QStringLiteral( "Could not retrieve geometry metadata for %1.%2 from geometry_columns: table does not exist or no matching record" ).arg( QgsMssqlUtils::quotedIdentifier( mSchemaName ), QgsMssqlUtils::quotedIdentifier( mTableName ) ) );
  }
}
