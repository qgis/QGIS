...
void QgsSpatiaLiteProviderConnection::createVectorTable(
...
) const
{
  ...
  Qgis::VectorExportResult res
    = QgsSpatiaLiteProvider::createEmptyLayer( uri() + u" table=%1 (geom)"_s.arg( QgsSqliteUtils::quotedIdentifier( name ) ), fields, wkbType, srs, overwrite, &map, createdLayerUri, &errCause, &opts );
...
void QgsSpatiaLiteProviderConnection::dropVectorTable( const QString &schema, const QString &name ) const
{
  ...
  else
  {
    ...
    else
    {
      ...
      if ( ret != SQLITE_OK )
      {
        QgsDebugError( u"Failed to run VACUUM after deleting table on database %1"_s.arg( pathFromUri() ) );
      }
...
void QgsSpatiaLiteProviderConnection::renameVectorTable( const QString &schema, const QString &name, const QString &newName ) const
{
  ...
  sql = u"UPDATE geometry_columns SET f_table_name = lower(%2) WHERE lower(f_table_name) = lower(%1)"_s.arg( QgsSqliteUtils::quotedString( name ), QgsSqliteUtils::quotedString( newName ) );
  executeSqlDirect( sql );
  sql = u"UPDATE layer_styles SET f_table_name = lower(%2) WHERE f_table_name = lower(%1)"_s.arg( QgsSqliteUtils::quotedString( name ), QgsSqliteUtils::quotedString( newName ) );
  try
  {
    executeSqlDirect( sql );
  }
