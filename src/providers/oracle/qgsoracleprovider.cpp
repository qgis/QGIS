...
//       singleGeomClause = QStringLiteral( " mod(FEATUREREQUEST.%2.sdo_gtype,100) = 2 AND FEATUREREQUEST.%1 in ( "
//                                          " SELECT res.PK FROM (SELECT subres.PK, MAX(subres.interpretation) as interpretation "
//                                          " FROM (SELECT FEATUREREQUEST.%1 as PK, MAX(DECODE(MOD(ROWNUM, 3), 1, t.COLUMN_VALUE, NULL)) offset "
//                                          " , MAX(DECODE(MOD(ROWNUM, 3), 2, t.COLUMN_VALUE, NULL)) etype , MAX(DECODE(MOD(ROWNUM, 3), 0, t.COLUMN_VALUE, NULL)) interpretation "
//                                          " FROM %3 FEATUREREQUEST, TABLE(FEATUREREQUEST.%2.sdo_elem_info) t GROUP BY FEATUREREQUEST.%1, TRUNC((ROWNUM - 1) / 3, 0) ) subres "
//                                          " GROUP BY subres.PK ORDER BY subres.PK ) res, %3 FEATUREREQUEST WHERE FEATUREREQUEST.%1 = res.PK AND not res.interpretation in (2,3) "
//                                          " )" ).arg( quotedIdentifier( mUri.keyColumn() ), mGeometryColumn, mQuery );
//       break;
...
//                                          " , MAX(DECODE(MOD(ROWNUM, 3), 2, t.COLUMN_VALUE, NULL)) etype , MAX(DECODE(MOD(ROWNUM, 3), 0, t.COLUMN_VALUE, NULL)) interpretation "
//                                          " FROM %3 FEATUREREQUEST, TABLE(FEATUREREQUEST.%2.sdo_elem_info) t GROUP BY FEATUREREQUEST.%1, TRUNC((ROWNUM - 1) / 3, 0) ) subres "
//                                          " GROUP BY subres.PK ORDER BY subres.PK ) res, %3 FEATUREREQUEST WHERE FEATUREREQUEST.%1 = res.PK AND res.interpretation in (2,3) "
//                                          " )" ).arg( quotedIdentifier( mUri.keyColumn() ), mGeometryColumn, mQuery );
//       break;
