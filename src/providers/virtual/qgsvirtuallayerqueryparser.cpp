...
namespace QgsVirtualLayerQueryParser
{
  ...
  QMap<QString, ColumnDef> columnCommentDefinitions( const QString &query )
  {
    ...
    QRegularExpression rx( "([a-zA-Z_\x80-\xFF][a-zA-Z0-9_\x80-\xFF]*)\\s*/\\*:(int|real|text|((?:multi)?(?:point|linestring|polygon)):(\\d+))\\s*\\*/", QRegularExpression::CaseInsensitive );
    QRegularExpressionMatch match;
    int pos;

    while ( ( match =  rx.match( query.mid( pos ) ) ).capturedStart() != -1 )
    {
      ...
      pos = rx.matchedLength() + match.capturedStart() ;
    }
    return defs;
  }
...
// set the type of the column type, given its text representation
  ...
  {
    // geometry type
    QRegularExpression geometryTypeRx( "\\(([0-9]+),([0-9]+)\\)" );
