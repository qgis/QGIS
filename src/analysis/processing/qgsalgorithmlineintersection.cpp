...
QVariantMap QgsLineIntersectionAlgorithm::processAlgorithm( const QVariantMap &parameters, QgsProcessingContext &context, QgsProcessingFeedback *feedback )
{
  ...
  QString dest;
  std::unique_ptr< QgsFeatureSink > sink( parameterAsSink( parameters, QStringLiteral( "OUTPUT" ), context, dest, outFields, QgsWkbTypes::Type::Point,  sourceA->sourceCrs(), QgsFeatureSink::RegeneratePrimaryKey ) );
  if ( !sink )
    ...
    if ( !lines.empty() )
    {
      ...
      while ( featuresB.nextFeature( inFeatureB ) )
      {
        ...
        if ( engine->intersects( tmpGeom.constGet() ) )
        {
          ...
          if ( QgsWkbTypes::flatType( intersectGeom.wkbType() ) == QgsWkbTypes::Type::GeometryCollection )
          {
          ...
          else if ( intersectGeom.type() == QgsWkbTypes::GeometryType::PointGeometry )
          {
