...
{
  return QObject::tr( "This algorithm checks the input geometries contained in the polygon layers.\n"
                      "A polygon layer can be checked against itself.\n"
                      "Contained features are errors.\n" );
}
...
void QgsGeometryCheckContainedAlgorithm::initAlgorithm( const QVariantMap &configuration )
{
  ...
  addParameter( new QgsProcessingParameterMultipleLayers(
    QStringLiteral( "POLYGONS" ), QObject::tr( "Polygon layers (one or more)" ),
...
QVariantMap QgsGeometryCheckContainedAlgorithm::processAlgorithm( const QVariantMap &parameters, QgsProcessingContext &context, QgsProcessingFeedback *feedback )
{
  ...
  std::unique_ptr< QgsFeatureSink > sink_errors( parameterAsSink(
    parameters, QStringLiteral( "ERRORS" ), context, dest_errors, fields, Qgis::WkbType::Point, QgsProject::instance()->crs()
  ) );
  if ( !sink_errors )
  ...
  std::unique_ptr<QgsGeometryCheckContext> checkContext = std::make_unique<QgsGeometryCheckContext>(
    mTolerance, QgsProject::instance()->crs(), project->transformContext(), project
  );
