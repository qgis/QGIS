...
QVariantMap QgsGeometryCheckPointInPolygonAlgorithm::processAlgorithm( const QVariantMap &parameters, QgsProcessingContext &context, QgsProcessingFeedback *feedback )
{
  ...
  std::shared_ptr<QgsVectorDataProviderFeaturePool> inputFeaturePool = std::make_shared<QgsVectorDataProviderFeaturePool>( inputLayer.get() );
  featurePools << inputFeaturePool;
  checkerFeaturePools.insert( inputLayer->id(), inputFeaturePool.get() );
  ...
  for ( QgsMapLayer *polygonLayer : polygonLayers )
  {
    ...
    {
      std::shared_ptr<QgsVectorDataProviderFeaturePool> featurePool = std::make_shared<QgsVectorDataProviderFeaturePool>( vlayer );
      checkerFeaturePools.insert( vlayer->id(), featurePool.get() );
      featurePools << featurePool;
    }
  }
