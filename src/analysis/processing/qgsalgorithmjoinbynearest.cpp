...
QVariantMap QgsJoinByNearestAlgorithm::processAlgorithm( const QVariantMap &parameters, QgsProcessingContext &context, QgsProcessingFeedback *feedback )
{
  ...
  while ( features.nextFeature( f ) )
  {
    ...
    else
    {
      ...
      {
        feedback->pushInfo( QObject::tr( "Multiple matching features found at same distance from search feature, found %n features instead of %1", nullptr, nearest.count() - ( sameSourceAndTarget ? 1 : 0 ) ).arg( neighbors ) );
      }
      QgsFeature out;
