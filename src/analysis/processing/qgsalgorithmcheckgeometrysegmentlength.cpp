...
QString QgsGeometryCheckSegmentLengthAlgorithm::name() const
{
  return u"checkgeometrysegmentlength"_s;
}
...
QString QgsGeometryCheckSegmentLengthAlgorithm::group() const
{
  return QObject::tr( "Check geometry" );
}

QString QgsGeometryCheckSegmentLengthAlgorithm::groupId() const
{
  return u"checkgeometry"_s;
}

QString QgsGeometryCheckSegmentLengthAlgorithm::shortHelpString() const
{
  return QObject::tr(
...
void QgsGeometryCheckSegmentLengthAlgorithm::initAlgorithm( const QVariantMap &configuration )
{
  ...
  addParameter( new QgsProcessingParameterFeatureSink( u"ERRORS"_s, QObject::tr( "Short segments errors" ), Qgis::ProcessingSourceType::VectorPoint ) );
  addParameter( new QgsProcessingParameterFeatureSink( u"OUTPUT"_s, QObject::tr( "Short segments features" ), Qgis::ProcessingSourceType::VectorAnyGeometry, QVariant(), true, false ) );

  addParameter( new QgsProcessingParameterNumber( u"MIN_SEGMENT_LENGTH"_s, QObject::tr( "Minimum segment length" ), Qgis::ProcessingNumberParameterType::Double, 0, false, 0.0 ) );
...
bool QgsGeometryCheckSegmentLengthAlgorithm::prepareAlgorithm( const QVariantMap &parameters, QgsProcessingContext &context, QgsProcessingFeedback * )
{
...
QgsFields QgsGeometryCheckSegmentLengthAlgorithm::outputFields()
{
  QgsFields fields;
...
QVariantMap QgsGeometryCheckSegmentLengthAlgorithm::processAlgorithm( const QVariantMap &parameters, QgsProcessingContext &context, QgsProcessingFeedback *feedback )
{
  ...
  const double segmentLength = parameterAsDouble( parameters, u"MIN_SEGMENT_LENGTH"_s, context );

  QVariantMap configurationCheck;
  configurationCheck.insert( "minSegmentLength", segmentLength );
  const QgsGeometrySegmentLengthCheck check( &checkContext, configurationCheck );
