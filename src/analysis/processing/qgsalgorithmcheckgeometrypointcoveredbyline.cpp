...
QVariantMap QgsGeometryCheckPointCoveredByLineAlgorithm::processAlgorithm( const QVariantMap &parameters, QgsProcessingContext &context, QgsProcessingFeedback *feedback )
{
  ...
  const QgsGeometryPointCoveredByLineCheck check( checkContext.get(), QVariantMap() );
  ...
  {
    auto vlayer = dynamic_cast<QgsVectorLayer *>( lineLayer );
    if ( vlayer && vlayer->geometryType() == Qgis::GeometryType::Line )
      featurePools << std::make_unique<QgsVectorDataProviderFeaturePool>( vlayer );
  }
  QMap<QString, QgsFeaturePool *> checkerFeaturePools;
  for ( const std::shared_ptr<QgsVectorDataProviderFeaturePool> &fp : featurePools )
  {
    checkerFeaturePools.insert( fp->layer()->id(), fp.get() );
  }
