...
void QgsGeometryCheckGapAlgorithm::initAlgorithm( const QVariantMap &configuration )
{
  ...
  addParameter( new QgsProcessingParameterNumber( u"GAP_THRESHOLD"_s, QObject::tr( "Gap threshold" ), Qgis::ProcessingNumberParameterType::Double, 0, false, 0.0 ) );

  // Optional allowed gaps layer and buffer value
  addParameter(
    new QgsProcessingParameterVectorLayer( u"ALLOWED_GAPS_LAYER"_s, QObject::tr( "Allowed gaps layer" ), QList<int>() << static_cast<int>( Qgis::ProcessingSourceType::VectorPolygon ), QVariant(), true )
  );
  addParameter( new QgsProcessingParameterDistance( u"ALLOWED_GAPS_BUFFER"_s, QObject::tr( "Allowed gaps buffer" ), QVariant(), u"ALLOWED_GAPS_LAYER"_s, true, 0.0 ) );

  addParameter( new QgsProcessingParameterFeatureSink( u"NEIGHBORS"_s, QObject::tr( "Neighbors layer" ), Qgis::ProcessingSourceType::Vector ) );
  addParameter( new QgsProcessingParameterFeatureSink( u"ERRORS"_s, QObject::tr( "Gap errors" ), Qgis::ProcessingSourceType::VectorPoint, QVariant(), true, false ) );
  addParameter( new QgsProcessingParameterFeatureSink( u"OUTPUT"_s, QObject::tr( "Gap features" ), Qgis::ProcessingSourceType::VectorPolygon ) );
...
QVariantMap QgsGeometryCheckGapAlgorithm::processAlgorithm( const QVariantMap &parameters, QgsProcessingContext &context, QgsProcessingFeedback *feedback )
{
  ...
  const std::unique_ptr<QgsFeatureSink> sink_neighbors( parameterAsSink( parameters, u"NEIGHBORS"_s, context, dest_neighbors, neighborsFields, Qgis::WkbType::NoGeometry ) );
  if ( !sink_neighbors )
  ...
  for ( const QgsGeometryCheckError *error : checkErrors )
  {
    ...
    attrs << inputLayer->id() << inputLayer->name() << error->vidx().part << error->vidx().ring << error->vidx().vertex << error->location().x() << error->location().y() << error->value().toString() << i;
