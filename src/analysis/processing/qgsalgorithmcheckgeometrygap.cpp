...
QVariantMap QgsGeometryCheckGapAlgorithm::processAlgorithm( const QVariantMap &parameters, QgsProcessingContext &context, QgsProcessingFeedback *feedback )
{
  ...
  QgsGeometryGapCheck check( checkContext.get(), configurationCheck );
  check.prepare( checkContext.get(), configurationCheck );
  ...
  featurePools.insert( inputLayer->id(), new QgsVectorDataProviderFeaturePool( inputLayer.get() ) );
