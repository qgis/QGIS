...
void QgsPdalDensityAlgorithm::initAlgorithm( const QVariantMap & )
{
  ...
  addParameter( new QgsProcessingParameterRasterDestination( u"OUTPUT"_s, QObject::tr( "Density" ) ) );
}
...
QStringList QgsPdalDensityAlgorithm::createArgumentLists( const QVariantMap &parameters, QgsProcessingContext &context, QgsProcessingFeedback *feedback )
{
  ...
  QStringList args = { u"density"_s, u"--input=%1"_s.arg( layer->source() ), u"--output=%1"_s.arg( outputFile ), u"--resolution=%1"_s.arg( resolution ), u"--tile-size=%1"_s.arg( tileSize ) };
