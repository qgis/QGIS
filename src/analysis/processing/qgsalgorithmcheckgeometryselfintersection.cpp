...
QVariantMap QgsGeometryCheckSelfIntersectionAlgorithm::processAlgorithm( const QVariantMap &parameters, QgsProcessingContext &context, QgsProcessingFeedback *feedback )
{
  ...
  std::unique_ptr<QgsVectorDataProviderFeaturePool> inputFeaturePool = std::make_unique<QgsVectorDataProviderFeaturePool>( inputLayer.get() );
  featurePools.insert( inputLayer->id(), inputFeaturePool.get() );
