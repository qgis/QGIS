...
/**
 * \ingroup analysis
 * \brief Adds the ability to calculate derivatives in x- and y-directions.
 *
...
class ANALYSIS_EXPORT QgsDerivativeFilter : public QgsNineCellFilter
{
  ...
  protected:
    ...
    [[nodiscard]] float calcFirstDerX( float x11, float x21, float x31, float x12, float x22, float x32, float x13, float x23, float x33 ) const;
    //! Calculates the first order derivative in y-direction according to Horn (1981)
    [[nodiscard]] float calcFirstDerY( float x11, float x21, float x31, float x12, float x22, float x32, float x13, float x23, float x33 ) const;
};
