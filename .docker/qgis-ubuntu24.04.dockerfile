...
ARG DOCKER_DEPS_TAG=release-3_44

FROM  qgis/qgis3-build-deps-24.04-qt5:${DOCKER_DEPS_TAG} AS BUILDER
LABEL maintainer="Denis Rouzaud <denis@opengis.ch>" 
...
RUN apt update && apt install -y python3-jinja2 python3-pygments python3-pexpect expect
