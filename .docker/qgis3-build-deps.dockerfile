...
# HANA: client side
# Install hdbsql tool
RUN curl -v -j -k -L -H "Cookie: eula_3_1_agreed=tools.hana.ondemand.com/developer-license-3_1.txt" http://tools.hana.ondemand.com/additional/hanaclient-2.5.86-linux-x64.tar.gz --output hanaclient-2.5.86-linux-x64.tar.gz \
  && tar -xvf hanaclient-2.5.86-linux-x64.tar.gz \
  && mkdir /usr/sap \
